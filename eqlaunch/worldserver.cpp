...
void WorldServer::Process() {
	...
	while((pack = tcpc.PopPacket())) {
		...
		case ServerOP_ZAAuthFailed: {
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "World server responded 'Not Authorized', disabling reconnect");
			pTryReconnect = false;
			Disconnect();
			break;
		}
		case ServerOP_LauncherZoneRequest: {
			if(pack->size != sizeof(LauncherZoneRequest)) {
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "Invalid size of LauncherZoneRequest: %d", pack->size);
				break;
			}
			...
			case ZR_Start: {
				if(m_zones.find(lzr->short_name) != m_zones.end()) {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "World told us to start zone %s, but it is already running.", lzr->short_name);
				} else {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "World told us to start zone %s.", lzr->short_name);
			...
			case ZR_Restart: {
				...
				if(res == m_zones.end()) {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "World told us to restart zone %s, but it is not running.", lzr->short_name);
				} else {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "World told us to restart zone %s.", lzr->short_name);
					res->second->Restart();
				}
				break;
			}
			case ZR_Stop: {
				std::map<std::string, ZoneLaunch *>::iterator res = m_zones.find(lzr->short_name);
				if(res == m_zones.end()) {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "World told us to stop zone %s, but it is not running.", lzr->short_name);
				} else {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "World told us to stop zone %s.", lzr->short_name);
					res->second->Stop();
				}
				break;
			}
		...
		default: {
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "Unknown opcode 0x%x from World of len %d", pack->opcode, pack->size);
			break;
		}
		}
		safe_delete(pack);
