...
int main(int argc, char *argv[]) {
	...
	if(launcher_name.length() < 1) {
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "You must specfify a launcher name as the first argument to this program.");
		return 1;
	}

	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "Loading server configuration..");
	if (!EQEmuConfig::LoadConfig()) {
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "Loading server configuration failed.");
		return 1;
	}
	...
	*/
	if (signal(SIGINT, CatchSignal) == SIG_ERR)	{
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "Could not set signal handler");
		return 1;
	}
	...
	*/
	...
	if (!world.Connect()) {
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "worldserver.Connect() FAILED! Will retry.");
	}
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "Starting main loop...");
...
void CatchSignal(int sig_num) {
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Launcher, "Caught signal %d", sig_num);
	RunLoops = false;
}
