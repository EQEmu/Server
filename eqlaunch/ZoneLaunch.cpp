...
ZoneLaunch::ZoneLaunch(WorldServer *world, const char *launcher_name, const char *zone_name, const EQEmuConfig *config) : m_state(StateStartPending), m_world(world), m_zone(zone_name), m_launcherName(launcher_name), m_config(config), m_timer(config->RestartWait), m_ref(ProcLauncher::ProcError), m_startCount(0), m_killFails(0) {
	m_timer.Trigger();
}
