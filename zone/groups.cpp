...
void Group::DelegateMainTank(const char *NewMainTankName, uint8 toggle)
{
	...
	if(updateDB) {
        ...
        auto results = database.QueryDatabase(query);
		if (!results.Success())
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to set group main tank: %s\n", results.ErrorMessage().c_str());
	}
}
...
void Group::DelegateMainAssist(const char *NewMainAssistName, uint8 toggle)
{
	...
	if(updateDB) {
        ...
        auto results = database.QueryDatabase(query);
		if (!results.Success())
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to set group main assist: %s\n", results.ErrorMessage().c_str());
...
void Group::DelegatePuller(const char *NewPullerName, uint8 toggle)
{
	...
	if(updateDB) {
        ...
        auto results = database.QueryDatabase(query);
		if (!results.Success())
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to set group main puller: %s\n", results.ErrorMessage().c_str());
...
void Group::UnDelegateMainTank(const char *OldMainTankName, uint8 toggle)
{
	...
	if(OldMainTankName == MainTankName) {
		...
		auto results = database.QueryDatabase(query);
		if (!results.Success())
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to clear group main tank: %s\n", results.ErrorMessage().c_str());
...
void Group::UnDelegateMainAssist(const char *OldMainAssistName, uint8 toggle)
{
	...
	if(OldMainAssistName == MainAssistName) {
        ...
        auto results = database.QueryDatabase(query);
		if (!results.Success())
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to clear group main assist: %s\n", results.ErrorMessage().c_str());
...
void Group::UnDelegatePuller(const char *OldPullerName, uint8 toggle)
{
	...
	if(OldPullerName == PullerName) {
        ...
        auto results = database.QueryDatabase(query);
		if (!results.Success())
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to clear group main puller: %s\n", results.ErrorMessage().c_str());
...
void Group::SetGroupMentor(int percent, char *name)
{
	...
	auto results = database.QueryDatabase(query);
	if (!results.Success())
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to set group mentor: %s\n", results.ErrorMessage().c_str());
}
...
void Group::ClearGroupMentor()
{
	...
	auto results = database.QueryDatabase(query);
	if (!results.Success())
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to clear group mentor: %s\n", results.ErrorMessage().c_str());
}
...
void Group::DelegateMarkNPC(const char *NewNPCMarkerName)
{
    ...
    auto results = database.QueryDatabase(query);
	if (!results.Success())
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to set group mark npc: %s\n", results.ErrorMessage().c_str());
}
...
void Group::UnDelegateMarkNPC(const char *OldNPCMarkerName)
{
    ...
    auto results = database.QueryDatabase(query);
	if (!results.Success())
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to clear group marknpc: %s\n", results.ErrorMessage().c_str());
