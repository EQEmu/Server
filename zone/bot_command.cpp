...
void bot_command_pickpocket(Client* c, const Seperator* sep)
{
	...
	auto mob_distance = (c->GetPosition() - target_mob->GetPosition());
	auto mob_xydistance = ((mob_distance.x * mob_distance.x) + (mob_distance.y * mob_distance.y));
	auto mob_zdistance = (mob_distance.z * mob_distance.z);
	if (mob_zdistance >= 25 || mob_xydistance > 250) {
	...
	auto over_level = target_mob->GetLevel();
	...
	auto steal_skill = my_bot->GetSkill(EQ::skills::SkillPickPockets);
	auto steal_chance = steal_skill * 100 / (5 * over_level + 5);

	//Determine whether to steal money or an item.
	uint32 money[6] = { 0, ((steal_skill >= 125) ? (target_npc->GetPlatinum() ) : (0)), ((steal_skill >= 60) ? (target_npc->GetGold()) : (0)), target_npc->GetSilver(), target_npc->GetCopper(), 0 };
	auto has_coin = ((money[PickPocketPlatinum] | money[PickPocketGold] | money[PickPocketSilver] | money[PickPocketCopper]) != 0);
	auto steal_item = (steal_skill >= steal_chance && (zone->random.Roll(50) || !has_coin));
	...
	while (steal_item) {
		...
		for (auto item_iter : target_npc->itemlist) {
			...
			if (item_test->Magic || !item_test->NoDrop || item_test->IsClassBag() || c->CheckLoreConflict(item_test) || item_iter->equip_slot != EQ::invslot::SLOT_INVALID) {
				continue;
			}
		...
		uint16 slot_id = c->GetInv().FindFreeSlot(false, true, (loot_selection[random].first->Size), (loot_selection[random].first->ItemType == EQ::item::ItemTypeArrow));
	...
	while (!steal_item && has_coin) {
		...
		if (zone->random.Roll(steal_chance)) { // Successful coin pickpocket
			...
			c->AddMoneyToPP(money[PickPocketCopper], money[PickPocketSilver], money[PickPocketGold], money[PickPocketPlatinum], true);
			return;
		}
