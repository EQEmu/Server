...

{
	...
	for (int i = 0; i < (HIGHEST_SKILL + 2); i++)
...
void Mob::FillSpawnStruct(NewSpawn_Struct* ns, Mob* ForWho) {
	...
	ns->spawn.x = FloatToEQ19(x_pos);
	ns->spawn.y = FloatToEQ19(y_pos);
	ns->spawn.z = FloatToEQ19(z_pos);
	...
	ns->spawn.invis = (invisible || hidden) ? 1 : 0;
	...
	if(IsNPC() && IsDestructibleObject()) {
		...
		ns->spawn.DestructibleAppearance = luclinface;
		ns->spawn.DestructibleUnk1 = 0x00000224;
		...
		ns->spawn.DestructibleUnk8 = 0x01;
		ns->spawn.DestructibleUnk9 = 0x00000002;
		ns->spawn.flymode = 0;
	}
}
...
void Mob::ShowStats(Client* client) {
	if (IsClient())
		...
		if (IsPlayerCorpse())
			client->Message(0, "  Character ID: %i  Player Corpse: %i", CastToCorpse()->getCharacterID(), CastToCorpse()->GetDBID());
		else
			client->Message(0, "  NPC Corpse", GetID());
	}
	else {
		client->Message(0, "  Level: %i  Armor Class: %i  Class: %i  Size: %1.1f  Haste: %i", GetLevel(), GetAC(), GetClass(), GetSize(), GetHaste());
		client->Message(0, "  Health: %i  Max Health: %i",GetHP(), GetMaxHP());
		client->Message(0, "  Mana: %i  Max Mana: %i", GetMana(), GetMaxMana());
		client->Message(0, "  Total Attack: %i  Worn|Spell Attack (Cap %i): %i", GetATK(), RuleI(Character, ItemATKCap), GetATKBonus());
		client->Message(0, "  Strength: %i  Stamina: %i  Dexterity: %i  Agility: %i  Intelligence: %i  Wisdom: %i  Charisma: %i", GetSTR(), GetSTA(), GetDEX(), GetAGI(), GetINT(), GetWIS(), GetCHA());
		client->Message(0, "  MR: %i  PR: %i  FR: %i  CR: %i  DR: %i Corruption: %i", GetMR(), GetPR(), GetFR(), GetCR(), GetDR(), GetCorrup());
		client->Message(0, "  Race: %i  Base Race: %i  Texture: %i  Helmet Texture: %i  Gender: %i  Base Gender: %i", GetRace(), GetBaseRace(), GetTexture(), GetHelmTexture(), GetGender(), GetBaseGender());
		if (client->Admin() >= 100)
			client->Message(0, "  Entity ID: %i  Pet ID: %i  Owner ID: %i AIControlled: %i Targeted: %i", GetID(), GetPetID(), GetOwnerID(), IsAIControlled(), targeted);
		...
		if (IsNPC()) {
			...
			client->Message(0, "  NPC ID: %u  Spawn Group ID: %u Grid: %i Loottable: %u Faction ID: %i Spells ID: %u ", GetNPCTypeID(),spawngroupid, n->GetGrid(), n->GetLoottableID(), n->GetNPCFactionID(), n->GetNPCSpellsID());
			client->Message(0, "  Accuracy: %i Merchant ID: %i Emote ID: %i Run Speed: %f Walk Speed: %f", n->GetAccuracyRating(), n->MerchantType, n->GetEmoteID(), n->GetRunspeed(), n->GetWalkspeed());
			n->QueryLoot(client);
		}
		if (IsAIControlled())
			client->Message(0, "  Aggro Range: %1.0f  Assist Range: %1.0f", GetAggroRange(), GetAssistRange());
	}
}
...
void Mob::ShowBuffs(Client* client) {
	...
	if (IsClient()) {
		...
		client->Message(0, "Str:%i Sta:%i Dex:%i Agi:%i Int:%i Wis:%i Cha:%i", itembonuses.STR,itembonuses.STA,itembonuses.DEX,itembonuses.AGI,itembonuses.INT,itembonuses.WIS,itembonuses.CHA);
		client->Message(0, "SvMagic:%i SvFire:%i SvCold:%i SvPoison:%i SvDisease:%i", itembonuses.MR,itembonuses.FR,itembonuses.CR,itembonuses.PR,itembonuses.DR);
		...
		client->Message(0, "Str:%i Sta:%i Dex:%i Agi:%i Int:%i Wis:%i Cha:%i", spellbonuses.STR,spellbonuses.STA,spellbonuses.DEX,spellbonuses.AGI,spellbonuses.INT,spellbonuses.WIS,spellbonuses.CHA);
		client->Message(0, "SvMagic:%i SvFire:%i SvCold:%i SvPoison:%i SvDisease:%i", spellbonuses.MR,spellbonuses.FR,spellbonuses.CR,spellbonuses.PR,spellbonuses.DR);
...
void Mob::SendIllusionPacket(uint16 in_race, uint8 in_gender, uint8 in_texture, uint8 in_helmtexture, uint8 in_haircolor, uint8 in_beardcolor, uint8 in_eyecolor1, uint8 in_eyecolor2, uint8 in_hairstyle, uint8 in_luclinface, uint8 in_beard, uint8 in_aa_title, uint32 in_drakkin_heritage, uint32 in_drakkin_tattoo, uint32 in_drakkin_details, float in_size) {
	...
	safe_delete(outapp);
	mlog(CLIENT__SPELLS, "Illusion: Race = %i, Gender = %i, Texture = %i, HelmTexture = %i, HairColor = %i, BeardColor = %i, EyeColor1 = %i, EyeColor2 = %i, HairStyle = %i, Face = %i, DrakkinHeritage = %i, DrakkinTattoo = %i, DrakkinDetails = %i, Size = %f", this->race, this->gender, this->texture, this->helmtexture, this->haircolor, this->beardcolor, this->eyecolor1, this->eyecolor2, this->hairstyle, this->luclinface, this->drakkin_heritage, this->drakkin_tattoo, this->drakkin_details, this->size);
}

uint8 Mob::GetDefaultGender(uint16 in_race, uint8 in_gender) {
	if ((in_race > 0 && in_race <= GNOME ) || in_race == IKSAR || in_race == VAHSHIR || in_race == FROGLOK || in_race == DRAKKIN || in_race == 15 || in_race == 50 || in_race == 57 || in_race == 70 || in_race == 98 || in_race == 118) {
		if (in_gender >= 2)
			return 1;
		else
			return in_gender;
	}
	else if (in_race == 44 || in_race == 52 || in_race == 55 || in_race == 65 || in_race == 67 || in_race == 88 || in_race == 117 || in_race == 127 || in_race == 77 || in_race == 78 || in_race == 81 || in_race == 90 || in_race == 92 || in_race == 93 || in_race == 94 || in_race == 106 || in_race == 112 || in_race == 471)
		return 0;
...
void Mob::SendSpellEffect(uint32 effectid, uint32 duration, uint32 finish_delay, bool zone_wide, uint32 unk020, bool perm_effect, Client *c) {
	...
	se->EffectID = effectid;
	se->EntityID = GetID();
	se->EntityID2 = GetID();
	se->Duration = duration;
	se->FinishDelay = finish_delay;
	se->Unknown020 = unk020;
	se->Unknown024 = 1;
	se->Unknown025 = 1;
	se->Unknown026 = 0;
...
void Mob::SetOwnerID(uint16 NewOwnerID) {
	if (NewOwnerID == GetID() && NewOwnerID != 0)
		return;
	ownerid = NewOwnerID;
...
float Mob::MobAngle(Mob *other, float ourx, float oury) const {
	...
	float mobx = -(other->GetX());
	float moby = other->GetY();
	float heading = other->GetHeading();
	heading = ((heading * 360.0f) / 256.0f);
	if (heading < 270)
		heading += 90;
	else
		heading -= 270;
	...
	vectorx = (mobx + (10.0f * cosf(heading)));
	vectory = (moby + (10.0f * sinf(heading)));
	lengthb = (float) sqrtf(((-ourx - mobx) * (-ourx - mobx)) + ((oury - moby) * (oury - moby)));
	dotp = (((vectorx - mobx) * (-ourx - mobx) + (vectory - moby) * (oury - moby)) / (10 * lengthb));
	if (dotp > 1)
		return 0.0f;
	else if (dotp < -1)
		return 180.0f;
	...
	for (int i = SLOT_RANGE; i <= SLOT_SECONDARY; i++) {
		...
		if (ItemToUse == nullptr) {
			if ((GetClass() == MONK) || (GetClass() == BEASTLORD)) {
				...
				if(speed < RuleI(Combat, MinHastedDelay) && IsClient())
					speed = RuleI(Combat, MinHastedDelay);
				TimerToUse->SetAtTrigger(speed, true);
			}
		}
...
void Mob::Say(const char *format, ...) {
	...
	if(!talker)
		talker = this;
	entity_list.MessageClose_StringID(talker, false, 200, 10, GENERIC_SAY, GetCleanName(), buf);
}
...
void Mob::Say_StringID(uint32 string_id, const char *message3, const char *message4, const char *message5, const char *message6, const char *message7, const char *message8, const char *message9) {
	...
	entity_list.MessageClose_StringID(this, false, 200, 10, GENERIC_STRINGID_SAY, GetCleanName(), string_id_str, message3, message4, message5, message6, message7, message8, message9);
}
...
void Mob::Say_StringID(uint32 type, uint32 string_id, const char *message3, const char *message4, const char *message5, const char *message6, const char *message7, const char *message8, const char *message9) {
	char string_id_str[10];
	snprintf(string_id_str, 10, "%d", string_id);
	entity_list.MessageClose_StringID(this, false, 200, type, GENERIC_STRINGID_SAY, GetCleanName(), string_id_str, message3, message4, message5, message6, message7, message8, message9);
}
...
void Mob::Shout(const char *format, ...) {
	...
	vsnprintf(buf, 1000, format, ap);
	va_end(ap);
	entity_list.Message_StringID(this, false, MT_Shout, GENERIC_SHOUT, GetCleanName(), buf);
}
...
void Mob::Emote(const char *format, ...) {
	...
	vsnprintf(buf, 1000, format, ap);
	va_end(ap);
	entity_list.MessageClose_StringID(this, false, 200, 10, GENERIC_EMOTE, GetCleanName(), buf);
}
...
int Mob::CountDispellableBuffs() {
	...
	for(int x = 0; x < buff_count; x++) {
		if(!IsValidSpell(buffs[x].spellid) || buffs[x].counters || spells[buffs[x].spellid].goodEffect == 0)
			continue;
			...
			if (IsValidSpell(spell_id)) {
				for(int i = 0; i < EFFECT_COUNT; i++) {
					if ((spells[spell_id].effectid[i] == SE_TriggerOnReqTarget) || (spells[spell_id].effectid[i] == SE_TriggerOnReqCaster)) {
						...
						if (IsHP) {
							if ((base2 >= 500 && base2 <= 520) && GetHPRatio() < ((base2 - 500) * 5) || (base2 = 1004 && GetHPRatio() < 80))
								use_spell = true;
						}

						else if (IsMana){
							if ((base2 = 521 && GetManaRatio() < 20) || (base2 = 523 && GetManaRatio() < 40) || (base2 = 38311 && GetManaRatio() < 10)) 
								use_spell = true;
						}
	...
	if (spellbonuses.FocusEffects[focusSpellVulnerability]) {	
		...
		for(int i = 0; i < buff_count; i++) {
			if((IsValidSpell(buffs[i].spellid) && IsEffectInSpell(buffs[i].spellid, SE_FcSpellVulnerability))) {
				...
				if ((tmp_focus && focus > tmp_focus) || !tmp_focus) {
					tmp_focus = focus;
					tmp_buffslot = i;
				}
...
bool Mob::TryFadeEffect(int slot) {
	if(IsValidSpell(buffs[slot].spellid)) {
		for(int i = 0; i < EFFECT_COUNT; i++) {
			if (spells[buffs[slot].spellid].effectid[i] == SE_CastOnWearoff || spells[buffs[slot].spellid].effectid[i] == SE_EffectOnFade || spells[buffs[slot].spellid].effectid[i] == SE_TriggerMeleeThreshold || spells[buffs[slot].spellid].effectid[i] == SE_TriggerSpellThreshold) {
...
bool Mob::TrySpellOnDeath() {
	if ((IsNPC() && !spellbonuses.SpellOnDeath[0] && !itembonuses.SpellOnDeath[0]) || IsClient() && !aabonuses.SpellOnDeath[0] && !spellbonuses.SpellOnDeath[0] && !itembonuses.SpellOnDeath[0])
		return false;
...
int16 Mob::GetCritDmgMob(uint16 skill) {
	int critDmg_mod = 0;
	critDmg_mod += (itembonuses.CritDmgMob[HIGHEST_SKILL + 1] + spellbonuses.CritDmgMob[HIGHEST_SKILL + 1] + aabonuses.CritDmgMob[HIGHEST_SKILL + 1] + itembonuses.CritDmgMob[skill] + spellbonuses.CritDmgMob[skill] + aabonuses.CritDmgMob[skill]);
	if(critDmg_mod < -100)
		critDmg_mod = -100;
	return critDmg_mod;
}
...
int16 Mob::GetCriticalChanceBonus(uint16 skill) {
	...
	critical_chance +=	(itembonuses.CriticalHitChance[HIGHEST_SKILL + 1] + spellbonuses.CriticalHitChance[HIGHEST_SKILL + 1] + aabonuses.CriticalHitChance[HIGHEST_SKILL + 1] + itembonuses.CriticalHitChance[skill] + spellbonuses.CriticalHitChance[skill] + aabonuses.CriticalHitChance[skill]);
...
int16 Mob::GetMeleeMinDamageMod_SE(uint16 skill) {
	int dmg_mod = 0;
	dmg_mod = (itembonuses.MinDamageModifier[skill] + spellbonuses.MinDamageModifier[skill] + itembonuses.MinDamageModifier[HIGHEST_SKILL + 1] + spellbonuses.MinDamageModifier[HIGHEST_SKILL + 1]);
	if(dmg_mod < -100)
		dmg_mod = -100;
	return dmg_mod;
}
...
void Mob::DoGravityEffect() {
	...
	if((fabs(my_x - cur_x) > 0.01) || (fabs(my_y - cur_y) > 0.01)) {
		...
		if(IsClient())
			this->CastToClient()->MovePC(zone->GetZoneID(), zone->GetInstanceID(), cur_x, cur_y, new_ground, (GetHeading() * 2));
		else
...
bool Mob::IsFacingMob(Mob *other) {
	...
	if ((angle > 472.0 && heading < 40.0) || (angle < 40.0 && heading > 472.0))
		angle = heading;
...
float Mob::HeadingAngleToMob(Mob *other) {
	...
	float angle = (atan2(x_diff, y_diff) * 180.0 * 0.3183099014828645);
...
int32 Mob::GetGlobalTime(const char* varname, uint32 charid, uint32 npcid, uint32 zoneid) {
	...
	MYSQL_RES* result;
	MYSQL_ROW row;
	if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT expdate FROM quest_globals WHERE name = '%s' AND (charid = '%u' || charid = '0') AND (npcid = '%u' || npcid = '0') AND (zoneid = '%u' || zoneid = '0')", varname, charid, npcid, zoneid), errbuf, &result)) {
		...
		LogFile->write(EQEMuLog::Debug, "Error in the GetGlobalTime() query: %s", query);
...
const char* Mob::GetGlobal(const char* varname, uint32 charid, uint32 npcid, uint32 zoneid) {
	...
	if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT value FROM quest_globals WHERE name = '%s' AND (charid = '%u' || charid = '0') AND (npcid = '%u' || npcid = '0') AND (zoneid = '%u' || zoneid = '0')", varname, charid, npcid, zoneid), errbuf, &result)) {
		...
		LogFile->write(EQEMuLog::Debug, "Error in the GetGlobal() query: %s", query);
	
	safe_delete_array(query);
	return value;
}

void Mob::SetGlobal(const char* varname, const char* newvalue, uint8 options, const char* duration) {
	...
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	uint32 duration_i = QGVarDuration(duration);

	if (this->IsNPC())
		npcid = this->GetNPCTypeID();

	if (this->IsClient())
		charid = this->CastToClient()->CharacterID();
	else
		charid = -npcid;
	...
	if (duration_i == INT_MAX)
		duration_ss << "NULL";
	else
		duration_ss << "unix_timestamp(now()) + " << duration_i;
		
	if (duration_i != 0 && !database.RunQuery(query, MakeAnyLenString(&query, "REPLACE INTO quest_globals (charid, npcid, zoneid, name, value, expdate) VALUES ('%u', '%u', '%u', '%s', '%s', %s)", charid, npcid, zoneid, varname, newvalue, duration_ss.str().c_str()), errbuf))
		LogFile->write(EQEMuLog::Debug, "Error in the SetGlobal() query: %s", query);
	else if (duration_i == 0 && !database.RunQuery(query, MakeAnyLenString(&query, "UPDATE quest_globals SET value = '%s' WHERE name = '%s' AND (charid = '%u' || charid = '0') AND (npcid = '%u' || npcid = '0') AND (zoneid = '%u' || zoneid = '0')", newvalue, varname, charid, npcid, zoneid), errbuf))
		LogFile->write(EQEMuLog::Debug, "Error in the SetGlobal() query: %s", query);
	safe_delete_array(query);	
}

void Mob::DeleteGlobal(const char* varname, uint32 charid, uint32 npcid, uint32 zoneid) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	...
	if (!database.RunQuery(query, MakeAnyLenString(&query, "DELETE FROM quest_globals WHERE name = '%s' AND (charid = '%u' || charid = '0') AND (npcid = '%u' || npcid = '0') AND (zoneid = '%u' || zoneid = '0')", varname, charid, npcid, zoneid), errbuf))
		LogFile->write(EQEMuLog::Debug, "Error in the DeleteGlobal() query: %s", query);
	safe_delete_array(query);
}

uint32 Mob::QGVarDuration(const char* fmt) {
	int duration = 0;
