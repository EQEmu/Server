...
{
	auto* entity = Find(other);
	if (entity) {
		if (in_damage > Damage::None) {
...
void HateList::AddEntToHateList(Mob *in_entity, int64 in_hate, int64 in_damage, bool in_is_entity_frenzied, bool iAddIfNotExist)
{
	...
	if (entity) {
		entity->hatelist_damage += (in_damage >= Damage::None) ? in_damage : Damage::None;
		entity->stored_hate_amount += in_hate;
...
void HateList::DamageHateList(int64 damage, uint32 distance, EntityFilterType filter_type, bool is_percentage)
{
	...
	for (const auto& h : l) {
		...
		if (is_percentage) {
			...
			e->Damage(hate_owner, total_damage, SPELL_UNKNOWN, Skill::EagleStrike);
		} else {
			e->Damage(hate_owner, damage, SPELL_UNKNOWN, Skill::EagleStrike);
		}
	}
}
