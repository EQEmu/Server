...
int16 Client::GetMaxStat() const { // Return max stat value for level
...
int32 Client::LevelRegen() {
	...
	if (level < 51) { //these calculations should match up with the info from Monkly Business, which was last updated ~05/2008: http://www.monkly-business.net/index.php?pageid=abilities
		if (sitting) {
			if (level < 20)
				hp += 2 * multiplier1;
			else if (level < 50)
	...
	else {	//there may be an easier way to calculate this next part, but I don't know what it is - level >= 51
		int32 tmp = 0;
		float multiplier2 = 1;
		if (level < 56) {
			tmp = 2;
...
int32 Client::CalcMaxHP() {
	...
	nd += aabonuses.MaxHP;	//The AA desc clearly says it only applies to base hp but the actual effect sent on live causes the client to apply it to (basehp + itemhp).. I will oblige to the client's whims over the aa description - Natural Durability, Physical Enhancement, Planar Durability
...
uint32 Client::GetClassHPFactor() { // This is for calculating Base HPs + STA bonus for SoD or later clients.
	int factor;
	switch(GetClass()) { //Note: Base HP factor under level 41 is equal to factor / 12, and from level 41 to 80 is factor / 6. Base HP over level 80 is factor / 10. HP per STA point per level is factor / 30 for level 80+. HP per STA under level 40 is the level 80 HP Per STA / 120, and for over 40 it is / 60.
		case DRUID:
		case ENCHANTER:
		case NECROMANCER:
		case MAGICIAN:
...
int32 Client::acmod() {
	...
	if (agility <= 74) {
		...
		else if (agility >= 101 && agility <= 105) {//level is >6
			if (level <= 19)
				return 29;
			else if (level <= 39)
		...
		else if (agility == 120) { //level is > 19
			if (level <= 39)
				return 42;
			else
				return 48;
	...
	else //seems about 21 agil per extra AC pt over 300...
...
int32 Client::CalcAC() { //This is a testing formula for AC, the value this returns should be the same value as the one the client shows... ac1 and ac2 are probably the damage migitation and damage avoidance numbers, not sure which is which. I forgot to include the iksar defense bonus and i cant find my notes now... AC from spells are not included (cant even cast spells yet..)
	int avoidance = (acmod() + ((GetSkill(SkillDefense) + itembonuses.HeroicAGI / 10) * 16) / 9); // new formula
	...
	if (m_pp.class_ == WIZARD || m_pp.class_ == MAGICIAN || m_pp.class_ == NECROMANCER || m_pp.class_ == ENCHANTER) {
		mitigation = ((GetSkill(SkillDefense) + itembonuses.HeroicAGI / 10) / 4 + (itembonuses.AC + 1)); //something is wrong with this, naked casters have the wrong natural AC - mitigation = (spellbonuses.AC/3) + (GetSkill(DEFENSE)/2) + (itembonuses.AC+1);
		mitigation -= 4; //this might be off by 4..
	}
	else {
		mitigation = ((GetSkill(SkillDefense) + itembonuses.HeroicAGI / 10) / 3 + ((itembonuses.AC * 4) / 3)); //mitigation = (spellbonuses.AC/4) + (GetSkill(DEFENSE)/3) + ((itembonuses.AC*4)/3);
		if(m_pp.class_ == MONK)
			mitigation += (GetLevel() * 1.3);	//the 13 / 10 might be wrong, but it is close...
	}
	
	int displayed = 0;
	displayed += (((avoidance + mitigation) * 1000) / 847);	//natural AC
	
	if (GetRace() == IKSAR) { //Iksar AC
		displayed += 12;
		...
		if (iksarlevel > 25)
			iksarlevel = 25;
		if (iksarlevel > 0)
			displayed += (iksarlevel * 1.2);
	}
	if(itembonuses.HeroicSTR) { //Shield AC bonus for HeroicSTR
		bool equipped = CastToClient()->m_inv.GetItem(14);
		if(equipped) {
	...
	displayed += spellbonuses.AC; //spell AC bonuses are added directly to natural total
	AC = displayed;
	return(AC);
}
...
int16 Client::CalcAGI() {
	...
	int16 str = GetSTR();	
	if(weight > (str * 10)) { //Encumbered penalty - AGI is halved when we double our weight, zeroed (defaults to 1) when we triple it. this includes AGI from AAs
...
int Client::CalcHaste() {
	...
	int cap = 0;
	int overhaste = 0;
	int level = GetLevel(); // 26+ no cap, 1-25 10
	if (level > 25) // 26+
		h += itembonuses.haste;
	else // 1-25
		h += itembonuses.haste > 10 ? 10 : itembonuses.haste;// 60+ 100, 51-59 85, 1-50 level+25
	...
	if (level > 50) // 51+ 25 (despite there being higher spells...), 1-50 10 - 51+
...
int16 Client::CalcMR() { //The AA multipliers are set to be 5, but were 2 on WR - The resistant discipline which I think should be here is implemented in Mob::ResistSpell
	MR = 25;
	if (GetBaseRace() == ERUDITE || GetBaseRace() == DWARF || GetBaseRace() == FROGLOK)
		MR = 30;
	else if (GetBaseRace() == DRAKKIN)
		MR = 35;
...
int16 Client::CalcFR() {
	FR = 25;
	if (GetBaseRace() == TROLL)
		FR = 5;
	else if (GetBaseRace() == IKSAR)
		FR = 30;
...
int16 Client::CalcPR() {
	PR = 15;
	if (GetBaseRace() == DWARF || GetBaseRace() == HALFLING)
		PR = 20;
	else if (GetBaseRace() == FROGLOK)
		PR = 30;
...
uint16 Mob::GetInstrumentMod(uint16 spell_id) const {
	...
	switch (spells[spell_id].skill) { //this should never use spell modifiers... if a spell grants better modifiers, they are copied into the item mods because the spells are supposed to act just like having the instrument. item mods are in 10ths of percent increases
