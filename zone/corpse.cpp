...
ServerLootItem_Struct* Corpse::GetItem(uint16 lootslot, ServerLootItem_Struct** bag_item_data) {
	...
	if (sitem && bag_item_data && EQ::InventoryProfile::SupportsContainers(sitem->equip_slot)) {
		...
		for (; cur != end; ++cur) {
			sitem2 = *cur;
			if (sitem2->equip_slot >= bagstart && sitem2->equip_slot < bagstart + EQ::invbag::SLOT_COUNT) {
...
void Corpse::LootItem(Client *client, const EQApplicationPacket *app)
{
	...
	ServerLootItem_Struct *item_data = nullptr, *bag_item_data[EQ::invbag::SLOT_COUNT] = {};
