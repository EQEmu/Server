...
void TitleManager::CreateNewPlayerTitle(Client *c, const char *Title) {
	...
	if (database.RunQuery(query, MakeAnyLenString(&query, "SELECT `id` from titles where `prefix` = '%s' and char_id = %i", EscTitle, c->CharacterID()), errbuf, &result)) {
	...
	safe_delete_array(query);
	if(!database.RunQuery(query, MakeAnyLenString(&query, "INSERT into titles (`char_id`, `prefix`) VALUES(%i, '%s')", c->CharacterID(), EscTitle), errbuf))
...
void TitleManager::CreateNewPlayerSuffix(Client *c, const char *Suffix) {
	...
	if (database.RunQuery(query, MakeAnyLenString(&query, "SELECT `id` from titles where `suffix` = '%s' and char_id = %i", EscSuffix, c->CharacterID()), errbuf, &result)) {
		if(mysql_num_rows(result) > 0) {
			mysql_free_result(result);
			safe_delete_array(query);
	...
	if(!database.RunQuery(query,MakeAnyLenString(&query, "INSERT into titles (`char_id`, `suffix`) VALUES(%i, '%s')", c->CharacterID(), EscSuffix), errbuf))
