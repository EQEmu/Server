...
bool TitleManager::LoadTitles()
{
	...
	if (!results.Success()) {
		logger.Log(EQEmuLogSys::Error, "Unable to load titles: %s : %s", query.c_str(), results.ErrorMessage().c_str());
		return false;
	}
...
void TitleManager::CreateNewPlayerTitle(Client *client, const char *title)
{
	...
	if(!results.Success()) {
		logger.Log(EQEmuLogSys::Error, "Error adding title: %s %s", query.c_str(), results.ErrorMessage().c_str());
        return;
    }
...
void TitleManager::CreateNewPlayerSuffix(Client *client, const char *suffix)
{
    ...
    results = database.QueryDatabase(query);
	if(!results.Success()) {
		logger.Log(EQEmuLogSys::Error, "Error adding title suffix: %s %s", query.c_str(), results.ErrorMessage().c_str());
        return;
    }
...
void Client::EnableTitle(int titleSet) {
    ...
    auto results = database.QueryDatabase(query);
	if(!results.Success())
		logger.Log(EQEmuLogSys::Error, "Error in EnableTitle query for titleset %i and charid %i", titleSet, CharacterID());
...
bool Client::CheckTitle(int titleSet) {
    ...
    auto results = database.QueryDatabase(query);
	if (!results.Success()) {
        logger.Log(EQEmuLogSys::Error, "Error in CheckTitle query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
        return false;
	}
...
void Client::RemoveTitle(int titleSet) {
    ...
    auto results = database.QueryDatabase(query);
	if (!results.Success())
		logger.Log(EQEmuLogSys::Error, "Error in RemoveTitle query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
