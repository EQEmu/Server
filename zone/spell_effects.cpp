		...
		switch(effect)
		{
			...
			{				
				...
				if(IsClient())
				{
					...
					if(!target_zone) {
#ifdef SPELL_EFFECT_SPAM
						Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Succor/Evacuation Spell In Same Zone.");
#endif
							if(IsClient())
...
#ifdef SPELL_EFFECT_SPAM
						Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Succor/Evacuation Spell To Another Zone.");
#endif
						if(IsClient())
			...
			case SE_Revive:
			{
				...
				if (IsCorpse() && CastToCorpse()->IsPlayerCorpse()) {
					...
					if(caster)
						Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Spells, " corpse being rezzed using spell %i by %s",
			...
			case SE_SummonCorpse:
			{
				...
				if(!IsNPC()) {
					...
					if(TargetClient) {
						...
						Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "%s attempted to cast spell id %u with spell effect SE_SummonCorpse, but could not cast target into a Client object.", GetCleanName(), spell_id);
					}
				}
...
int Mob::CalcSpellEffectValue(uint16 spell_id, int effect_id, int caster_level, Mob *caster, int ticsremaining)
{
	...
	(spells[spell_id].effectid[effect_id] != SE_ManaRegen_v2))
	{
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Effect value %d altered with bard modifier of %d to yeild %d", oval, mod, effect_value);
	}
...
*/
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "CSEV: spell %d, formula %d, base %d, max %d, lvl %d. Up/Down %d",
	...
	switch(formula)
	{
		...
		default:
		{
			...
			else if((formula >= 2000) && (formula <= 2650))
			{
				...
				Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Unknown spell effect value forumula %d", formula);
		}
	}
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Result: %d (orig %d), cap %d %s", result, oresult, max, (base < 0 && result > 0)?"Inverted due to negative base":"");
			...
			if(spells[buffs[buffs_i].spellid].buffdurationformula != DF_Permanent)
			{
				if(!zone->BuffTimersSuspended() || !IsSuspendableSpell(buffs[buffs_i].spellid))
				{
					...
					if (buffs[buffs_i].ticsremaining == 0) {
						...
						{
							Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Buff %d in slot %d has expired. Fading.", buffs[buffs_i].spellid, buffs_i);
					...
					{
						Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Buff %d in slot %d has %d tics remaining.", buffs[buffs_i].spellid, buffs_i, buffs[buffs_i].ticsremaining);
					}
				}
...
void Mob::BuffFadeBySlot(int slot, bool iRecalcBonuses)
{
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Fading buff %d from slot %d", buffs[slot].spellid, slot);
...
int16 Mob::CalcFocusEffect(focusType type, uint16 focus_id, uint16 spell_id, bool best_focus) {
	...
	*/ 
	...
	for (int i = 0; i < EFFECT_COUNT; i++) {
		...
		case SE_LimitSpellType:
			switch( focus_spell.base[i]){
				...
				case 1:
					...
					break;
				default:
					Log.Out(EQEmuLogSys::General, EQEmuLogSys::Normal, "CalcFocusEffect: unknown limit spelltype %d", focus_spell.base[i]);
			}
			break;
		...
		default:
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Normal, "CalcFocusEffect: unknown effectid %d", focus_spell.effectid[i]);
#endif
		}
