...
void Raid::AddMember(Client *c, uint32 group, bool rleader, bool groupleader, bool looter){
	...
	if(!results.Success()) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error inserting into raid members: %s", results.ErrorMessage().c_str());
	}
...
void Raid::SetRaidLeader(const char *wasLead, const char *name)
{
	...
	auto results = database.QueryDatabase(query);
	if (!results.Success())
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Set Raid Leader error: %s\n", results.ErrorMessage().c_str());
...
void Raid::SaveGroupLeaderAA(uint32 gid)
{
	...
	auto results = database.QueryDatabase(query);
	if (!results.Success())
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to store LeadershipAA: %s\n", results.ErrorMessage().c_str());
}
...
void Raid::GetRaidDetails()
{
    ...
    if (results.RowCount() == 0) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error getting raid details for raid %lu: %s", (unsigned long)GetID(), results.ErrorMessage().c_str());
        return;
    }
...
bool Raid::LearnMembers()
{
	...
	if(results.RowCount() == 0) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error getting raid members for raid %lu: %s", (unsigned long)GetID(), results.ErrorMessage().c_str());
        disbandCheck = true;
        return false;
    }
...
void Raid::SetGroupMentor(uint32 group_id, int percent, char *name)
{
	...
	auto results = database.QueryDatabase(query);
	if (!results.Success())
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to set raid group mentor: %s\n", results.ErrorMessage().c_str());
}
...
void Raid::ClearGroupMentor(uint32 group_id)
{
	...
	auto results = database.QueryDatabase(query);
	if (!results.Success())
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unable to clear raid group mentor: %s\n", results.ErrorMessage().c_str());
}
