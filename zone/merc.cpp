...
int32 Merc::CalcMaxHP() {
	...
	int hp_perc_cap = spellbonuses.HPPercCap[RESOURCE_PERCENT_CAP];
	if(hp_perc_cap) {
		int curHP_cap = (max_hp * hp_perc_cap) / 100;
		if (current_hp > curHP_cap || (spellbonuses.HPPercCap[RESOURCE_AMOUNT_CAP] && current_hp > spellbonuses.HPPercCap[RESOURCE_AMOUNT_CAP]))
...
int32 Merc::CalcMaxMana()
{
	...
	int mana_perc_cap = spellbonuses.ManaPercCap[RESOURCE_PERCENT_CAP];
	if(mana_perc_cap) {
		int curMana_cap = (max_mana * mana_perc_cap) / 100;
		if (current_mana > curMana_cap  || (spellbonuses.ManaPercCap[RESOURCE_AMOUNT_CAP] && current_mana > spellbonuses.ManaPercCap[RESOURCE_AMOUNT_CAP]))
...
void Merc::CalcMaxEndurance()
{
	...
	int end_perc_cap = spellbonuses.EndPercCap[RESOURCE_PERCENT_CAP];
	if(end_perc_cap) {
		int curEnd_cap = (max_end * end_perc_cap) / 100;
		if (cur_end > curEnd_cap || (spellbonuses.EndPercCap[RESOURCE_AMOUNT_CAP] && cur_end > spellbonuses.EndPercCap[RESOURCE_AMOUNT_CAP]))
			cur_end = curEnd_cap;
	}
}
		...
		if(atCombatRange)
		{
			...
			if(!IsMercCaster() && GetTarget() && !IsStunned() && !IsMezzed() && (GetAppearance() != eaDead))
			{
				...
				if(attack_timer.Check())
				{
					...
					int16 ExtraAttackChanceBonus = spellbonuses.ExtraAttackChance[EXTRA_ATTACK_CHANCE] + itembonuses.ExtraAttackChance[EXTRA_ATTACK_CHANCE] + aabonuses.ExtraAttackChance[EXTRA_ATTACK_CHANCE];
