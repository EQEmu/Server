	...
	if (bot_owner) {
		...
		if (bot_owner->HasGroup()) {
			...
			for (uint8 class_id = WARRIOR; class_id <= BERSERKER; class_id++) {
				auto bot_spawn_limit = bot_owner->GetBotSpawnLimit(class_id);
				bot_class_spawn_limits.push_back(bot_spawn_limit);
			}
...
void Bot::SpawnBotGroupByName(Client* c, std::string botgroup_name, uint32 leader_id)
{
	...
	for (uint8 class_id = WARRIOR; class_id <= BERSERKER; class_id++) {
		auto bot_spawn_limit = c->GetBotSpawnLimit(class_id);
		bot_class_spawn_limits.push_back(bot_spawn_limit);
	}
