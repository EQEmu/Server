...
void Client::NukeTraderItem(uint16 Slot,int16 Charges,uint16 Quantity,Client* Customer,uint16 TraderSlot, int SerialNumber) {
	...
	if(!Customer) return;
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "NukeTraderItem(Slot %i, Charges %i, Quantity %i", Slot, Charges, Quantity);
	if(Quantity < Charges) {
...
void Client::BuyTraderItem(TraderBuy_Struct* tbs,Client* Trader,const EQApplicationPacket* app){
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Buyitem: Name: %s, IsStackable: %i, Requested Quantity: %i, Charges on Item %i",
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Actual quantity that will be traded is %i", outtbs->Quantity);
...
static void UpdateTraderCustomerPriceChanged(uint32 CustomerID, TraderCharges_Struct* gis, uint32 ItemID, int32 Charges, uint32 NewPrice) {
	...
	if(NewPrice == 0) {
		...
		for(int i = 0; i < 80; i++) {
			...
			if(gis->ItemID[i] == ItemID) {
				tdis->ItemID = gis->SerialNumber[i];
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Telling customer to remove item %i with %i charges and S/N %i",
...
void Client::HandleTraderPriceUpdate(const EQApplicationPacket *app) {
	...
	if(IDOfItemToUpdate == 0) {
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Unable to find item to update price for. Rechecking trader satchels");
...
void Client::SendBuyerResults(char* searchString, uint32 searchID) {
	...
	//
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "[CLIENT] Client::SendBuyerResults %s\n", searchString);
	...
	auto results = database.QueryDatabase(query);
    if (!results.Success()) {
        Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::None, "[CLIENT] Failed to retrieve Barter Search!! %s %s\n", query.c_str(), results.ErrorMessage().c_str());
        return;
    }
	...
	if(!item->Stackable) {
		...
		for(uint32 i = 0; i < Quantity; i++) {
			...
			if (SellerSlot == INVALID_INDEX) {
				...
				Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unexpected error while moving item from seller to buyer.");
...
void Client::UpdateBuyLine(const EQApplicationPacket *app) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "UpdateBuyLine: Char: %s BuySlot: %i ItemID %i %s Quantity %i Toggle: %i Price %i ItemCount %i LoreConflict %i",
	...
	if((item->NoDrop != 0) && !LoreConflict && (Quantity > 0) && HasMoney(Quantity * Price) && ToggleOnOff && (ItemCount == 0)) {
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Adding to database");
