...
void QGlobalCache::LoadByNPCID(uint32 npcID) {
	...
	if (database.RunQuery(query, MakeAnyLenString(&query, "select name, charid, npcid, zoneid, value, expdate from quest_globals where npcid = %d", npcID), errbuf, &result)) {
...
void QGlobalCache::LoadByCharID(uint32 charID) {
	...
	if (database.RunQuery(query, MakeAnyLenString(&query, "select name, charid, npcid, zoneid, value, expdate from quest_globals where charid = %d && npcid = 0", charID), errbuf, &result)) {
		while((row = mysql_fetch_row(result)))
			AddGlobal(0, QGlobal(std::string(row[0]), atoi(row[1]), atoi(row[2]), atoi(row[3]), row[4], row[5]?atoi(row[5]):0xFFFFFFFF));
		mysql_free_result(result);
	}
...
void QGlobalCache::LoadByZoneID(uint32 zoneID) {
	...
	if (database.RunQuery(query, MakeAnyLenString(&query, "select name, charid, npcid, zoneid, value, expdate from quest_globals where zoneid = %d && npcid = 0 && charid = 0", zoneID), errbuf, &result)) {
		while((row = mysql_fetch_row(result)))
			AddGlobal(0, QGlobal(std::string(row[0]), atoi(row[1]), atoi(row[2]), atoi(row[3]), row[4], row[5]?atoi(row[5]):0xFFFFFFFF));
		mysql_free_result(result);
	}
...
void QGlobalCache::LoadByGlobalContext() {
	...
	if (database.RunQuery(query, MakeAnyLenString(&query, "select name, charid, npcid, zoneid, value, expdate from quest_globals where zoneid = 0 && npcid = 0 && charid = 0"), errbuf, &result)) {
		while((row = mysql_fetch_row(result)))
			AddGlobal(0, QGlobal(std::string(row[0]), atoi(row[1]), atoi(row[2]), atoi(row[3]), row[4], row[5]?atoi(row[5]):0xFFFFFFFF));
		mysql_free_result(result);
	}
