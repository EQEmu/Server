...
bool Mob::CastSpell(uint16 spell_id, uint16 target_id, CastingSlot slot,
	...
	uint32 aa_id)
{
	LogSpells("Mob::CastSpell(): called for spell [{}] ([{}]) on entity [{}], slot [{}], time [{}], mana [{}], from item slot [{}]",
	...
	{
		LogSpells("Mob::CastSpell(): Spell casting canceled: not able to cast now. Valid? [{}], casting [{}], waiting? [{}], spellend? [{}], stunned? [{}], feared? [{}], mezed? [{}], silenced? [{}], amnesiad? [{}]",
	...
	if(DivineAura()) {
		LogSpells("Mob::CastSpell(): Spell casting canceled: cannot cast while Divine Aura is in effect");
	...
	if(resist_adjust)
	{
		LogSpells("Mob::CastSpell(): true resist_adjust");
	...
	{
		LogSpells("Mob::CastSpell(): fail resist_adjust");
					...
					int16 resist_adjust, uint32 aa_id)
{
	...
	LogSpells("Mob::DoCastSpell(): called for spell [{}] ([{}]) on entity [{}], slot [{}], time [{}], mana [{}], from item [{}]",
	...
	if (slot < CastingSlot::MaxGems && !CheckFizzle(spell_id)) {
		...
		LogSpells("Mob::DoCastSpell(): Spell casting canceled: fizzled. [{}] mana has been consumed", use_mana);
	...
	SaveSpellLoc();
	LogSpells("Mob::DoCastSpell(): Casting [{}] Started at ({},{},{})", spell_id, m_SpellLocation.x, m_SpellLocation.y, m_SpellLocation.z);
	...
	{
		LogSpells("Mob::DoCastSpell(): Spell [{}] auto-targeted the caster. Group? [{}], target type [{}]", spell_id, IsGroupSpell(spell_id), spell.targettype);
		target_id = GetID();
	}
	...
	if(target_id == 0) {
		LogSpells("Mob::DoCastSpell(): Spell Error: no target. spell=[{}]", spell_id);
		if(IsClient()) {
	...
	if(mana_cost > 0 && slot != CastingSlot::Item)
	{
		...
		if(my_curmana < mana_cost)	// not enough mana
		{
			...
			if(IsNPC() && my_curmana == my_maxmana)
			{
				...
				LogSpells("Mob::DoCastSpell(): Spell Error not enough mana spell=[{}] mymana=[{}] cost=[{}]\n", spell_id, my_curmana, mana_cost);
				if(IsClient()) {
					//clients produce messages... npcs should not for this case
	...
	LogSpells("Mob::DoCastSpell(): Spell [{}]: Casting time [{}] (orig [{}]), mana cost [{}]",
...
bool Mob::DoCastingChecks()
{
	...
	if (RuleB(Spells, BuffLevelRestrictions)) {
		...
		if (spell_target && spells[spell_id].targettype != ST_Self && !spell_target->CheckSpellLevelRestriction(spell_id)) {
			LogSpells("Mob::DoCastingChecks(): Spell [{}] failed: recipient did not meet the level restrictions", spell_id);
...
bool Client::CheckFizzle(uint16 spell_id)
{
	...
	LogSpells("Client::CheckFizzle(): Adjusted casting skill: [{}]+[{}]+[{}]+[{}]+[{}]=[{}]", GetSkill(spells[spell_id].skill), GetLevel(), itembonuses.adjusted_casting_skill, spellbonuses.adjusted_casting_skill, aabonuses.adjusted_casting_skill, act_skill);
	...
	LogSpells("Client::CheckFizzle(): Check Fizzle [{}] spell [{}] fizzlechance: [{}]  diff: [{}] roll: [{}]", GetName(), spell_id, fizzlechance, diff, fizzle_roll);
							...
							uint16 mana_used, uint32 inventory_slot, int16 resist_adjust)
{
	...
	if(IsClient() && slot != CastingSlot::Item && slot != CastingSlot::PotionBelt && spells[spell_id].recast_time > 1000) { // 10 is item
		if(!CastToClient()->GetPTimers().Expired(&database, pTimerSpellStart + spell_id, false)) {
			...
			LogSpells("Mob::CastedSpellFinished(): Casting of [{}] canceled: spell reuse timer not expired", spell_id);
			StopCasting();
			return;
		}
	}
	...
	if(IsClient() && (slot == CastingSlot::Item || slot == CastingSlot::PotionBelt))
	{
		...
		if(item && item->GetItem()->RecastDelay > 0)
		{
			if(!CastToClient()->GetPTimers().Expired(&database, (pTimerItemStart + item->GetItem()->RecastType), false)) {
				MessageString(Chat::Red, SPELL_RECAST);
				LogSpells("Mob::CastedSpellFinished(): Casting of [{}] canceled: item spell reuse timer not expired", spell_id);
				StopCasting();
				return;
			}
		}
	...
	if(!IsValidSpell(spell_id))
	{
		LogSpells("Mob::CastedSpellFinished(): Casting of [{}] canceled: invalid spell id", spell_id);
		InterruptSpell();
		return;
	}
	...
	if(IsClient())
	{
		if(delaytimer)
		{
			LogSpells("Mob::CastedSpellFinished(): Casting of [{}] canceled: recast too quickly", spell_id);
	...
	if (casting_spell_id != spell_id)
	{
		LogSpells("Mob::CastedSpellFinished(): Casting of [{}] canceled: already casting", spell_id);
	...
	if(GetClass() == BARD) // bard's can move when casting any spell...
	{
		if (IsBardSong(spell_id)) {
			if(spells[spell_id].buffduration == 0xFFFF) {
				LogSpells("Mob::CastedSpellFinished(): Bard song [{}] not applying bard logic because duration. dur=[{}], recast=[{}]", spells[spell_id].buffduration);
			} else {
				...
				LogSpells("Mob::CastedSpellFinished(): Bard song [{}] started: slot [{}], target id [{}]", bardsong, (int) bardsong_slot, bardsong_target_id);
				bard_song_mode = true;
			}
		}
	}
	else // not bard, check movement
	{
		...
		)
		{
			...
			LogSpells("Mob::CastedSpellFinished(): Checking Interruption: spell x: [{}] spell y: [{}] cur x: [{}] cur y: [{}] channelchance [{}] channeling skill [{}]\n", GetSpellX(), GetSpellY(), GetX(), GetY(), channelchance, GetSkill(EQ::skills::SkillChanneling));

			if(!spells[spell_id].uninterruptable && zone->random.Real(0, 100) > channelchance) {
				LogSpells("Mob::CastedSpellFinished(): Casting of [{}] canceled: interrupted", spell_id);
				InterruptSpell();
				return;
			}
	...
	if(IsClient()) {
		...
		if (item && item->GetItem() && item->GetItem()->CastTime == 0) {
			LogSpells("Mob::CastedSpellFinished(): Spell [{}]: Casted from instant clicky, prevent reagent consumption", spell_id);
		} else if(zone->random.Roll(reg_focus)) {
			LogSpells("Mob::CastedSpellFinished(): Spell [{}]: Reagent focus item prevented reagent consumption ([{}] chance)", spell_id, reg_focus);
		} else {
			if(reg_focus > 0)
				LogSpells("Mob::CastedSpellFinished(): Spell [{}]: Reagent focus item failed to prevent reagent consumption ([{}] chance)", spell_id, reg_focus);
				...
				if(bard_song_mode) {
					...
					switch (InstComponent) {
						...
						default:	// some non-instrument component. Let it go, but record it in the log
							LogSpells("Mob::CastedSpellFinished(): Something odd happened: Song [{}] required component [{}]", spell_id, component);
					}

					if(!HasInstrument) {	// if the instrument is missing, log it and interrupt the song
						LogSpells("Mob::CastedSpellFinished(): Song [{}]: Canceled. Missing required instrument [{}]", spell_id, component);
						if(c->GetGM())
				...
				// handle the components for traditional casters
					...
					else if(c->GetInv().HasItem(component, component_count, invWhereWorn|invWherePersonal) == -1) // item not found
					{
						...
						if(item) {
							...
							LogSpells("Mob::CastedSpellFinished(): Spell [{}]: Canceled. Missing required reagent [{}] ([{}])", spell_id, item->Name, component);
						}
						else {
							char TempItemName[64];
							strcpy((char*)&TempItemName, "UNKNOWN");
							LogSpells("Mob::CastedSpellFinished(): Spell [{}]: Canceled. Missing required reagent [{}] ([{}])", spell_id, TempItemName, component);
						}
					}
			...
			else if (!bard_song_mode)
			{
				...
				for(int t_count = 0; t_count < 4; t_count++) {
					...
					if (component == -1 || noexpend == component)
						continue;
					component_count = spells[spell_id].component_counts[t_count];
					LogSpells("Mob::CastedSpellFinished(): Spell [{}]: Consuming [{}] of spell component item id [{}]", spell_id, component_count, component);
		...
		&& inventory_slot != 0xFFFFFFFF)	// 10 is an item
	{
		...
		if (item && item->IsClassCommon() && (item->GetItem()->Click.Effect == spell_id) && item->GetCharges() || fromaug)
		{
			...
			if(charges > -1) {	// charged item, expend a charge
				LogSpells("Mob::CastedSpellFinished(): Spell [{}]: Consuming a charge from item [{}] ([{}]) which had [{}]/[{}] charges", spell_id, item->GetItem()->Name, item->GetItem()->ID, item->GetCharges(), item->GetItem()->MaxCharges);
				DeleteChargeFromSlot = inventory_slot;
			} else {
				LogSpells("Mob::CastedSpellFinished(): Spell [{}]: Cast from unlimited charge item [{}] ([{}]) ([{}] charges)", spell_id, item->GetItem()->Name, item->GetItem()->ID, item->GetItem()->MaxCharges);
			}
		}
		else
		{
			LogSpells("Mob::CastedSpellFinished(): Item used to cast spell [{}] was missing from inventory slot [{}] after casting!", spell_id, inventory_slot);
	...
	{
		LogSpells("Mob::CastedSpellFinished(): Casting of [{}] canceled: SpellFinished returned false", spell_id);
	...
	if(bard_song_mode)
	{
		...
		LogSpells("Mob::CastedSpellFinished(): Bard song [{}] should be started", spell_id);
	}
	else
	{
		if(IsClient())
	...
	LogSpells("Mob::CastedSpellFinished(): Spell casting of [{}] is finished", spell_id);
...
*/
	...
	if(IsPlayerIllusionSpell(spell_id)
		...
		|| IsRaidGrouped())
		&& (HasProjectIllusion())){
			LogAA("Mob::DetermineSpellTargets(): Project Illusion overwrote target caster: [{}] spell id: [{}] was ON", GetName(), spell_id);
...
// single target spells
		...
		case ST_TargetOptional:
		{
			if (!spell_target)
			{
				LogSpells("Mob::DetermineSpellTargets(): Spell [{}] canceled: invalid target (normal)", spell_id);
				MessageString(Chat::Red, SPELL_NEED_TAR);
		...
		case ST_Undead: {
			...
			{
				//invalid target
				LogSpells("Mob::DetermineSpellTargets(): Spell [{}] canceled: invalid target of body type [{}] (undead)", spell_id, mob_body);
				if(!spell_target)
					MessageString(Chat::Red,SPELL_NEED_TAR);
				else
		...
		case ST_Summoned: {
			...
			{
				//invalid target
				LogSpells("Mob::DetermineSpellTargets(): Spell [{}] canceled: invalid target of body type [{}] (summoned)", spell_id, mob_body);
				MessageString(Chat::Red,SPELL_NEED_TAR);
				return false;
			}
			CastAction = SingleTarget;
		...
		case ST_SummonedPet:
		{
			...
			{
				LogSpells("Mob::DetermineSpellTargets(): Spell [{}] canceled: invalid target of body type [{}] (summoned pet)",
							spell_id, mob_body);
...

		{
			...
			{
				//invalid target
				LogSpells("Mob::DetermineSpellTargets(): Spell [{}] canceled: invalid target of body type [{}] (want body Type [{}])", spell_id, mob_body, target_bt);
				if(!spell_target)
					MessageString(Chat::Red,SPELL_NEED_TAR);
				else
					MessageString(Chat::Red,CANNOT_AFFECT_NPC);
		...
		case ST_Target: {
			if(IsLDoNObjectSpell(spell_id))
			{
				if(!spell_target)
				{
					LogSpells("Mob::DetermineSpellTargets(): Spell [{}] canceled: invalid target (ldon object)", spell_id);
					MessageString(Chat::Red,SPELL_NEED_TAR);
					return false;
				}
				else
		...
		case ST_Corpse:
		{
			...
			{
				LogSpells("Mob::DetermineSpellTargets(): Spell [{}] canceled: invalid target (corpse)", spell_id);
		...
		case ST_Pet:
		{
			...
			if(!spell_target)
			{
				LogSpells("Mob::DetermineSpellTargets(): Spell [{}] canceled: invalid target (no pet)", spell_id);
		...
		case ST_TargetAENoPlayersPets:
		{
			if(!spell_target)
			{
				LogSpells("Mob::DetermineSpellTargets(): Spell [{}] canceled: invalid target (AOE)", spell_id);
				MessageString(Chat::Red,SPELL_NEED_TAR);
				return false;
			}
			ae_center = spell_target;
		...
		case ST_GroupClientAndPet:
		{
			if(!spell_target)
			{
				LogSpells("Mob::DetermineSpellTargets(): Spell [{}] canceled: invalid target (Group Required: Single Target)", spell_id);
				MessageString(Chat::Red,SPELL_NEED_TAR);
				return false;
			}
			...
			if(spell_target != this)
			{
				...
				else
				{
					...
					{
						LogSpells("Mob::DetermineSpellTargets(): Spell [{}] canceled: Attempted to cast a Single Target Group spell on a ungrouped member", spell_id);
		...
		default:
		{
			LogSpells("Mob::DetermineSpellTargets(): I dont know Target Type: [{}]  Spell: ([{}]) [{}]", spells[spell_id].targettype, spell_id, spells[spell_id].name);
...
bool Mob::SpellFinished(uint16 spell_id, Mob *spell_target, CastingSlot slot, uint16 mana_used, uint32 inventory_slot, int16 resist_adjust, bool isproc, int level_override)
{
  ...
  //Guard Assist Code
	...
	if (IsClient() && CastToClient()->GetGM()){
		if (zone->IsSpellBlocked(spell_id, glm::vec3(GetPosition()))){
			LogSpells("Mob::SpellFinished(): GM Cast Blocked Spell: [{}] (ID [{}])", GetSpellName(spell_id), spell_id);
		}
	}
	...
	LogSpells("Mob::SpellFinished(): Spell [{}]: target type [{}], target [{}], AE center [{}]", spell_id, CastAction, spell_target?spell_target->GetName():"NONE", ae_center?ae_center->GetName():"NONE");
	...
	if(IsAEDurationSpell(spell_id)) {
		...
		LogSpells("Mob::SpellFinished(): Spell [{}]: AE duration beacon created, entity id [{}]", spell_id, beacon->GetName());
		spell_target = nullptr;
		ae_center = beacon;
		CastAction = AECaster;
	}
	...
	if(spell_target != nullptr && spell_target != this) {
		...
		if(dist2 > range2) {
			...
			LogSpells("Mob::SpellFinished(): Spell [{}]: Spell target is out of range (squared: [{}] > [{}])", spell_id, dist2, range2);
			MessageString(Chat::Red, TARGET_OUT_OF_RANGE);
			return(false);
		}
		else if (dist2 < min_range2){
			//target is too close range.
			LogSpells("Mob::SpellFinished(): Spell [{}]: Spell target is too close (squared: [{}] < [{}])", spell_id, dist2, min_range2);
	...
	switch(CastAction)
	{
		...
		case SingleTarget:
		{
			...
			if(spell_target == nullptr) {
				LogSpells("Mob::SpellFinished(): Spell [{}]: Targeted spell, but we have no target", spell_id);
				return(false);
			}
			if (isproc) {
			...
			&& IsClient()
			&& (HasProjectIllusion())){
				LogAA("Mob::SpellFinished(): Effect Project Illusion for [{}] on spell id: [{}] was ON", GetName(), spell_id);
				SetProjectIllusion(false);
			}
			else{
				LogAA("Mob::SpellFinished(): Effect Project Illusion for [{}] on spell id: [{}] was OFF", GetName(), spell_id);
			}
			break;
		}
	...
	if(slot != CastingSlot::Item && slot != CastingSlot::PotionBelt && mana_used > 0)
	{
		...
		if (GetMana() < mana_used)
			mana_used = GetMana();
		LogSpells("Mob::SpellFinished(): Spell [{}]: consuming [{}] mana", spell_id, mana_used);
	...
	if(IsClient() && !isproc)
	{
		...
		else if(spell_id == casting_spell_id && casting_spell_timer != 0xFFFFFFFF)
		{
			...
			LogSpells("Mob::SpellFinished(): Spell [{}]: Setting custom reuse timer [{}] to [{}]", spell_id, casting_spell_timer, casting_spell_timer_duration);
		}
			...
			LogSpells("Mob::SpellFinished(): Spell [{}]: Setting long reuse timer to [{}] s (orig [{}])", spell_id, recast, spells[spell_id].recast_time);
...
bool Mob::ApplyNextBardPulse(uint16 spell_id, Mob *spell_target, CastingSlot slot) {
	if(slot == CastingSlot::Item) {
		...
		LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse [{}]: Supposidly cast from an item. Killing song", spell_id);
		return(false);
	}
	...
	if(!DetermineSpellTargets(spell_id, spell_target, ae_center, CastAction, slot)) {
		LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse [{}]: was unable to determine target. Stopping", spell_id);
		return(false);
	}

	if(ae_center != nullptr && ae_center->IsBeacon()) {
		LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse [{}]: Unsupported Beacon NPC AE spell", spell_id);
		return(false);
	}
	...
	if(mana_used > 0) {
		if(mana_used > GetMana()) {
			...
			LogSpells("Mob::ApplyNextBardPulse(): Ran out of mana while singing song [{}]", spell_id);
			return(false);
		}

		LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse [{}]: consuming [{}] mana (have [{}])", spell_id, mana_used, GetMana());
		SetMana(GetMana() - mana_used);
	}
	...
	{
		LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse [{}]: cannot see target [{}]", spell_target->GetName());
		MessageString(Chat::Red, CANT_SEE_TARGET);
		return(false);
	}
	...
	if(spell_target != nullptr && spell_target != this) {
		...
		if(dist2 > range2) {
			//target is out of range.
			LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse [{}]: Spell target is out of range (squared: [{}] > [{}])", spell_id, dist2, range2);
			MessageString(Chat::Red, TARGET_OUT_OF_RANGE);
			return(false);
		}
	}
	...
	switch(CastAction)
	{
		...
		{
			if(spell_target == nullptr) {
				LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse [{}]: Targeted spell, but we have no target", spell_id);
				return(false);
			}
			LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse: Targeted. spell [{}], target [{}]", spell_id, spell_target->GetName());
		...
		case AETarget:
		{
			...
			if(ae_center == nullptr) {
				LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse [{}]: AE Targeted spell, but we have no target", spell_id);
				return(false);
			}
			...
			if(spell_target) {	// this must be an AETarget spell
				...
				spell_target->BardPulse(spell_id, this);
				LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse: spell [{}], AE target [{}]", spell_id, spell_target->GetName());
			} else {
				LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse: spell [{}], AE with no target", spell_id);
			}
			bool affect_caster = !IsNPC();	//NPC AE spells do not affect the NPC caster
		...
		{
			if(spell_target->IsGrouped()) {
				LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse: spell [{}], Group targeting group of [{}]", spell_id, spell_target->GetName());
				Group *target_group = entity_list.GetGroupByMob(spell_target);
				if(target_group)
					target_group->GroupBardPulse(this, spell_id);
			}
			else if(spell_target->IsRaidGrouped() && spell_target->IsClient()) {
				LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse: spell [{}], Raid group targeting raid group of [{}]", spell_id, spell_target->GetName());
				...
				LogSpells("Mob::ApplyNextBardPulse(): Bard Song Pulse: spell [{}], Group target without group. Affecting caster", spell_id);
				BardPulse(spell_id, this);
#ifdef GROUP_BUFF_PETS
				if (GetPet() && HasPetAffinity() && !GetPet()->IsCharmed())
					GetPet()->BardPulse(spell_id, this);
...
void Mob::BardPulse(uint16 spell_id, Mob *caster) {
	...
	for (buffs_i = 0; buffs_i < buff_count; buffs_i++) {
		...
		if(buffs[buffs_i].casterid != caster->GetID()) {
			LogSpells("Mob::BardPulse(): Bard Pulse for [{}]: found buff from caster [{}] and we are pulsing for [{}] are there two bards playing the same song???", spell_id, buffs[buffs_i].casterid, caster->GetID());
			return;
		}
		//extend the spell if it will expire before the next pulse
		if(buffs[buffs_i].ticsremaining <= 3) {
			buffs[buffs_i].ticsremaining += 3;
			LogSpells("Mob::BardPulse(): Bard Song Pulse [{}]: extending duration in slot [{}] to [{}] tics", spell_id, buffs_i, buffs[buffs_i].ticsremaining);
		}
...
//this may be the wrong packet...
	...
	LogSpells("Mob::BardPulse(): Bard Song Pulse [{}]: Buff not found, reapplying spell", spell_id);
...
int Mob::CalcBuffDuration(Mob *caster, Mob *target, uint16 spell_id, int32 caster_level_override, bool pvp)
{
	...
	// PVP duration
	int duration, durationformula = 0;
	...
	LogSpells("Mob::CalcBuffDuration(): PVP [{}], durationformula [{}], pvpbuffduration [{}]", pvp, durationformula, duration);

	int castlevel = caster->GetCasterLevel(spell_id);
	if(caster_level_override > 0)
		castlevel = caster_level_override;

	int res = CalcBuffDuration_formula(castlevel, durationformula, duration);
	...
	LogSpells("Mob::CalcBuffDuration(): Spell [{}]: Casting level [{}], formula [{}], base_duration [{}]: result [{}]", spell_id, castlevel, durationformula, duration, res);
...
int Mob::CheckStackConflict(uint16 spellid1, int caster_level1, uint16 spellid2, int caster_level2, Mob* caster1, Mob* caster2, int buffslot, bool pvp1, bool pvp2)
{
		...
		if (!IsStackableDot(spellid1) && !IsEffectInSpell(spellid1, SE_ManaBurn)) { // mana burn spells we need to use the stacking command blocks live actually checks those first, we should probably rework to that too
			if (caster_level1 > caster_level2) { // cur buff higher level than new
				if (IsEffectInSpell(spellid1, SE_ImprovedTaunt)) {
					LogSpells("Mob::CheckStackConflict(): SE_ImprovedTaunt level exception, overwriting");
					return 1;
				} else {
					LogSpells("Mob::CheckStackConflict(): Spells the same but existing is higher level, stopping");
					return -1;
				}
			} else {
				LogSpells("Mob::CheckStackConflict(): Spells the same but newer is higher or equal level, overwriting");
				return 1;
			}
		} else if (spellid1 == 2751) {
			LogSpells("Mob::CheckStackConflict(): Blocking spell because manaburn does not stack with itself");
			return -1;
		}
	}
	...
	*/
	if(IsBardSong(spellid1) != IsBardSong(spellid2))
	{
		...
		{
			LogSpells("Mob::CheckStackConflict(): [{}] and [{}] are beneficial, and one is a bard song, no action needs to be taken", sp1.name, sp2.name);
			return (0);
		}
	}
	...
	if (spellid1 != spellid2) {
		...
		LogSpells("Mob::CheckStackConflict(): We have a Mana Burn spell that is the same, they won't stack");
		return -1;
	}
		...
		for(i = 0; i < EFFECT_COUNT; i++)
		{
			...
			*/
			...
			if(effect2 == SE_StackingCommand_Overwrite)
			{
				...
				{
					sp1_value = CalcSpellEffectValue(spellid1, overwrite_slot, caster_level1, 10, nullptr, 0, 0, pvp1);

					LogSpells("Mob::CheckStackConflict(): [{}] ([{}]) overwrites existing spell if effect [{}] on slot [{}] is below [{}]. Old spell has value [{}] on that slot/effect. [{}]",
					...
					{
						LogSpells("Mob::CheckStackConflict(): Overwrite spell because sp1_value < overwrite_below_value");
						return 1;			// overwrite spell if its value is less
					}
				} else {
					LogSpells("Mob::CheckStackConflict(): [{}] ([{}]) overwrites existing spell if effect [{}] on slot [{}] is below [{}], but we do not have that effect on that slot. Ignored",
			...
			} else if (effect1 == SE_StackingCommand_Block)
			{
				...
				{
					sp2_value = CalcSpellEffectValue(spellid2, blocked_slot, caster_level2, 10, nullptr, 0, 0, pvp2);

					LogSpells("Mob::CheckStackConflict(): [{}] ([{}]) blocks effect [{}] on slot [{}] below [{}]. New spell has value [{}] on that slot/effect. [{}]",
					...
					{
						LogSpells("Mob::CheckStackConflict(): Blocking spell because sp2_Value < blocked_below_value");
						return -1;		//blocked
					}
				} else {
					LogSpells("Mob::CheckStackConflict(): [{}] ([{}]) blocks effect [{}] on slot [{}] below [{}], but we do not have that effect on that slot. Ignored",
						sp1.name, spellid1, blocked_effect, blocked_slot, blocked_below_value);
				}
			}
		}
	} else {
		LogSpells("Mob::CheckStackConflict(): [{}] ([{}]) and [{}] ([{}]) appear to be in the same line, skipping Stacking Overwrite/Blocking checks",
	...
	for (i = 0; i < EFFECT_COUNT; i++)
	{
		...
		*/
		if (IsNPC() && caster1 && caster2 && caster1 != caster2) {
			if (effect1 == SE_CurrentHP && sp1_detrimental && sp2_detrimental) {
				LogSpells("Mob::CheckStackConflict(): Both casters exist and are not the same, the effect is a detrimental dot, moving on");
				continue;
			}
		}

		if (effect1 == SE_CompleteHeal) { //SE_CompleteHeal never stacks or overwrites ever, always block.
			LogSpells("Mob::CheckStackConflict(): Blocking spell because complete heal never stacks or overwries");
			return (-1);
		}
		...
		*/
		if (effect1 == SE_CurrentHP && spellid1 != spellid2 && sp1_detrimental && sp2_detrimental) {
			LogSpells("Mob::CheckStackConflict(): The spells are not the same and it is a detrimental dot, passing");
			continue;
		}

		sp1_value = CalcSpellEffectValue(spellid1, i, caster_level1, 10, nullptr, 0, 0, pvp1);
		sp2_value = CalcSpellEffectValue(spellid2, i, caster_level2, 10, nullptr, 0, 0, pvp2);

		LogSpells("Mob::CheckStackConflict(): spellid1 [{}], sp1_value [{}], spellid2 [{}], sp2_value [{}]", spellid1, sp1_value, spellid2, sp2_value);

		// Movement Effects
		if (effect1 == SE_MovementSpeed && effect2 == SE_MovementSpeed)
		{
			// Caster1 has a positive effect and caster2 is trying to apply a negative effect
			// Negative effects should always overwrite positive effects
			// Example to apply Clinging Darkness to a target that has Spirit of Wolf
		...
		if (effect1 == SE_AttackSpeed || effect1 == SE_AttackSpeed2)
		{
			sp1_value -= 100;
			sp2_value -= 100;
		}
		...
		if(sp2_value < sp1_value) {
			LogSpells("Mob::CheckStackConflict(): Spell [{}] (value [{}]) is not as good as [{}] (value [{}]). Rejecting [{}]",
		...
		LogSpells("Mob::CheckStackConflict(): Spell [{}] (value [{}]) is not as good as [{}] (value [{}]). We will overwrite [{}] if there are no other conflicts",
	...
	if(will_overwrite) {
		if (values_equal && effect_match && !IsGroupSpell(spellid2) && IsGroupSpell(spellid1)) {
			LogSpells("Mob::CheckStackConflict(): [{}] ([{}]) appears to be the single target version of [{}] ([{}]), rejecting",
					sp2.name, spellid2, sp1.name, spellid1);
			return -1;
		}
		LogSpells("Mob::CheckStackConflict(): Stacking code decided that [{}] should overwrite [{}]", sp2.name, sp1.name);
		return(1);
	}

	LogSpells("Mob::CheckStackConflict(): Stacking code decided that [{}] is not affected by [{}]", sp2.name, sp1.name);
	return 0;
}
...
int Mob::AddBuff(Mob *caster, uint16 spell_id, int duration, int32 level_override, bool pvp)
{
	...
	if (duration == 0) {
		LogSpells("Mob::AddBuff(): Buff [{}] failed to add because its duration came back as 0", spell_id);
		return -2;	// no duration? this isn't a buff
	}

	LogSpells("Mob::AddBuff(): Trying to add buff [{}] cast by [{}] (cast level [{}]) with duration [{}]",
	...
	for (buffslot = 0; buffslot < buff_count; buffslot++) {
		...
		if (curbuf.spellid != SPELL_UNKNOWN) {
			...
			if (ret == 1) {	// set a flag to indicate that there will be overwriting
				LogSpells("Mob::AddBuff(): Adding buff [{}] will overwrite spell [{}] in slot [{}] with caster level [{}]",
						spell_id, curbuf.spellid, buffslot, curbuf.casterlevel);
		...
		if (IsDetrimentalSpell(spell_id))  {//Sucks to be you, bye bye one of your buffs
			for (buffslot = 0; buffslot < buff_count; buffslot++) {
				...
				if (IsBeneficialSpell(curbuf.spellid)) {
					LogSpells("Mob::AddBuff(): No slot for detrimental buff [{}], so we are overwriting a beneficial buff [{}] in slot [{}]",
			...
			if(emptyslot == -1) {
				LogSpells("Mob::AddBuff(): Unable to find a buff slot for detrimental buff [{}]", spell_id);
				return -1;
			}
		} else {
			LogSpells("Mob::AddBuff(): Unable to find a buff slot for beneficial buff [{}]", spell_id);
			return -1;
		}
	}
	...
	//if (IsClient() && caster && caster->IsClient() && IsDetrimentalSpell(spell_id))
		LogSpells("Mob::AddBuff(): pvp_duration_cap [{}]", spells[spell_id].pvp_duration_cap);
		if (buffs[emptyslot].ticsremaining > (1 + CalcBuffDuration_formula(caster_level, spells[spell_id].pvp_duration, spells[spell_id].pvp_duration_cap)))
			buffs[emptyslot].UpdateClient = true;
	}
	else {
		LogSpells("Mob::AddBuff(): buffduration [{}]", spells[spell_id].buffduration);
	...
	LogSpells("Mob::AddBuff(): Buff [{}] added to slot [{}] with caster level [{}]", spell_id, emptyslot, caster_level);
...
int Mob::CanBuffStack(uint16 spellid, uint8 caster_level, bool iFailIfOverwrite)
{
	...
	LogAI("Mob::CanBuffStack(): Checking if buff [{}] cast at level [{}] can stack on me.[{}]", spellid, caster_level, iFailIfOverwrite?" failing if we would overwrite something":"");
	...
	for (i=0; i < buff_count; i++)
	{
		...
		if(ret == 1) {
			...
			if(iFailIfOverwrite) {
				LogAI("Mob::CanBuffStack(): Buff [{}] would overwrite [{}] in slot [{}], reporting stack failure", spellid, curbuf.spellid, i);
				return(-1);
			}
			if(firstfree == -2)
				firstfree = i;
		...
		if(ret == -1) {
			...
			LogAI("Mob::CanBuffStack(): Buff [{}] would conflict with [{}] in slot [{}], reporting stack failure", spellid, curbuf.spellid, i);
			return -1;	// stop the spell, can't stack it
		}
	}

	LogAI("Mob::CanBuffStack(): Reporting that buff [{}] could successfully be placed into slot [{}]", spellid, firstfree);
			...
			bool isproc, int level_override)
{
	...
	if(!spelltar)
	{
		LogSpells("Mob::SpellOnTarget(): Unable to apply spell [{}] without a target", spell_id);
	...
	LogSpells("Mob::SpellOnTarget(): Casting spell [{}] on [{}] with effective caster level [{}]", spell_id, spelltar->GetName(), caster_level);
	...
	if (RuleB(Spells, EnableBlockedBuffs)) {
		...
		if (spelltar->IsBlockedBuff(spell_id)) {
			LogSpells("Mob::SpellOnTarget(): Spell [{}] not applied to [{}] as it is a Blocked Buff",
		...
		if (spelltar->IsPet() && spelltar->GetOwner() &&
			...
			LogSpells("Mob::SpellOnTarget(): Spell [{}] not applied to [{}] ([{}]'s pet) as it is a Pet Blocked Buff",
		...
		if (RuleB(Pets, UnTargetableSwarmPet)) {
			if (spelltar->IsNPC()) {
				if (!spelltar->CastToNPC()->GetSwarmOwner()) {
					LogSpells("Mob::SpellOnTarget(): Casting spell [{}] on [{}] aborted: they are untargetable", spell_id, spelltar->GetName());
					safe_delete(action_packet);
					return(false);
				}
			} else {
				...
				spelltar != this)
			{
				...
				)
				{
					if(spells[spell_id].targettype == ST_AEBard) {
						...
						LogSpells("Mob::SpellOnTarget(): Beneficial ae bard song [{}] can't take hold [{}] -> [{}], IBA? [{}]", spell_id, GetName(), spelltar->GetName(), IsBeneficialAllowed(spelltar));
					} else {
						LogSpells("Mob::SpellOnTarget(): Beneficial spell [{}] can't take hold [{}] -> [{}], IBA? [{}]", spell_id, GetName(), spelltar->GetName(), IsBeneficialAllowed(spelltar));
						MessageString(Chat::SpellFailure, SPELL_NO_HOLD);
					}
					safe_delete(action_packet);
					return false;
	...
	if(spelltar->IsImmuneToSpell(spell_id, this))
	{
		...
		LogSpells("Mob::SpellOnTarget(): Spell [{}] can't take hold due to immunity [{}] -> [{}]", spell_id, GetName(), spelltar->GetName());
		safe_delete(action_packet);
		return false;
	}
	...
	if(IsResistableSpell(spell_id))
	{
		...
		LogSpells("Mob::SpellOnTarget()->ResistSpell(): resisttype [{}], use_resist_adjust [{}], resist_adjust [{}], level_override [{}]", spells[spell_id].resisttype, use_resist_adjust, resist_adjust, level_override);
		...
		if(spell_effectiveness < 100)
		{
			...
			{
				LogSpells("Mob::SpellOnTarget(): Spell [{}] was completely resisted by [{}]", spell_id, spelltar->GetName());
	...
	if (spelltar->IsAIControlled() && IsDetrimentalSpell(spell_id) && !IsHarmonySpell(spell_id)) {
		...
		LogSpells("Mob::SpellOnTarget(): Spell [{}] cast on [{}] generated [{}] hate", spell_id,
	...
	{
		LogSpells("Mob::SpellOnTarget(): Spell [{}] failed: recipient did not meet the level restrictions", spell_id);
		if(!IsBardSong(spell_id))
			MessageString(Chat::SpellFailure, SPELL_TOO_POWERFUL);
		safe_delete(action_packet);
		return false;
	...
	LogSpells("Mob::SpellOnTarget(): pvp", pvp);
	...
	if(!spelltar->SpellEffect(this, spell_id, spell_effectiveness, level_override, pvp))
	{
		...
		LogSpells("Mob::SpellOnTarget(): Spell [{}] could not apply its effects [{}] -> [{}]\n", spell_id, GetName(), spelltar->GetName());
		if(casting_spell_aa_id)
			MessageString(Chat::SpellFailure, SPELL_NO_HOLD);
		safe_delete(action_packet);
		return false;
	...
	LogSpells("Mob::SpellOnTarget(): Cast of [{}] by [{}] on [{}] complete successfully", spell_id, GetName(), spelltar->GetName());
...
bool Mob::IsImmuneToSpell(uint16 spell_id, Mob *caster)
{
	...
	LogSpells("Mob::IsImmuneToSpell(): Checking to see if we are immune to spell [{}] cast by [{}]", spell_id, caster->GetName());
	...
	if(IsMezSpell(spell_id))
	{
		if(GetSpecialAbility(UNMEZABLE)) {
			LogSpells("Mob::IsImmuneToSpell(): We are immune to Mez spells");
		...
		{
			LogSpells("Mob::IsImmuneToSpell(): Our level ([{}]) is higher than the limit of this Mez spell ([{}])", GetLevel(), spells[spell_id].max[effect_index]);
	...
	{
		LogSpells("Mob::IsImmuneToSpell(): We are immune to Slow spells");
	...
	if(IsEffectInSpell(spell_id, SE_Fear))
	{
		...
		if(GetSpecialAbility(UNFEARABLE)) {
			LogSpells("Mob::IsImmuneToSpell(): We are immune to Fear spells");
		...
		{
			LogSpells("Mob::IsImmuneToSpell(): Level is [{}], cannot be feared by this spell", GetLevel());
		...
		else if (CheckAATimer(aaTimerWarcry))
		{
			...
			LogSpells("Mob::IsImmuneToSpell(): Clients has WarCry effect, immune to fear!");
			caster->MessageString(Chat::Red, IMMUNE_FEAR);	// need to verify message type, not in MQ2Cast for easy look up
			return true;
		}
	}
	...
	if(IsCharmSpell(spell_id))
	{
		...
		{
			LogSpells("Mob::IsImmuneToSpell(): We are immune to Charm spells");
		...
		if(this == caster)
		{
			LogSpells("Mob::IsImmuneToSpell(): You are immune to your own charms");
		...
		if(!caster->IsNPC())
		{
			...
			{
				LogSpells("Mob::IsImmuneToSpell(): Our level ([{}]) is higher than the limit of this Charm spell ([{}])", GetLevel(), spells[spell_id].max[effect_index]);
	...
	)
	{
		if(GetSpecialAbility(UNSNAREABLE)) {
			LogSpells("Mob::IsImmuneToSpell(): We are immune to Snare spells");
	...
	if(IsLifetapSpell(spell_id))
	{
		if(this == caster)
		{
			LogSpells("Mob::IsImmuneToSpell(): You cannot lifetap yourself");
	...
	if(IsSacrificeSpell(spell_id))
	{
		if(this == caster)
		{
			LogSpells("Mob::IsImmuneToSpell(): You cannot sacrifice yourself");
			caster->MessageString(Chat::SpellFailure, CANNOT_SAC_SELF);
			return true;
		}
	}

	LogSpells("Mob::IsImmuneToSpell(): No immunities to spell [{}] found", spell_id);
...
float Mob::ResistSpell(uint8 resist_type, uint16 spell_id, Mob *caster, bool use_resist_override, int resist_override, bool CharismaCheck, bool CharmTick, bool IsRoot, int level_override)
{
	...
	LogSpells("Mob::ResistSpell(): Called");
	...
	{
		LogSpells("Mob::ResistSpell(): We are immune to magic, so we fully resist the spell [{}]", spell_id);
		return(0);
	}
	...
	LogSpells("Mob::ResistSpell(): base resist_modifier: [{}]", resist_modifier);

	if (caster->GetSpecialAbility(CASTING_RESIST_DIFF))
	{
		resist_modifier += caster->GetSpecialAbilityParam(CASTING_RESIST_DIFF, 0);
		LogSpells("Mob::ResistSpell(): GetSpecialAbility [{}], resist_modifier: [{}]", caster->GetSpecialAbilityParam(CASTING_RESIST_DIFF, 0), resist_modifier);
	}

	int focus_resist = caster->GetFocusEffect(focusResistRate, spell_id);

	resist_modifier -= 2 * focus_resist;
	LogSpells("Mob::ResistSpell(): focusResistRate [{}], calc [{}], resist_modifier: [{}]", focus_resist, 2 * focus_resist, resist_modifier);


	int focus_incoming_resist = GetFocusEffect(focusFcResistIncoming, spell_id);

	resist_modifier -= focus_incoming_resist;
	LogSpells("Mob::ResistSpell(): focusFcResistIncoming [{}], resist_modifier: [{}]", focus_incoming_resist, resist_modifier);
	...
	if(IsFearSpell(spell_id))
	{
		...
		{
			LogSpells("Mob::ResistSpell(): Resisted spell in fear resistance, had [{}] chance to resist", fear_resist_bonuses);
			return 0;
		}
	}
	...
	if (!CharmTick){
		...
		{
			LogSpells("Mob::ResistSpell(): Resisted spell in sanctification, had [{}] chance to resist", resist_bonuses);
			return 0;
		}
	}
	...
	{
		LogSpells("Mob::ResistSpell(): Spell was unresistable");
		return 100;
	}
	...
	if (CharismaCheck)
	{
		LogSpells("Mob::ResistSpell(): CharismaCheck");
		/*
		...
		*/
		...
		if (IsFear && (spells[spell_id].targettype != ST_Undead)){
			...
			if (charisma >= 75){
				...
				LogSpells("Mob::ResistSpell(): charisma >= 75: calc [{}], resist_modifier [{}]", (charisma - 75) / RuleI(Spells, CharismaEffectiveness), resist_modifier);
			}
			else {
				resist_modifier += ((75 - charisma) / 10) * 6; //Increase Resist Chance
				LogSpells("Mob::ResistSpell(): charisma else: calc [{}], resist_modifier [{}]", ((75 - charisma) / 10) * 6, resist_modifier);
			}
		}
	}
	...
	if (IsClient() && caster->IsClient())
	{
		...
		{
			LogSpells("Mob::ResistSpell(): pvpresistcap positive [{}], max [{}]", resist_modifier, spells[spell_id].pvpresistcap);
		...
		{
			LogSpells("Mob::ResistSpell(): pvpresistcap negative [{}], max [{}]", resist_modifier, spells[spell_id].pvpresistcap);
			resist_modifier = spells[spell_id].pvpresistcap;
		}
	}
	...
	LogSpells("Mob::ResistSpell(): level_mod [{}], resist_modifier [{}], target_resist [{}], Sum resist_chance: [{}]", level_mod, resist_modifier, target_resist, resist_chance);
	...
	if(resist_chance > spells[spell_id].MaxResist && spells[spell_id].MaxResist != 0)
	{
		...
		LogSpells("Mob::ResistSpell(): Max resist_chance [{}]", resist_chance);
	}

	if(resist_chance < spells[spell_id].MinResist && spells[spell_id].MinResist != 0)
	{
		resist_chance = spells[spell_id].MinResist;
		LogSpells("Mob::ResistSpell(): Min resist_chance [{}]", resist_chance);
	}
	...
	LogSpells("Mob::ResistSpell(): resist_chance [{}], roll [{}]", resist_chance, roll);
...
void Client::MemSpell(uint16 spell_id, int slot, bool update_client)
{
	...
	LogSpells("Client::MemSpell(): Spell [{}] memorized into slot [{}]", spell_id, slot);
...
void Client::UnmemSpell(int slot, bool update_client)
{
	...
	LogSpells("Client::UnmemSpell(): Spell [{}] forgotten from slot [{}]", m_pp.mem_spells[slot], slot);
...
void Client::ScribeSpell(uint16 spell_id, int slot, bool update_client)
{
	...
	LogSpells("Client::ScribeSpell(): Spell [{}] scribed into spell book slot [{}]", spell_id, slot);
...
void Client::UnscribeSpell(int slot, bool update_client)
{
	...
	LogSpells("Client::UnscribeSpell(): Spell [{}] erased from spell book slot [{}]", m_pp.spell_book[slot], slot);
...
void Client::UntrainDisc(int slot, bool update_client)
{
	...
	LogSpells("Client::UntrainDisc(): Discipline [{}] untrained from slot [{}]", m_pp.disciplines.values[slot], slot);
...
bool Client::SpellGlobalCheck(uint16 spell_id, uint32 char_id) {
	...
	if (!results.Success()) {
		LogError(
			"Client::SpellGlobalCheck(): Spell ID [{}] query of spell_globals with Name: [{}] Value: [{}] failed",
			spell_id,
	...
	if (results.RowCount() != 1) {
		LogError(
			"Client::SpellGlobalCheck(): Char ID: [{}] does not have the Qglobal Name: [{}] for Spell ID [{}]",
			char_id,
			spell_global_name.c_str(),
			spell_id
		);
	...
	LogError(
		"Client::SpellGlobalCheck(): Char ID: [{}] SpellGlobals Name: [{}] Value: [{}] did not match QGlobal Value: [{}] for Spell ID [{}]",
		char_id,
		spell_global_name.c_str(),
		spell_global_value,
		global_value,
...
bool Client::SpellBucketCheck(uint16 spell_id, uint32 char_id) {
	...
	if (!results.Success()) {
		LogError(
			"Client::SpellBucketCheck(): Spell bucket [{}] for spell ID [{}] for char ID [{}] failed",
	...
	if (results.RowCount() != 1) {
		LogError(
			"Client::SpellBucketCheck(): Spell bucket [{}] does not exist for spell ID [{}] for char ID [{}]",
			spell_bucket_name.c_str(),
			spell_id,
			char_id
		);
  ...
  LogError("Client::SpellBucketCheck(): Spell bucket [{}] for spell ID [{}] for char ID [{}] did not match value [{}]", spell_bucket_name.c_str(), spell_id, char_id, spell_bucket_value);
    return false;
}
			...
			for (int j = 0; j < EFFECT_COUNT; j++) {
				...
				if (bOffensive) {
					if (spells[buffs[i].spellid].effectid[j] == type) {
						...
						int16 value;
						if(buffs[i].client && IsClient() && IsDetrimentalSpell(buffs[i].spellid))
							value = CalcSpellEffectValue_formula(spells[buffs[i].spellid].pvp_duration, spells[buffs[i].spellid].base[j], spells[buffs[i].spellid].max[j], buffs[i].casterlevel, buffs[i].spellid, spells[buffs[i].spellid].pvp_duration_cap);
						else
							value = CalcSpellEffectValue_formula(spells[buffs[i].spellid].buffdurationformula, spells[buffs[i].spellid].base[j], spells[buffs[i].spellid].max[j], buffs[i].casterlevel, buffs[i].spellid, spells[buffs[i].spellid].buffduration);
						
						LogSpells("Mob::FindType(): type [{}], value [{}], threshold [{}], client [{}]", type, value, threshold, buffs[i].client);
						if (value < threshold)
							return true;
					}
				} else {
	...
	if (bPerma) {
		for (i = 0; i < MAX_PROCS; i++) {
			if (PermaProcs[i].spellID == SPELL_UNKNOWN) {
				...
				LogSpells("Mob::AddProcToWeapon(): Added permanent proc spell [{}] with chance [{}] to slot [{}]", spell_id, iChance, i);

				return true;
			}
		}
		LogSpells("Mob::AddProcToWeapon(): Too many perma procs for [{}]", GetName());
	} else {
		...
		if (base_spell_id == POISON_PROC) {
			for (i = 0; i < MAX_PROCS; i++) {
				...
				if (SpellProcs[i].base_spellID == POISON_PROC) {
					...
					Log(Logs::Detail, Logs::Spells, "Mob::AddProcToWeapon(): Replaced poison-granted proc spell %d with chance %d to slot %d", spell_id, iChance, i);
					return true;
				}
			}
		}
		...
		for (i = 0; i < MAX_PROCS; i++) {
			if (SpellProcs[i].spellID == SPELL_UNKNOWN) {
				...
				SpellProcs[i].level_override = level_override;
				LogSpells("Mob::AddProcToWeapon(): Added [{}]-granted proc spell [{}] with chance [{}] to slot [{}]", (base_spell_id == POISON_PROC) ? "poison" : "spell", spell_id, iChance, i);
				return true;
			}
		}
		LogSpells("Mob::AddProcToWeapon(): Too many procs for [{}]", GetName());
	}
	return false;
}
...
bool Mob::RemoveProcFromWeapon(uint16 spell_id, bool bAll) {
	for (int i = 0; i < MAX_PROCS; i++) {
		if (bAll || SpellProcs[i].spellID == spell_id) {
			...
			LogSpells("Mob::RemoveProcFromWeapon(): Removed proc [{}] from slot [{}]", spell_id, i);
		}
	}
	return true;
}
...
bool Mob::AddDefensiveProc(uint16 spell_id, uint16 iChance, uint16 base_spell_id)
{
	...
	for (i = 0; i < MAX_PROCS; i++) {
		if (DefensiveProcs[i].spellID == SPELL_UNKNOWN) {
			...
			LogSpells("Mob::AddDefensiveProc(): Added spell-granted defensive proc spell [{}] with chance [{}] to slot [{}]", spell_id, iChance, i);
			return true;
		}
	}
...
bool Mob::RemoveDefensiveProc(uint16 spell_id, bool bAll)
{
	for (int i = 0; i < MAX_PROCS; i++) {
		if (bAll || DefensiveProcs[i].spellID == spell_id) {
			...
			LogSpells("Mob::RemoveDefensiveProc(): Removed defensive proc [{}] from slot [{}]", spell_id, i);
		}
	}
	return true;
}
...
bool Mob::AddRangedProc(uint16 spell_id, uint16 iChance, uint16 base_spell_id)
{
	...
	for (i = 0; i < MAX_PROCS; i++) {
		if (RangedProcs[i].spellID == SPELL_UNKNOWN) {
			...
			LogSpells("Mob::AddRangedProc(): Added spell-granted ranged proc spell [{}] with chance [{}] to slot [{}]", spell_id, iChance, i);
			return true;
		}
	}
...
bool Mob::RemoveRangedProc(uint16 spell_id, bool bAll)
{
	for (int i = 0; i < MAX_PROCS; i++) {
		if (bAll || RangedProcs[i].spellID == spell_id) {
			...
			LogSpells("Mob::RemoveRangedProc(): Removed ranged proc [{}] from slot [{}]", spell_id, i);
		}
	}
	return true;
}
...
int Mob::GetCasterLevel(uint16 spell_id) {
	...
	LogSpells("Mob::GetCasterLevel(): Determined effective casting level [{}]+[{}]+[{}]=[{}]", GetLevel(), spellbonuses.effective_casting_level, itembonuses.effective_casting_level, level);
...
void Client::SetLinkedSpellReuseTimer(uint32 timer_id, uint32 duration)
{
	if (timer_id > 19)
		return;
	LogSpells("Client::SetLinkedSpellReuseTimer(): Setting Linked Spell Reuse [{}] for [{}]", timer_id, duration);
