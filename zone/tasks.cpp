...
bool TaskManager::LoadTaskSets() {
	const char *TaskSetQuery = "SELECT `id`, `taskid` from `tasksets` WHERE `id` > 0 AND `id` < %i AND `taskid` >= 0 AND `taskid` < %i ORDER BY `id`, `taskid` ASC";
...
bool TaskManager::LoadTasks(int SingleTask) {
	const char *AllTaskQuery = "SELECT `id`, `duration`, `title`, `description`, `reward`, `rewardid`, `cashreward`, `xpreward`, `rewardmethod`, `startzone`, `minlevel`, `maxlevel`, `repeatable` from `tasks` WHERE `id` < %i";
	const char *SingleTaskQuery = "SELECT `id`, `duration`, `title`, `description`, `reward`, `rewardid`,`cashreward`, `xpreward`, `rewardmethod`, `startzone`, `minlevel`, `maxlevel`, `repeatable` from `tasks` WHERE `id` = %i";
	const char *AllActivityQuery = "SELECT `taskid`, `step`, `activityid`, `activitytype`, `text1`, `text2`,`text3`, `goalid`, `goalmethod`, `goalcount`, `delivertonpc`, `zoneid`, `optional` from `activities` WHERE `taskid` < %i AND `activityid` < %i ORDER BY taskid, activityid ASC";
	const char *SingleTaskActivityQuery = "SELECT `taskid`, `step`, `activityid`, `activitytype`, `text1`, `text2`, `text3`, `goalid`, `goalmethod`, `goalcount`, `delivertonpc`, `zoneid`, `optional` from `activities` WHERE `taskid` = %i AND `activityid` < %i ORDER BY taskid, activityid ASC";
	const char *ERR_TASK_OOR = "[TASKS]Task ID %i out of range while loading tasks from database";
	const char *ERR_TASK_OR_ACTIVITY_OOR = "[TASKS]Task or Activity ID (%i, %i) out of range while loading activities from database";
	...
	if(database.RunQuery(query,QueryLength, errbuf, &result)) {
		...
		while((row = mysql_fetch_row(result))) {
			...
			_log(TASKS__GLOBALLOAD, "Activity Slot %2i: ID %i for Task %5i. Type: %3i, GoalID: %8i, GoalMethod: %i, GoalCount: %3i, ZoneID:%3i", Tasks[TaskID]->ActivityCount, ActivityID, TaskID,Tasks[TaskID]->Activity[Tasks[TaskID]->ActivityCount].Type,Tasks[TaskID]->Activity[Tasks[TaskID]->ActivityCount].GoalID,Tasks[TaskID]->Activity[Tasks[TaskID]->ActivityCount].GoalMethod,Tasks[TaskID]->Activity[Tasks[TaskID]->ActivityCount].GoalCount,Tasks[TaskID]->Activity[Tasks[TaskID]->ActivityCount].ZoneID);
...
bool TaskManager::SaveClientState(Client *c, ClientTaskState *state) {
	const char *TaskQuery="REPLACE INTO character_tasks (charid, taskid, slot, acceptedtime) VALUES (%i, %i, %i, %i)";
	const char *ActivityQuery="REPLACE INTO character_activities (charid, taskid, activityid, donecount, completed) VALUES ";
	const char *CompletedTaskQuery="REPLACE INTO completed_tasks (charid, completedtime, taskid, activityid) VALUES (%i, %i, %i, %i)";
	...
	if(state->ActiveTaskCount > 0) {
		for(int Task = 0; Task < MAXACTIVETASKS; Task++) {
			...
			if(state->ActiveTasks[Task].Updated) {
				...
				if(!database.RunQuery(query,MakeAnyLenString(&query, TaskQuery,CharacterID,TaskID,Task,state->ActiveTasks[Task].AcceptedTime), errbuf))
					LogFile->write(EQEMuLog::Error, ERR_MYSQLERROR, errbuf);
				else
			...
			for(int Activity=0; Activity<Tasks[TaskID]->ActivityCount; Activity++) {
				if(state->ActiveTasks[Task].Activity[Activity].Updated) {
					_log(TASKS__CLIENTSAVE, "TaskManager::SaveClientSate for character ID %d, Updating Activity %i, %i",CharacterID, Task, Activity);

					if(UpdatedActivityCount==0)
						MakeAnyLenString(&buf, "(%i, %i, %i, %i, %i)", CharacterID, TaskID,Activity,state->ActiveTasks[Task].Activity[Activity].DoneCount,state->ActiveTasks[Task].Activity[Activity].State ==ActivityCompleted);
					else
						MakeAnyLenString(&buf, ", (%i, %i, %i, %i, %i)", CharacterID, TaskID, Activity, state->ActiveTasks[Task].Activity[Activity].DoneCount, state->ActiveTasks[Task].Activity[Activity].State == ActivityCompleted);
			...
			if(UpdatedActivityCount > 0) {
				...
				if(!database.RunQuery(query,MakeAnyLenString(&query, UpdateActivityQuery.c_str()), errbuf))
					LogFile->write(EQEMuLog::Error, ERR_MYSQLERROR, errbuf);
				else {
					state->ActiveTasks[Task].Updated = false;
					for(int Activity = 0; Activity < Tasks[TaskID]->ActivityCount; Activity++)
	...
	if(RuleB(TaskSystem, RecordCompletedTasks) && (state->CompletedTasks.size() > (unsigned int)state->LastCompletedTaskLoaded)) {
		for(unsigned int i=state->LastCompletedTaskLoaded; i<state->CompletedTasks.size(); i++) {
			...
			if(!database.RunQuery(query,MakeAnyLenString(&query, CompletedTaskQuery, CharacterID, state->CompletedTasks[i].CompletedTime, TaskID, -1), errbuf)) {
				LogFile->write(EQEMuLog::Error, ERR_MYSQLERROR, errbuf);
				continue;
			}
			safe_delete_array(query);
			...
			for(int j = 0; j < Tasks[TaskID]->ActivityCount; j++) {
				if(Tasks[TaskID]->Activity[j].Optional && state->CompletedTasks[i].ActivityDone[j]) {
					if(!database.RunQuery(query,MakeAnyLenString(&query, CompletedTaskQuery, CharacterID, state->CompletedTasks[i].CompletedTime, TaskID, j), errbuf))
						LogFile->write(EQEMuLog::Error, ERR_MYSQLERROR, errbuf);
					safe_delete_array(query);
				}
			}
...
bool TaskManager::LoadClientState(Client *c, ClientTaskState *state) {
	const char *TaskQuery = "SELECT `taskid`, `slot`, `acceptedtime` from `character_tasks` WHERE `charid` = %i ORDER BY acceptedtime";
	...
	const char *ActivityQuery = "SELECT `taskid`, `activityid`, `donecount`, `completed` from `character_activities` WHERE `charid` = %i ORDER BY `taskid` ASC, `activityid` ASC";
	...
	if(database.RunQuery(query,MakeAnyLenString(&query, ActivityQuery, CharacterID), errbuf, &result)) {
		while((row = mysql_fetch_row(result))) {
			int TaskID = atoi(row[0]);
			if((TaskID < 0) || TaskID >= MAXTASKS) {
	...
	const char *CompletedTaskQuery = "SELECT `taskid`, `activityid`, `completedtime` from `completed_tasks` WHERE `charid` = %i ORDER BY completedtime, taskid, activityid";
	...
	if(RuleB(TaskSystem, RecordCompletedTasks)) {
		if(database.RunQuery(query,MakeAnyLenString(&query, CompletedTaskQuery, CharacterID), errbuf, &result)) {
	...
	const char *EnabledTaskQuery = "SELECT `taskid` FROM character_enabledtasks WHERE `charid` = %i AND `taskid` >0 AND `taskid` < %i ORDER BY `taskid` ASC";
	const char *ERR_MYSQLERROR4 = "[TASKS]Error in TaskManager::LoadClientState load enabled tasks: %s";
	if(database.RunQuery(query,MakeAnyLenString(&query, EnabledTaskQuery, CharacterID, MAXTASKS), errbuf, &result)) {
		while((row = mysql_fetch_row(result))) {
			int TaskID = atoi(row[0]);
	...
	const char *ERR_INCONSISTENT = "[TASKS]Fatal error in character %i task state. Activity %i for Task %i either missing from client state or from task.";
	for(int i = 0; i < MAXACTIVETASKS; i++) {
		int TaskID = state->ActiveTasks[i].TaskID;
		if(TaskID == TASKSLOTEMPTY)
			continue;
		if(!Tasks[TaskID]) {
			c->Message(13, "Active Task Slot %i, references a task (%i), that does not exist. Removing from memory. Contact a GM to resolve this.",i, TaskID);
		...
		for(int j = 0; j < Tasks[TaskID]->ActivityCount; j++) {
			if(state->ActiveTasks[i].Activity[j].ActivityID != j) {
				c->Message(13, "Active Task %i, %s. Activity count does not match expected value. Removing from memory. Contact a GM to resolve this.", TaskID, Tasks[TaskID]->Title);
...
int TaskManager::FirstTaskInSet(int TaskSetID) {
	if(TaskSetID <= 0 || TaskSetID >= MAXTASKSETS || TaskSets[TaskSetID].size() == 0)
		return 0;
...
bool TaskManager::AppropriateLevel(int TaskID, int PlayerLevel) {
	if(Tasks[TaskID] == nullptr || (Tasks[TaskID]->MinLevel && PlayerLevel < Tasks[TaskID]->MinLevel) || (Tasks[TaskID]->MaxLevel && PlayerLevel > Tasks[TaskID]->MaxLevel))
		return false;
	return true;
...
void TaskManager::TaskSetSelector(Client *c, ClientTaskState *state, Mob *mob, int TaskSetID) {
	...
	_log(TASKS__UPDATE, "TaskSetSelector called for taskset %i. EnableTaskSize is %i", TaskSetID, state->EnabledTasks.size());
	if((TaskSetID<=0) || (TaskSetID>=MAXTASKSETS))
		return;
	...
	while((EnabledTaskIndex < state->EnabledTasks.size()) && (TaskSetIndex < TaskSets[TaskSetID].size()) && (TaskListIndex < MAXCHOOSERENTRIES)) {
		_log(TASKS__UPDATE, "Comparing EnabledTasks[%i] (%i) with TaskSets[%i][%i] (%i)", EnabledTaskIndex, state->EnabledTasks[EnabledTaskIndex], TaskSetID, TaskSetIndex, TaskSets[TaskSetID][TaskSetIndex]);

		if((TaskSets[TaskSetID][TaskSetIndex] > 0) && (state->EnabledTasks[EnabledTaskIndex] == TaskSets[TaskSetID][TaskSetIndex])) {
			if(AppropriateLevel(TaskSets[TaskSetID][TaskSetIndex], PlayerLevel) && !state->IsTaskActive(TaskSets[TaskSetID][TaskSetIndex]) && (IsTaskRepeatable(TaskSets[TaskSetID][TaskSetIndex]) || !state->IsTaskCompleted(TaskSets[TaskSetID][TaskSetIndex]))) {
...
void TaskManager::SendTaskSelector(Client *c, Mob *mob, int TaskCount, int *TaskList) {
	...
	int ValidTasks = 0;
	for(int i = 0; i < TaskCount; i++) {
		if(!AppropriateLevel(TaskList[i], PlayerLevel) || c->IsTaskActive(TaskList[i]) || (!IsTaskRepeatable(TaskList[i]) && c->IsTaskCompleted(TaskList[i])))
			continue;
		ValidTasks++;
...
void TaskManager::SendTaskSelectorNew(Client *c, Mob *mob, int TaskCount, int *TaskList) {
	...
	for(int i = 0; i < TaskCount; i++) {
		...
		PacketLength += 21;
		PacketLength += (strlen(Tasks[TaskList[i]]->Title) + 1 + strlen(Tasks[TaskList[i]]->Description) + 1);
		sprintf(StartZone, "%i", Tasks[TaskList[i]]->StartZone);
		PacketLength += (11 + 11 + 11 + 3 + 3 + (strlen(StartZone) * 2) + 2);
		PacketLength += 28;
	}
	...
	outapp->WriteUInt32(ValidTasks);
	outapp->WriteUInt32(2);
	outapp->WriteUInt32(mob->GetID());

	for(int i = 0; i < TaskCount;i++) {
		if(!AppropriateLevel(TaskList[i], PlayerLevel) || c->IsTaskActive(TaskList[i]) || (!IsTaskRepeatable(TaskList[i]) && c->IsTaskCompleted(TaskList[i])))
			continue;
			
		outapp->WriteUInt32(TaskList[i]);
...
int TaskManager::GetActivityCount(int TaskID) {
	if(TaskID > 0 && TaskID < MAXTASKS) {
		if(Tasks[TaskID])
			return Tasks[TaskID]->ActivityCount;
	}
...
int ClientTaskState::GetActiveTaskID(int index) {
	if(index < 0 || index >= MAXACTIVETASKS)
		return 0;
...
bool ClientTaskState::UnlockActivities(int CharID, int TaskIndex) {
	...
	_log(TASKS__UPDATE, "CharID: %i Task: %i Sequence mode is %i", CharID, ActiveTasks[TaskIndex].TaskID, Task->SequenceMode);
	if(Task->SequenceMode == ActivitiesSequential) {
		...
		for(int i = 0; i<Task->ActivityCount; i++) {
			if((ActiveTasks[TaskIndex].Activity[i].State == ActivityActive) && (!Task->Activity[i].Optional)) {
	...
	if(ActiveTasks[TaskIndex].CurrentStep == -1) {
		for(int i = 0; i < Task->ActivityCount; i++) {
			if(Task->Activity[i].StepNumber == 0 && ActiveTasks[TaskIndex].Activity[i].State == ActivityHidden)
				ActiveTasks[TaskIndex].Activity[i].State = ActivityActive;
		}
	...
	for(int Step=ActiveTasks[TaskIndex].CurrentStep; Step<=Task->LastStep; Step++) {
		for(int Activity=0; Activity<Task->ActivityCount; Activity++) {
			if(Task->Activity[Activity].StepNumber == (int)ActiveTasks[TaskIndex].CurrentStep) {
				if(ActiveTasks[TaskIndex].Activity[Activity].State != ActivityCompleted && !Task->Activity[Activity].Optional) {
	...
	for(int Activity = 0; Activity < Task->ActivityCount; Activity++) {
		if((Task->Activity[Activity].StepNumber == (int)ActiveTasks[TaskIndex].CurrentStep) && (ActiveTasks[TaskIndex].Activity[Activity].State == ActivityHidden)) {
...
bool ClientTaskState::UpdateTasksByNPC(Client *c, int ActivityType, int NPCTypeID) {
	...
	if(!taskmanager || ActiveTaskCount == 0)
		return false;
	...
	for(int i = 0; i < MAXACTIVETASKS; i++) {
		...
		for(int j = 0; j < Task->ActivityCount; j++) {
			if(ActiveTasks[i].Activity[j].State != ActivityActive || Task->Activity[j].Type != ActivityType)
				continue;
			if((Task->Activity[j].ZoneID >0) && (Task->Activity[j].ZoneID != (int)zone->GetZoneID())) {
				_log(TASKS__UPDATE, "Char: %s Task: %i, Activity %i, Activity type %i for NPC %i failed zone check", c->GetName(), ActiveTasks[i].TaskID, j, ActivityType, NPCTypeID);
				continue;
			}
...
int ClientTaskState::ActiveSpeakTask(int NPCTypeID) {
	...
	for(int i = 0; i < MAXACTIVETASKS; i++) {
		...
		for(int j = 0; j < Task->ActivityCount; j++) {
			if(ActiveTasks[i].Activity[j].State != ActivityActive || Task->Activity[j].Type != ActivitySpeakWith || (Task->Activity[j].ZoneID > 0 && Task->Activity[j].ZoneID != (int)zone->GetZoneID()))
				continue;
			if((Task->Activity[j].GoalMethod == METHODQUEST) && (Task->Activity[j].GoalID == NPCTypeID))
				return ActiveTasks[i].TaskID;
		}
	}
	return 0;
}
...
int ClientTaskState::ActiveSpeakActivity(int NPCTypeID, int TaskID) {
	if(!taskmanager || ActiveTaskCount == 0 || TaskID <= 0 || TaskID >= MAXTASKS) return -1;
...
void ClientTaskState::UpdateTasksForItem(Client *c, ActivityType Type, int ItemID, int Count) {
	...
	for(int i = 0; i < MAXACTIVETASKS; i++) {
		...
		for(int j = 0; j < Task->ActivityCount; j++) {
			if(ActiveTasks[i].Activity[j].State != ActivityActive || Task->Activity[j].Type != (int)Type) continue;
			if((Task->Activity[j].ZoneID >0) && (Task->Activity[j].ZoneID != (int)zone->GetZoneID())) {
				_log(TASKS__UPDATE, "Char: %s Activity type %i for Item %i failed zone check", c->GetName(), Type, ItemID);
				continue;
			}
			switch(Task->Activity[j].GoalMethod) {
				case METHODSINGLEID:
					if(Task->Activity[j].GoalID != ItemID)
						continue;
					break;

				case METHODLIST:
					if(!taskmanager->GoalListManager.IsInList(Task->Activity[j].GoalID, ItemID))
						continue;
					break;
...
void ClientTaskState::UpdateTasksOnExplore(Client *c, int ExploreID) {
	...
	for(int i = 0; i < MAXACTIVETASKS; i++) {
		...
		for(int j=0; j<Task->ActivityCount; j++) {
			if(ActiveTasks[i].Activity[j].State != ActivityActive || Task->Activity[j].Type != ActivityExplore)
				continue;
			if((Task->Activity[j].ZoneID >0) && (Task->Activity[j].ZoneID != (int)zone->GetZoneID())) {
				_log(TASKS__UPDATE, "Char: %s Explore exploreid %i failed zone check", c->GetName(), ExploreID);
				continue;
			}
			switch(Task->Activity[j].GoalMethod) {
				case METHODSINGLEID:
			...
			IncrementDoneCount(c, Task, i, j, Task->Activity[j].GoalCount - ActiveTasks[i].Activity[j].DoneCount);
		}
	}
	return;
}
...
bool ClientTaskState::UpdateTasksOnDeliver(Client *c, uint32 *Items, int Cash, int NPCTypeID) {
	...
	for(int i = 0; i < MAXACTIVETASKS; i++) {
		...
		for(int j = 0; j < Task->ActivityCount; j++) {
			if(ActiveTasks[i].Activity[j].State != ActivityActive || (Task->Activity[j].Type != ActivityDeliver && Task->Activity[j].Type != ActivityGiveCash))
				continue;
			if((Task->Activity[j].ZoneID >0) && (Task->Activity[j].ZoneID != (int)zone->GetZoneID())) {
				_log(TASKS__UPDATE, "Char: %s Deliver activity failed zone check (current zone %i, need zone %i", c->GetName(), zone->GetZoneID(), Task->Activity[j].ZoneID);
				continue;
			}
			...
			if((Task->Activity[j].Type == ActivityGiveCash) && Cash) {
				...
				for(int k = 0; k < 4; k++) {
					...
					switch(Task->Activity[j].GoalMethod) {
						case METHODSINGLEID:
							if(Task->Activity[j].GoalID != (int)Items[k])
								continue;
							break;

						case METHODLIST:
							if(!taskmanager->GoalListManager.IsInList(Task->Activity[j].GoalID, Items[k]))
								continue;
							break;
...
void ClientTaskState::UpdateTasksOnTouch(Client *c, int ZoneID) {
	...
	for(int i = 0; i < MAXACTIVETASKS; i++) {
		...
		for(int j=0; j<Task->ActivityCount; j++) {
			if(ActiveTasks[i].Activity[j].State != ActivityActive || Task->Activity[j].Type != ActivityTouch || Task->Activity[j].GoalMethod != METHODSINGLEID) continue;
			if(Task->Activity[j].ZoneID != ZoneID) {
				_log(TASKS__UPDATE, "Char: %s Touch activity failed zone check", c->GetName());
				continue;
			}
...
void ClientTaskState::IncrementDoneCount(Client *c, TaskInformation* Task, int TaskIndex, int ActivityID, int Count) {
	...
	if(ActiveTasks[TaskIndex].Activity[ActivityID].DoneCount >= Task->Activity[ActivityID].GoalCount) {
		_log(TASKS__UPDATE, "Done (%i) = Goal (%i) for Activity %i", ActiveTasks[TaskIndex].Activity[ActivityID].DoneCount, Task->Activity[ActivityID].GoalCount, ActivityID);
		...
		taskmanager->SendTaskActivityLong(c, ActiveTasks[TaskIndex].TaskID, ActivityID, TaskIndex, Task->Activity[ActivityID].Optional);
}
...
bool ClientTaskState::IsTaskActivityActive(int TaskID, int ActivityID) {
	...
	if(ActivityID < 0 || ActiveTaskCount == 0) return false;
	...
	if(Task == nullptr || ActivityID >= Task->ActivityCount)
		return false;

	_log(TASKS__UPDATE, "ClientTaskState IsTaskActivityActive(%i, %i). State is %i ", TaskID, ActivityID, ActiveTasks[ActiveTaskIndex].Activity[ActivityID].State);
...
void ClientTaskState::UpdateTaskActivity(Client *c, int TaskID, int ActivityID, int Count) {
	...
	if(ActivityID < 0 || ActiveTaskCount == 0)
		return;
	...
	if(Task == nullptr || ActivityID >= Task->ActivityCount || ActiveTasks[ActiveTaskIndex].Activity[ActivityID].State != ActivityActive)
		return;
...
void ClientTaskState::ResetTaskActivity(Client *c, int TaskID, int ActivityID) {
	...
	if(Task == nullptr || ActivityID >= Task->ActivityCount || ActiveTasks[ActiveTaskIndex].Activity[ActivityID].State != ActivityActive) return;
	...
	taskmanager->SendTaskActivityLong(c, ActiveTasks[ActiveTaskIndex].TaskID, ActivityID, ActiveTaskIndex, Task->Activity[ActivityID].Optional);
}
...
void ClientTaskState::ShowClientTasks(Client *c) {
	...
	for(int i = 0; i < MAXACTIVETASKS; i++) {
		...
		for(int j=0; j<taskmanager->GetActivityCount(ActiveTasks[i].TaskID); j++)
			c->Message(0, "  Activity: %2d, DoneCount: %2d, Status: %d (0=Hidden, 1=Active, 2=Complete)", ActiveTasks[i].Activity[j].ActivityID, ActiveTasks[i].Activity[j].DoneCount, ActiveTasks[i].Activity[j].State);
	}
}
...
int ClientTaskState::TaskTimeLeft(int TaskID) {
	...
	for(int i = 0; i < MAXACTIVETASKS; i++) {
		...
		if(Task == nullptr || !Task->Duration) return -1;
...
bool ClientTaskState::TaskOutOfTime(int Index) {
	if(Index < 0 || Index >= MAXACTIVETASKS || ActiveTasks[Index].TaskID <= 0 || ActiveTasks[Index].TaskID >= MAXTASKS)
		return false;
...
void ClientTaskState::SendTaskHistory(Client *c, int TaskIndex) {
	...
	if(AdjustedTaskIndex < 0 || AdjustedTaskIndex >= (int)CompletedTasks.size())
		return;
	...
	for(int i = 0; i<Task->ActivityCount; i++) {
		if(CompletedTasks[AdjustedTaskIndex].ActivityDone[i]) {
			...
			PacketLength = (PacketLength + sizeof(TaskHistoryReplyData1_Struct) + strlen(Task->Activity[i].Text1) + 1 + strlen(Task->Activity[i].Text2) + 1 + sizeof(TaskHistoryReplyData2_Struct) + strlen(Task->Activity[i].Text3) + 1);
		}
	}
...
void TaskManager::SendCompletedTasksToClient(Client *c, ClientTaskState *State) {
	...
	_log(TASKS__UPDATE, "Completed Task Count: %i, First Task to send is %i, Last is %i", State->CompletedTasks.size(), FirstTaskToSend, LastTaskToSend);
...
void TaskManager::SendTaskActivityLong(Client *c, int TaskID, int ActivityID, int ClientTaskIndex, bool Optional, bool TaskComplete) {
	...
	PacketLength = (PacketLength + strlen(Tasks[TaskID]->Activity[ActivityID].Text1) + 1 + strlen(Tasks[TaskID]->Activity[ActivityID].Text2) + 1 + strlen(Tasks[TaskID]->Activity[ActivityID].Text3) + 1);
...
void TaskManager::SendTaskActivityNew(Client *c, int TaskID, int ActivityID, int ClientTaskIndex, bool Optional, bool TaskComplete) {
	...
	PacketLength += (strlen(Tasks[TaskID]->Activity[ActivityID].Text1) + 1 + strlen(Tasks[TaskID]->Activity[ActivityID].Text2) + 1 + strlen(Tasks[TaskID]->Activity[ActivityID].Text3) + 1 + ((strlen(itoa(Tasks[TaskID]->Activity[ActivityID].ZoneID)) + 1) * 2) + 3 + String2Len);
	EQApplicationPacket* outapp = new EQApplicationPacket(OP_TaskActivity, PacketLength);
	outapp->WriteUInt32(ClientTaskIndex);
	outapp->WriteUInt32(2);
	outapp->WriteUInt32(TaskID);
	...
	outapp->WriteUInt32((strlen(Tasks[TaskID]->Activity[ActivityID].Text2) + 1));
	...
	if(Tasks[TaskID]->Activity[ActivityID].Type != ActivityGiveCash)
		outapp->WriteUInt32(c->GetTaskActivityDoneCount(ClientTaskIndex, ActivityID));
	else
		...
		for(int Activity = 0; Activity < GetActivityCount(TaskID); Activity++) {
			if(c->GetTaskActivityState(TaskIndex, Activity) != ActivityHidden) {
				...
				if(Activity==GetActivityCount(TaskID)-1)
					SendTaskActivityLong(c, TaskID, Activity, TaskIndex, Tasks[TaskID]->Activity[Activity].Optional, TaskComplete);
				else
					SendTaskActivityLong(c, TaskID, Activity, TaskIndex, Tasks[TaskID]->Activity[Activity].Optional, 0);
			}
			else {
...
void TaskManager::SendSingleActiveTaskToClient(Client *c, int TaskIndex, bool TaskComplete, bool BringUpTaskJournal) {
	if(TaskIndex < 0 || TaskIndex >= MAXACTIVETASKS)
		return;
...
void TaskManager::SendActiveTaskDescription(Client *c, int TaskID, int SequenceNumber, int StartTime, int Duration, bool BringUpTaskJournal) {
	...
	int PacketLength = (sizeof(TaskDescriptionHeader_Struct) + strlen(Tasks[TaskID]->Title) + 1 + sizeof(TaskDescriptionData1_Struct) + strlen(Tasks[TaskID]->Description) + 1 + sizeof(TaskDescriptionData2_Struct) + 1 + sizeof(TaskDescriptionTrailer_Struct));
...
void ClientTaskState::RemoveTask(Client *c, int SequenceNumber) {
	...
	if(!database.RunQuery(query,MakeAnyLenString(&query, ActivityQuery, CharacterID, ActiveTasks[SequenceNumber].TaskID), errbuf)) {
	...
	safe_delete_array(query);
	if(!database.RunQuery(query,MakeAnyLenString(&query, TaskQuery, CharacterID, ActiveTasks[SequenceNumber].TaskID), errbuf))
		LogFile->write(EQEMuLog::Error, "[TASKS]Error in CientTaskState::CancelTask %s", errbuf);
...
void ClientTaskState::AcceptNewTask(Client *c, int TaskID, int NPCID) {
	...
	int FreeSlot = -1;
	for(int i = 0; i < MAXACTIVETASKS; i++) {
		_log(TASKS__UPDATE, "ClientTaskState Looking for free slot in slot %i, found TaskID of %i ", i, ActiveTasks[i].TaskID);
...
bool TaskGoalListManager::LoadLists() {
	const char *CountQuery = "SELECT `listid`, COUNT(`entry`) FROM `goallists` GROUP by `listid` ORDER BY `listid`";
	const char *ListQuery = "SELECT `entry` from `goallists` WHERE `listid`=%i ORDER BY `entry` ASC LIMIT %i";
...
int TaskGoalListManager::GetFirstEntry(int ListID) {
	...
	if(ListIndex < 0 || ListIndex >= NumberOfLists || TaskGoalLists[ListIndex].Size == 0)
		return -1;
...
bool TaskGoalListManager::IsInList(int ListID, int Entry) {
	...
	int ListIndex = GetListByID(ListID);
	if(ListIndex < 0 || ListIndex >= NumberOfLists || Entry < TaskGoalLists[ListIndex].Min || Entry > TaskGoalLists[ListIndex].Max)
		return false;
...
bool TaskProximityManager::LoadProximities(int ZoneID) {
	const char *ProximityQuery = "SELECT `exploreid`, `minx`, `maxx`, `miny`, `maxy`, `minz`, `maxz` from `proximities` WHERE `zoneid`=%i ORDER BY `zoneid` ASC";
...
int TaskProximityManager::CheckProximities(float X, float Y, float Z) {
	for(unsigned int i = 0; i<TaskProximities.size(); i++) {
		...
		_log(TASKS__PROXIMITY, "Checking %8.3f, %8.3f, %8.3f against %8.3f, %8.3f, %8.3f, %8.3f, %8.3f, %8.3f", X, Y, Z, P->MinX, P->MaxX, P->MinY, P->MaxY, P->MinZ, P->MaxZ);
