...
public:
	...
	static void GMSayHookCallBackProcess(uint16 log_category, std::string message){
		...
		if (message.find("\n") != std::string::npos){
			...
			entity_list.Message(0, Log.GetGMSayColorFromCategory(log_category), "%s", message_split[0].c_str());
			for (size_t iter = 1; iter < message_split.size(); ++iter) {
				entity_list.Message(0, Log.GetGMSayColorFromCategory(log_category), "--- %s", message_split[iter].c_str());
			}
		}
		else{
			entity_list.Message(0, Log.GetGMSayColorFromCategory(log_category), "%s", message.c_str());
		}
	}
