...
void Client::AddEXP(uint32 in_add_exp, uint8 conlevel, bool resexp) {
	...
	if(resexp)
		...
		if(RuleB(Character,UseRaceClassExpBonuses)) {
			if(GetBaseRace() == HALFLING || GetClass() == ROGUE || GetClass() == WARRIOR)
				totalmod *= 1.05;
		}
		...
		if(IsLeadershipEXPOn() && ((conlevel == CON_BLUE) || (conlevel == CON_WHITE) || (conlevel == CON_YELLOW) || (conlevel == CON_RED))) {
			...
			if(GetGroup()) {
				if((m_pp.group_leadership_points < MaxBankedGroupLeadershipPoints(GetLevel())) && (RuleI(Character, KillsPerGroupLeadershipAA) > 0)) {
				...
				if((m_pp.raid_leadership_points < MaxBankedRaidLeadershipPoints(GetLevel())) && (RuleI(Character, KillsPerRaidLeadershipAA) > 0)) {
...
void Client::SetEXP(uint32 set_exp, uint32 set_aaxp, bool ismRessurectionExp) {
	...
	else if((set_exp + set_aaxp) < (m_pp.exp + m_pp.expAA))
...
void Client::SetLevel(uint8 set_level, bool command) {
	...
	QueuePacket(outapp);
	safe_delete(outapp);
	this->SendAppearancePacket(AT_WhoLevel, set_level);
...
void Group::SplitExp(uint32 exp, Mob* other) {
	if(other->CastToNPC()->MerchantType != 0 || (other->GetOwner() && other->GetOwner()->IsClient()))
		return;
	...
	if(conlevel == CON_GREEN || membercount == 0)
		return;
