...
void Client::HandleAAAction(aaID activate) {
	...
	switch(caa->action) {
		...
		default:
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unknown AA nonspell action type %d", caa->action);
			return;
	}
...
void Mob::TemporaryPets(uint16 spell_id, Mob *targ, const char *name_override, uint32 duration_override, bool followme, bool sticktarg) {
	...
	if(npc_type == nullptr) {
		//log write
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unknown npc type for swarm pet spell id: %d", spell_id);
...
void Mob::TypesTemporaryPets(uint32 typesid, Mob *targ, const char *name_override, uint32 duration_override, bool followme, bool sticktarg) {
	...
	if(npc_type == nullptr) {
		//log write
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Unknown npc type for swarm pet type id: %d", typesid);
		Message(0,"Unable to find pet!");
		return;
	}
...
{
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::AA, "Starting to buy AA %d", action->ability);
	...
	if(!aa2) {
		...
		for(i=1;i<MAX_AA_ACTION_RANKS;i++){
			...
			if(a <= 0)
				break;
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::AA, "Could not find AA %d, trying potential parent %d", action->ability, a);
			aa2 = zone->FindAA(a);
			if(aa2 != nullptr)
				break;
		}
	...
	if((aa2->id + cur_level) != action->ability) { //got invalid AA
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::AA, "Unable to find or match AA %d (found %d + lvl %d)", action->ability, aa2->id, cur_level);
		return;
	}
	...
	if (m_pp.aapoints >= real_cost && cur_level < aa2->max_level) {
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::AA, "Set AA %d to level %d", aa2->id, cur_level + 1);
...
void Zone::LoadAAs() {
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::Status, "Loading AA information...");
	totalAAs = database.CountAAs();
	if(totalAAs == 0) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Failed to load AAs!");
		aas = nullptr;
		return;
	}
	...
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::Status, "Loading AA Effects...");
	if (database.LoadAAEffects2())
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Status, "Loaded %d AA Effects.", aa_effects.size());
	else
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Failed to load AA Effects!");
}
...
bool ZoneDatabase::LoadAAEffects2() {
	...
	if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in ZoneDatabase::LoadAAEffects2 query: '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return false;
	}

	if (!results.RowCount()) { //no results
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error loading AA Effects, none found in the database.");
        return false;
	}
...
bool ZoneDatabase::LoadAAEffects() {
    ...
    auto results = QueryDatabase(query);
    if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in LoadAAEffects query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return false;
    }
...
uint8 ZoneDatabase::GetTotalAALevels(uint32 skill_id) {
    ...
    auto results = QueryDatabase(query);
    if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in GetTotalAALevels '%s: %s", query.c_str(), results.ErrorMessage().c_str());
        return 0;
    }
...
uint32 ZoneDatabase::CountAAs(){
	...
	auto results = QueryDatabase(query);
	if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in ZoneDatabase::CountAAs query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
        return 0;
	}
...
uint32 ZoneDatabase::CountAAEffects() {
	...
	auto results = QueryDatabase(query);
	if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in ZoneDatabase::CountAALevels query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
        return 0;
	}
...
void ZoneDatabase::LoadAAs(SendAA_Struct **load){
	...
	if (results.Success()) {
		...
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in ZoneDatabase::LoadAAs query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
	}
...
SendAA_Struct* ZoneDatabase::GetAASkillVars(uint32 skill_id)
{
    ...
    auto results = QueryDatabase(query);
    if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in GetAASkillVars '%s': %s", query.c_str(), results.ErrorMessage().c_str());
        return nullptr;
    }
