...
const NPCType *Horse::BuildHorseType(uint16 spell_id) {
	...
	if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in Mount query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return nullptr;
	}
...
void Client::SummonHorse(uint16 spell_id) {
	...
	if(!Horse::IsHorseSpell(spell_id)) {
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "%s tried to summon an unknown horse, spell id %d", GetName(), spell_id);
		return;
	}
