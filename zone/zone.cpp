	...
	hotzone_timer(120000)
{
	...
	if(graveyard_id() > 0)
	{
		...
		if(GraveYardLoaded)
			Log.Out(Logs::General, Logs::None, "Loaded a graveyard for zone %s: graveyard zoneid is %u x is %f y is %f z is %f heading is %f.", short_name, graveyard_zoneid(), graveyard_x(), graveyard_y(), graveyard_z(), graveyard_heading());
		else
...
ZonePoint* Zone::GetClosestZonePoint(float x, float y, float z, uint32 to, Client* client, float max_distance) {
	...
	if(closest_dist > 400.0f && closest_dist < max_distance2)
	{
		...
		Log.Out(Logs::General, Logs::Status, "<Real Zone Points>. %f x %f y %f z ", x, y, z);
	}
