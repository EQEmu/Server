...
void Zone::LoadMercTemplates(){
    ...
    auto results = database.QueryDatabase(query);
    if (!results.Success())
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in ZoneDatabase::LoadMercTemplates()");
	else {
		for (auto row = results.begin(); row != results.end(); ++row) {
...
void Zone::LoadMercSpells(){
    ...
    auto results = database.QueryDatabase(query);
    if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in Zone::LoadMercSpells()");
        return;
    }
    ...
    if(MERC_DEBUG > 0)
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Mercenary Debug: Loaded %i merc spells.", merc_spells_list[1].size() + merc_spells_list[2].size() + merc_spells_list[9].size() + merc_spells_list[12].size());
	...
	hotzone_timer(120000)
{
	...
	if(graveyard_id() > 0)
	{
		...
		if(GraveYardLoaded)
			Log.Out(EQEmuLogSys::General, EQEmuLogSys::None, "Loaded a graveyard for zone %s: graveyard zoneid is %u x is %f y is %f z is %f heading is %f.", short_name, graveyard_zoneid(), graveyard_x(), graveyard_y(), graveyard_z(), graveyard_heading());
		else
...
bool Zone::Init(bool iStaticZone) {
	...
	Log.Out(EQEmuLogSys::General, EQEmuLogSys::Status, "Loading adventure flavor text...");
	LoadAdventureFlavor();
...
ZonePoint* Zone::GetClosestZonePoint(float x, float y, float z, uint32 to, Client* client, float max_distance) {
	...
	if(closest_dist > 400.0f && closest_dist < max_distance2)
	{
		...
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Status, "<Real Zone Points>. %f x %f y %f z ", x, y, z);
	}
...
void Zone::LoadLDoNTraps()
{
    ...
    auto results = database.QueryDatabase(query);
    if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in Zone::LoadLDoNTraps: %s (%s)", query.c_str(), results.ErrorMessage().c_str());
		return;
    }
...
void Zone::LoadLDoNTrapEntries()
{
    ...
    auto results = database.QueryDatabase(query);
    if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in Zone::LoadLDoNTrapEntries: %s (%s)", query.c_str(), results.ErrorMessage().c_str());
		return;
    }
...
void Zone::LoadVeteranRewards()
{
    ...
    auto results = database.QueryDatabase(query);
    if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in Zone::LoadVeteranRewards: %s (%s)", query.c_str(), results.ErrorMessage().c_str());
        return;
    }
...
void Zone::LoadAlternateCurrencies()
{
    ...
    auto results = database.QueryDatabase(query);
    if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in Zone::LoadAlternateCurrencies: %s (%s)", query.c_str(), results.ErrorMessage().c_str());
		return;
    }
...
void Zone::LoadAdventureFlavor()
{
	...
	auto results = database.QueryDatabase(query);
	if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in Zone::LoadAdventureFlavor: %s (%s)", query.c_str(), results.ErrorMessage().c_str());
		return;
	}
