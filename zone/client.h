...
public:
	...
	*/
	...
	virtual uint16 GetSkill(EQ::skills::SkillType skill_id) const { 
		uint16 skill_value = 0;
		if (skill_id <= EQ::skills::HIGHEST_SKILL) {
			if (itembonuses.skillmod[skill_id] > 0) {
				if (itembonuses.skillmodmax[skill_id] > 0) {
					skill_value = std::min(m_pp.skills[skill_id] + itembonuses.skillmodmax[skill_id], m_pp.skills[skill_id] * (100 + itembonuses.skillmod[skill_id]) / 100);
				}
				else {
					skill_value = ((m_pp.skills[skill_id] * (100 + itembonuses.skillmod[skill_id]) / 100));
				}
			}
			else {
				skill_value = m_pp.skills[skill_id];
			}

			if (spellbonuses.ReduceSkill[skill_id] > 0) {
				skill_value = skill_value * (100 - spellbonuses.ReduceSkill[skill_id] / 100);
			}
		}
		return skill_value;
	}
		//	return(itembonuses.skillmod[skill_id] > 0 ? (itembonuses.skillmodmax[skill_id] > 0 ? std::min(m_pp.skills[skill_id] + itembonuses.skillmodmax[skill_id], m_pp.skills[skill_id] * (100 + itembonuses.skillmod[skill_id]) / 100) : m_pp.skills[skill_id] * (100 + itembonuses.skillmod[skill_id]) / 100) : m_pp.skills[skill_id]); } return 0; }
