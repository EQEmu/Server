...
void Object::HandleAugmentation(Client* user, const AugmentItem_Struct* in_augment, Object *worldo)
{
	...
	if(!container)
	{
		logger.Log(EQEmuLogSys::Error, "Player tried to augment an item without a container set.");
...
{
	if (!user || !in_combine) {
		logger.Log(EQEmuLogSys::Error, "Client or NewCombine_Struct not set in Object::HandleCombine");
		return;
	}
	...
	if(success && spec.replace_container) {
		if(worldcontainer){
			...
			logger.Log(EQEmuLogSys::Error, "Replace container combine executed in a world container.");
		}
		else
			user->DeleteItemInInventory(in_combine->container_slot, 0, true);
	}
...
void Object::HandleAutoCombine(Client* user, const RecipeAutoCombine_Struct* rac) {
	...
	if (!database.GetTradeRecipe(rac->recipe_id, rac->object_type, rac->some_id, user->CharacterID(), &spec)) {
		logger.Log(EQEmuLogSys::Error, "Unknown recipe for HandleAutoCombine: %u\n", rac->recipe_id);
		user->QueuePacket(outapp);
		safe_delete(outapp);
		return;
	}
	...
	if (!results.Success()) {
        logger.Log(EQEmuLogSys::Error, "Error in HandleAutoCombine query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		user->QueuePacket(outapp);
		safe_delete(outapp);
		return;
	}
	...
	if(results.RowCount() > 10) {
		logger.Log(EQEmuLogSys::Error, "Error in HandleAutoCombine: too many components returned (%u)", results.RowCount());
		user->QueuePacket(outapp);
		safe_delete(outapp);
		return;
	}
...
void Client::TradeskillSearchResults(const std::string query, unsigned long objtype, unsigned long someid) {
    ...
    auto results = database.QueryDatabase(query);
	if (!results.Success()) {
		logger.Log(EQEmuLogSys::Error, "Error in TradeskillSearchResults query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return;
	}
	...
	if(results.ColumnCount() != 6) {
		logger.Log(EQEmuLogSys::Error, "Error in TradeskillSearchResults query '%s': Invalid column count in result", query.c_str());
		return;
	}
...
void Client::SendTradeskillDetails(uint32 recipe_id) {
    ...
    auto results = database.QueryDatabase(query);
	if (!results.Success()) {
		logger.Log(EQEmuLogSys::Error, "Error in SendTradeskillDetails query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return;
	}
	...
	if(results.RowCount() > 10) {
		logger.Log(EQEmuLogSys::Error, "Error in SendTradeskillDetails: too many components returned (%u)", results.RowCount());
		return;
	}
	...
	uint32 char_id, DBTradeskillRecipe_Struct *spec)
{
	...
	if (!results.Success()) {
		logger.Log(EQEmuLogSys::Error, "Error in GetTradeRecipe search, query: %s", query.c_str());
		logger.Log(EQEmuLogSys::Error, "Error in GetTradeRecipe search, error: %s", results.ErrorMessage().c_str());
		return false;
	}
    ...
    if (results.RowCount() > 1) {
        ...
        for (auto row = results.begin(); row != results.end(); ++row, ++index) {
            ...
            if(index == 214) { //Maximum number of recipe matches (19 * 215 = 4096)
                logger.Log(EQEmuLogSys::Error, "GetTradeRecipe warning: Too many matches. Unable to search all recipe entries. Searched %u of %u possible entries.", index + 1, results.RowCount());
                break;
            }
        }
        ...
        if (!results.Success()) {
            logger.Log(EQEmuLogSys::Error, "Error in GetTradeRecipe, re-query: %s", query.c_str());
            logger.Log(EQEmuLogSys::Error, "Error in GetTradeRecipe, error: %s", results.ErrorMessage().c_str());
            return false;
        }
    }
	...
	if(results.RowCount() > 1) {
		...
		if(results.RowCount() == 0) { //Recipe contents matched more than 1 recipe, but not in this container
			logger.Log(EQEmuLogSys::Error, "Combine error: Incorrect container is being used!");
			return false;
		}

		if (results.RowCount() > 1) //Recipe contents matched more than 1 recipe in this container
			logger.Log(EQEmuLogSys::Error, "Combine error: Recipe is not unique! %u matches found for container %u. Continuing with first recipe match.", results.RowCount(), containerId);
	...
	results = QueryDatabase(query);
    if (!results.Success()) {
        logger.Log(EQEmuLogSys::Error, "Error in tradeskill verify query: '%s': %s", query.c_str(), results.ErrorMessage().c_str());
	...
	uint32 char_id, DBTradeskillRecipe_Struct *spec)
{
    ...
    auto results = QueryDatabase(query);
	if (!results.Success()) {
		logger.Log(EQEmuLogSys::Error, "Error in GetTradeRecipe, query: %s", query.c_str());
		logger.Log(EQEmuLogSys::Error, "Error in GetTradeRecipe, error: %s", results.ErrorMessage().c_str());
		return false;
	}
    ...
    results = QueryDatabase(query);
	if (!results.Success()) {
		logger.Log(EQEmuLogSys::Error, "Error in GetTradeRecept success query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return false;
	}

	if(results.RowCount() < 1) {
		logger.Log(EQEmuLogSys::Error, "Error in GetTradeRecept success: no success items returned");
		return false;
	}
...
void ZoneDatabase::UpdateRecipeMadecount(uint32 recipe_id, uint32 char_id, uint32 madeCount)
{
    ...
    auto results = QueryDatabase(query);
	if (!results.Success())
		logger.Log(EQEmuLogSys::Error, "Error in UpdateRecipeMadecount query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
}
...
void Client::LearnRecipe(uint32 recipeID)
{
    ...
    auto results = database.QueryDatabase(query);
	if (!results.Success()) {
		logger.Log(EQEmuLogSys::Error, "Error in Client::LearnRecipe query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return;
	}
    ...
    if (!results.Success())
        logger.Log(EQEmuLogSys::Error, "Error in LearnRecipe query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
...
bool ZoneDatabase::EnableRecipe(uint32 recipe_id)
{
    ...
    auto results = QueryDatabase(query);
	if (!results.Success())
		logger.Log(EQEmuLogSys::Error, "Error in EnableRecipe query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
...
bool ZoneDatabase::DisableRecipe(uint32 recipe_id)
{
    ...
    auto results = QueryDatabase(query);
	if (!results.Success())
		logger.Log(EQEmuLogSys::Error, "Error in DisableRecipe query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
