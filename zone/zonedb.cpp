...
ZoneDatabase::ZoneDatabase(const char* host, const char* user, const char* passwd, const char* database, uint32 port) : SharedDatabase(host, user, passwd, database, port) {
	ZDBInitVars();
}
...
void ZoneDatabase::UpdateSpawn2Timeleft(uint32 id, uint16 instance_id, uint32 timeleft) {
	...
	if(timeleft == 0) {
		if (!RunQuery(query, MakeAnyLenString(&query, "DELETE FROM respawn_times WHERE id=%lu AND instance_id=%lu",(unsigned long)id, (unsigned long)instance_id),errbuf))
			LogFile->write(EQEMuLog::Error, "Error in UpdateTimeLeft query %s: %s", query, errbuf);
		safe_delete_array(query);
	}
	else {
		if (!RunQuery(query, MakeAnyLenString(&query, "REPLACE INTO respawn_times (id,start,duration,instance_id) VALUES(%lu,%lu,%lu,%lu)",(unsigned long)id, (unsigned long)cur, (unsigned long)timeleft, (unsigned long)instance_id),errbuf))
			LogFile->write(EQEMuLog::Error, "Error in UpdateTimeLeft query %s: %s", query, errbuf);
		safe_delete_array(query);
	}
	return;
...
uint32 ZoneDatabase::GetSpawnTimeLeft(uint32 id, uint16 instance_id) {
	...
	MakeAnyLenString(&query, "SELECT start, duration FROM respawn_times WHERE id=%lu AND instance_id=%lu", (unsigned long)id, (unsigned long)zone->GetInstanceID());
...
const char* ZoneDatabase::GetGlobal(const char* varname, uint32 charid, uint32 npcid, uint32 zoneid) {
	...
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT value FROM quest_globals WHERE `name` = '%s' AND (charid = '%u' || charid = '0') AND (npcid = '%u' || npcid = 0) AND (zoneid = '%u' || zoneid = '0')", varname, charid, npcid, zoneid), errbuf, &result)) {
		...
		std::cerr << "Error in GetGlobal query '" << query << "' " << errbuf << std::endl;
...
void ZoneDatabase::UpdateBug(BugStruct* bug) {
	...
	if (!RunQuery(query, MakeAnyLenString(&query, "INSERT INTO bugs (zone, name, ui, x, y, z, type, flag, target, bug, date) values('%s', '%s', '%s', '%.2f', '%.2f', '%.2f', '%s', %d, '%s', '%s', CURDATE())", zone->GetShortName(), bug->name, uitext == nullptr ? "":uitext, bug->y, bug->x, bug->z, bug->chartype, bug->type, targettext == nullptr ? "Unknown Target":targettext, bugtext == nullptr ? "":bugtext), errbuf))
...
void ZoneDatabase::LoadWorldContainer(uint32 parentid, ItemInst* container) {
	...
	uint32 len_query = MakeAnyLenString(&query, "select bagidx,itemid,charges,augslot1,augslot2,augslot3,augslot4,augslot5 from object_contents where parentid=%i", parentid);
...
void ZoneDatabase::SaveWorldContainer(uint32 zone_id, uint32 parent_id, const ItemInst* container) {
	...
	for (uint8 index = 0; index < 10; index++) {
		...
		if (inst) {
			...
			uint32 len_query = MakeAnyLenString(&query,	"replace into object_contents (zoneid,parentid,bagidx,itemid,charges,augslot1,augslot2,augslot3,augslot4,augslot5,droptime) values (%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,now())", zone_id, parent_id, index, item_id, inst->GetCharges(),augslot[0],augslot[1],augslot[2],augslot[3],augslot[4]);
...
void ZoneDatabase::DeleteWorldContainer(uint32 parent_id,uint32 zone_id) {
	...
	uint32 len_query = MakeAnyLenString(&query, "delete from object_contents where parentid=%i and zoneid=%i", parent_id,zone_id);
	if (!RunQuery(query, len_query, errbuf))
...
ItemInst* ZoneDatabase::LoadSingleTraderItem(uint32 CharID, int SerialNumber) {
	...
	if (RunQuery(query,MakeAnyLenString(&query, "select * from trader where char_id=%i and serialnumber=%i order by slot_id limit 80", CharID, SerialNumber),errbuf,&result)) {
		safe_delete_array(query);
...
void ZoneDatabase::SaveTraderItem(uint32 CharID, uint32 ItemID, uint32 SerialNumber, int32 Charges, uint32 ItemCost, uint8 Slot) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	if (!(RunQuery(query,MakeAnyLenString(&query, "replace INTO trader VALUES(%i,%i,%i,%i,%i,%i)", CharID, ItemID, SerialNumber, Charges, ItemCost, Slot),errbuf)))
...
void ZoneDatabase::UpdateTraderItemCharges(int CharID, uint32 SerialNumber, int32 Charges) {
	...
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	if (!(RunQuery(query,MakeAnyLenString(&query, "update trader set charges=%i where char_id=%i and serialnumber=%i", Charges, CharID, SerialNumber),errbuf)))
...
void ZoneDatabase::UpdateTraderItemPrice(int CharID, uint32 ItemID, uint32 Charges, uint32 NewPrice) {
	...
	if(NewPrice == 0) {
		...
		if (!(RunQuery(Query,MakeAnyLenString(&Query, "delete from trader where char_id=%i and item_id=%i", CharID, ItemID),errbuf)))
		...
		if(!item->Stackable) {
			if (!(RunQuery(Query,MakeAnyLenString(&Query, "update trader set item_cost=%i where char_id=%i and item_id=%i" " and charges=%i", NewPrice, CharID, ItemID, Charges),errbuf)))

				_log(TRADING__CLIENT, "Failed to update price for trader item: %i for char_id: %i, the error was: %s\n",
								ItemID, CharID, errbuf);
		}
		else {
			if (!(RunQuery(Query,MakeAnyLenString(&Query, "update trader set item_cost=%i where char_id=%i and item_id=%i", NewPrice, CharID, ItemID),errbuf)))
				_log(TRADING__CLIENT, "Failed to update price for trader item: %i for char_id: %i, the error was: %s\n", ItemID, CharID, errbuf);
		}
		safe_delete_array(Query);
	}
...
void ZoneDatabase::AddBuyLine(uint32 CharID, uint32 BuySlot, uint32 ItemID, const char* ItemName, uint32 Quantity, uint32 Price) {
	...
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	if (!(RunQuery(query,MakeAnyLenString(&query, "replace INTO buyer VALUES(%i,%i, %i,\"%s\",%i,%i)", CharID, BuySlot, ItemID, ItemName, Quantity, Price),errbuf)))
...
void ZoneDatabase::UpdateBuyLine(uint32 CharID, uint32 BuySlot, uint32 Quantity) {
	...
	if (!(RunQuery(query,MakeAnyLenString(&query, "update buyer set quantity=%i where charid=%i and buyslot=%i", Quantity, CharID, BuySlot), errbuf)))
			...
			while((row = mysql_fetch_row(result))) {
				...
				if (armor_tint_id > 0) {
					if (tmpNPCType->armor_tint[0] == 0) {
						...
						"SELECT red1h,grn1h,blu1h,red2c,grn2c,blu2c,red3a,grn3a,blu3a,red4b,grn4b,blu4b,red5g,grn5g,blu5g,red6l,grn6l,blu6l,red7f,grn7f,blu7f,red8x,grn8x,blu8x,red9x,grn9x,blu9x FROM npc_types_tint WHERE id=%d", armor_tint_id);
				...
				tmpNPCType->see_invis_undead = atoi(row[r++]) == 0 ? false:true;
				if (row[r] != nullptr)
					strn0cpy(tmpNPCType->lastname, row[r], 32);
				r++;

				tmpNPCType->qglobal = atoi(row[r++]) == 0 ? false:true;
			...
			while((row = mysql_fetch_row(result))) {
				...
				if (armor_tint_id > 0) {
					if (tmpNPCType->armor_tint[0] == 0) {
						...
						MakeAnyLenString(&at_query, "SELECT red1h,grn1h,blu1h,red2c,grn2c,blu2c,red3a,grn3a,blu3a,red4b,grn4b,blu4b,red5g,grn5g,blu5g,red6l,grn6l,blu6l,red7f,grn7f,blu7f,red8x,grn8x,blu8x,red9x,grn9x,blu9x FROM npc_types_tint WHERE id=%d", armor_tint_id);
...
bool ZoneDatabase::LoadBlockedSpells(int32 blockedSpellsCount, ZoneSpellsBlocked* into, uint32 zoneid) {
	...
	MakeAnyLenString(&query, "SELECT id, spellid, type, x, y, z, x_diff, y_diff, z_diff, message FROM blocked_spells WHERE zoneid=%d ORDER BY id asc", zoneid);
	if (RunQuery(query, strlen(query), errbuf, &result)) {
		safe_delete_array(query);
		int32 r;
...
uint32 ZoneDatabase::GetKarma(uint32 acct_id) {
	...
	if (!RunQuery(query,MakeAnyLenString(&query, "select `karma` from `account` where `id`='%i' limit 1", acct_id),errbuf,&result)) {
		safe_delete_array(query);
		return 0;
	}
	safe_delete_array(query);
...
void ZoneDatabase::ListAllInstances(Client* c, uint32 charid) {
	...
	if (RunQuery(query,MakeAnyLenString(&query, "SELECT instance_list.id, zone, version FROM instance_list JOIN instance_list_player ON instance_list.id = instance_list_player.id WHERE instance_list_player.charid=%lu", (unsigned long)charid),errbuf,&result)) {
		safe_delete_array(query);
		char name[64];
		database.GetCharName(charid, name);
		c->Message(0, "%s is part of the following instances:", name);
		while(row = mysql_fetch_row(result))
			c->Message(0, "%s - id: %lu, version: %lu", database.GetZoneName(atoi(row[1])), (unsigned long)atoi(row[0]), (unsigned long)atoi(row[2]));
		mysql_free_result(result);
	}
	else
		safe_delete_array(query);
...
void ZoneDatabase::UpdateAltCurrencyValue(uint32 char_id, uint32 currency_id, uint32 value) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	database.RunQuery(query, MakeAnyLenString(&query, "REPLACE INTO character_alt_currency (char_id, currency_id, amount) VALUES('%u', '%u', '%u')", char_id, currency_id, value), errbuf);
	safe_delete_array(query);
}
...
void ZoneDatabase::LoadBuffs(Client *c) {
	...
	MYSQL_RES *result;
	MYSQL_ROW row;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT spell_id, slot_id, caster_level, caster_name, ticsremaining, counters, numhits, melee_rune, magic_rune, persistent, dot_rune, caston_x, caston_y, caston_z, ExtraDIChance FROM `character_buffs` WHERE `character_id`='%u'", c->CharacterID()), errbuf, &result)) {
		safe_delete_array(query);
		while ((row = mysql_fetch_row(result))) {
...
void ZoneDatabase::SavePetInfo(Client *c) {
	...
	if(!database.RunQuery(query, MakeAnyLenString(&query, "DELETE FROM `character_pet_buffs` WHERE `char_id`=%u", c->CharacterID()), errbuf)) {
		safe_delete_array(query);
		return;
	}
	safe_delete_array(query);
	if (!database.RunQuery(query, MakeAnyLenString(&query, "DELETE FROM `character_pet_inventory` WHERE `char_id`=%u", c->CharacterID()), errbuf)) {
		safe_delete_array(query);
		return;
	}
	safe_delete_array(query);

	if(!database.RunQuery(query, MakeAnyLenString(&query, "INSERT INTO `character_pet_info` (`char_id`, `pet`, `petname`, `petpower`, `spell_id`, `hp`, `mana`, `size`) values (%u, 0, '%s', %i, %u, %u, %u, %f) ON DUPLICATE KEY UPDATE `petname`='%s', `petpower`=%i, `spell_id`=%u, `hp`=%u, `mana`=%u, `size`=%f", c->CharacterID(), petinfo->Name, petinfo->petpower, petinfo->SpellID, petinfo->HP, petinfo->Mana, petinfo->size,petinfo->Name, petinfo->petpower, petinfo->SpellID, petinfo->HP, petinfo->Mana, petinfo->size), errbuf)) {
		safe_delete_array(query);
		return;
	}
	safe_delete_array(query);
	...
	for(i = 0; i < RuleI(Spells, MaxTotalSlotsPET); i++) {
		if (petinfo->Buffs[i].spellid != SPELL_UNKNOWN && petinfo->Buffs[i].spellid != 0) {
			database.RunQuery(query, MakeAnyLenString(&query, "INSERT INTO `character_pet_buffs` (`char_id`, `pet`, `slot`, `spell_id`, `caster_level`, `ticsremaining`, `counters`) values (%u, 0, %u, %u, %u, %u, %d)",c->CharacterID(), i, petinfo->Buffs[i].spellid, petinfo->Buffs[i].level, petinfo->Buffs[i].duration,petinfo->Buffs[i].counters),errbuf);
			safe_delete_array(query);
		}
		if (suspended->Buffs[i].spellid != SPELL_UNKNOWN && suspended->Buffs[i].spellid != 0) {
			database.RunQuery(query, MakeAnyLenString(&query,"INSERT INTO `character_pet_buffs` (`char_id`, `pet`, `slot`, `spell_id`, `caster_level`, `ticsremaining`, `counters`) values (%u, 1, %u, %u, %u, %u, %d)", c->CharacterID(), i, suspended->Buffs[i].spellid, suspended->Buffs[i].level, suspended->Buffs[i].duration, suspended->Buffs[i].counters), errbuf);
			safe_delete_array(query);
		}
	}
	...
	for(i = 0; i < MAX_WORN_INVENTORY; i++) {
		if(petinfo->Items[i]) {
			database.RunQuery(query, MakeAnyLenString(&query, "INSERT INTO `character_pet_inventory` (`char_id`, `pet`, `slot`, `item_id`) values (%u, 0, %u, %u)", c->CharacterID(), i, petinfo->Items[i]), errbuf);
			safe_delete_array(query);
		}
	}


	if(!database.RunQuery(query, MakeAnyLenString(&query, "INSERT INTO `character_pet_info` (`char_id`, `pet`, `petname`, `petpower`, `spell_id`, `hp`, `mana`, `size`) values (%u, 1, '%s', %u, %u, %u, %u, %f) ON DUPLICATE KEY UPDATE `petname`='%s', `petpower`=%i, `spell_id`=%u, `hp`=%u, `mana`=%u, `size`=%f", c->CharacterID(), suspended->Name, suspended->petpower, suspended->SpellID, suspended->HP, suspended->Mana, suspended->size, suspended->Name, suspended->petpower, suspended->SpellID, suspended->HP, suspended->Mana, suspended->size), errbuf)) {
		safe_delete_array(query);
		return;
	}
	safe_delete_array(query);
	...
	for(i = 0; i < MAX_WORN_INVENTORY; i++) {
		if(suspended->Items[i]) {
			database.RunQuery(query, MakeAnyLenString(&query, "INSERT INTO `character_pet_inventory` (`char_id`, `pet`, `slot`, `item_id`) values (%u, 1, %u, %u)", c->CharacterID(), i, suspended->Items[i]), errbuf);
			safe_delete_array(query);
		}
	}
...
void ZoneDatabase::LoadPetInfo(Client *c) {
	...
	if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT `pet`, `petname`, `petpower`, `spell_id`, `hp`, `mana`, `size` from `character_pet_info` where `char_id`=%u", c->CharacterID()), errbuf, &result)) {
		safe_delete_array(query);
		while ((row = mysql_fetch_row(result))) {
			pet = atoi(row[0]);
			if (pet == 0)
	...
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT `pet`, `slot`, `spell_id`, `caster_level`, `castername`, `ticsremaining`, `counters` FROM `character_pet_buffs` WHERE `char_id`=%u", c->CharacterID()), errbuf, &result)) {
		safe_delete_array(query);
		while ((row = mysql_fetch_row(result))) {
			pet = atoi(row[0]);
			if (pet == 0)
	...
	if (database.RunQuery(query, MakeAnyLenString(&query, "SELECT `pet`, `slot`, `item_id` FROM `character_pet_inventory` WHERE `char_id`=%u", c->CharacterID()), errbuf, &result)) {
		safe_delete_array(query);
		while((row = mysql_fetch_row(result))) {
			pet = atoi(row[0]);
			if (pet == 0)
...
bool ZoneDatabase::SetCharacterFactionLevel(uint32 char_id, int32 faction_id, int32 value, uint8 temp, faction_map &val_list) {
	...
	if (!RunQuery(query, MakeAnyLenString(&query, "INSERT INTO faction_values (char_id,faction_id,current_value,temp) VALUES (%i,%i,%i,%i)", char_id, faction_id,value,temp), errbuf, 0, &affected_rows)) {
		std::cerr << "Error in SetCharacterFactionLevel query '" << query << "' " << errbuf << std::endl;
		safe_delete_array(query);
		return false;
	}
