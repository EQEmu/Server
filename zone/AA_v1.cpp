...
void Client::BuyAA(AA_Action* action)
{
	...
	if(m_pp.aapoints >= real_cost && cur_level < aa2->max_level) {
		...
		if ((RuleB(AA, Stacking) && (GetClientVersionBit() >= 4))
			&& ((aa2->max_level == (cur_level+1)) && aa2->sof_next_id)){
			SendAA(aa2->id);
			SendAA(aa2->sof_next_id);
		}
		//else if ((RuleB(AA, Stacking) && (GetClientVersionBit() >= 4) && cur_level == 0 && aa2->hotkey_sid != 4294967295u))
		//{
			//Shout("Current lv 0 for AA hot key %i %i", cur_level, aa2->hotkey_sid);
		//}
...
void Client::SendAA(uint32 id, int seq, bool seqrest) {
	...
	Shout("Reset: %i", seqrest);

	if(id==0){
		saa2 = zone->GetAABySequence(seq);
		//Shout("SAA2 %i x seq %i", GetAA(saa2->id), seq);
	}
	else
		saa2 = zone->FindAA(id);
	if(!saa2)
...
*/
	...
	*/
	...
	SendAA_Struct* saa=(SendAA_Struct*)buffer;
	memcpy(saa,saa2,size);
	Shout("[AA ID %i] SEQ %i SEQLIVE %i",saa->id, saa->seq, saa->seqlive);
	if(saa->spellid==0)
		saa->spellid=0xFFFFFFFF;
	...
	if(value && saa->id){
		...
		if(value < saa->max_level){
			...
			//Max value of current tier reached.
			saa->id+=value-1;
			...
			//	Shout("SET SEQ 1");
			//}
			Shout("AA Completed: Next");
		}

		else{
			saa->id+=value-1;
			saa->next_id=0xFFFFFFFF;
			Shout("AA Completed: Final");
		}
		...
		saa->last_id=saa->id-1;
		Shout("1 Current level %i + Value %i",saa->sof_current_level, value);
	...
	if (aa_stack){
		Shout("2 Current level %i VALUE %i",saa->sof_current_level, value);
		//After finishing an AA tier transfer over the current level to the next tier to display.
		if (saa->sof_current_level >= 1 && value == 0){
			saa->current_level = saa->sof_current_level+1;

			Shout("value = 0 SET LAST AND SEQ");
			saa->last_id = 131;
			//saa->seq = 38;
			if (saa->seqlive)
				saa->seq = saa->seqlive;
...
void Client::SendAAList(bool seqrest){
