...
void Client::SendTributeDetails(uint32 client_id, uint32 tribute_id) {
	if(tribute_list.count(tribute_id) != 1) {
		logger.Log(EQEmuLogSys::Error, "Details request for invalid tribute %lu", (unsigned long)tribute_id);
		return;
	}
...
bool ZoneDatabase::LoadTributes() {
	...
	if (!results.Success()) {
        logger.Log(EQEmuLogSys::Error, "Error in LoadTributes first query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return false;
	}
	...
	if (!results.Success()) {
        logger.Log(EQEmuLogSys::Error, "Error in LoadTributes level query '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return false;
	}
    ...
    for (auto row = results.begin(); row != results.end(); ++row) {
        ...
        if(tribute_list.count(id) != 1) {
            logger.Log(EQEmuLogSys::Error, "Error in LoadTributes: unknown tribute %lu in tribute_levels", (unsigned long)id);
            continue;
        }
        ...
        if(cur.tier_count >= MAX_TRIBUTE_TIERS) {
            logger.Log(EQEmuLogSys::Error, "Error in LoadTributes: on tribute %lu: more tiers defined than permitted", (unsigned long)id);
            continue;
        }
