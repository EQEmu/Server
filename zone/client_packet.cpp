...
void Client::Handle_Connect_OP_ZoneEntry(const EQApplicationPacket *app)
{
	...
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "%s inventory version to %s(%i)", (siv ? "Succeeded in setting" : "Failed to set"), EQClientVersionName(ClientVersion), ClientVersion);
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::AA, "Received OP_AAAction");
...
void Client::Handle_OP_AcceptNewTask(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(AcceptNewTask_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_AcceptNewTask expected %i got %i",
...
{
	if (app->size != sizeof(EntityId_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_AssistGroup expected %i got %i", sizeof(EntityId_Struct), app->size);
		return;
	}
	QueuePacket(app);
	return;
...
void Client::Handle_OP_AugmentInfo(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(AugmentInfo_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_AugmentInfo expected %i got %i",
...
void Client::Handle_OP_AutoFire(const EQApplicationPacket *app)
{
	if (app->size != sizeof(bool)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_AutoFire expected %i got %i", sizeof(bool), app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_Bandolier(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(BandolierCreate_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_Bandolier expected %i got %i",
	...
	case BandolierSet:
		...
		break;
	default:
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Uknown Bandolier action %i", bs->action);
...
void Client::Handle_OP_BankerChange(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_BankerChange expected %i got %i", sizeof(BankerChange_Struct), app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_Barter(const EQApplicationPacket *app)
{
	...
	if (app->size < 4)
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "OP_Barter packet below minimum expected size. The packet was %i bytes.", app->size);
		DumpPacket(app);
		return;
	}
	...
	default:
		...
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Trading, "Unrecognised Barter Action %i", Action);
...
void Client::Handle_OP_BlockedBuffs(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_BlockedBuffs expected %i got %i",
...
void Client::Handle_OP_Buff(const EQApplicationPacket *app)
{
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Spells, "Client requested that buff with spell id %d be canceled.", spid);
...
void Client::Handle_OP_CancelTask(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(CancelTask_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_CancelTask expected %i got %i",
...
void Client::Handle_OP_ClearBlockedBuffs(const EQApplicationPacket *app)
{
	...
	if (app->size != 1)
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_ClearBlockedBuffs expected 1 got %i", app->size);
...
void Client::Handle_OP_ClearNPCMarks(const EQApplicationPacket *app)
{
	...
	if (app->size != 0)
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_ClearNPCMarks expected 0 got %i",
			app->size);
...
void Client::Handle_OP_DelegateAbility(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_DelegateAbility expected %i got %i",
...
void Client::Handle_OP_DoGroupLeadershipAbility(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(DoGroupLeadershipAbility_Struct)) {
		...
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_DoGroupLeadershipAbility expected %i got %i",
	...
	default:
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Got unhandled OP_DoGroupLeadershipAbility Ability: %d Parameter: %d",
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GetGuildMOTD");
...
void Client::Handle_OP_GMTraining(const EQApplicationPacket *app)
{
	if (app->size != sizeof(GMTrainee_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_GMTraining expected %i got %i", sizeof(GMTrainee_Struct), app->size);
		DumpPacket(app);
		return;
	}
	OPGMTraining(app);
...
void Client::Handle_OP_GuildCreate(const EQApplicationPacket *app)
{
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "%s: Creating guild %s with leader %d via UF+ GUI. It was given id %lu.", GetName(),
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GuildDemote");

	if (app->size != sizeof(GuildDemoteStruct)) {
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Error: app size of %i != size of GuildDemoteStruct of %i\n", app->size, sizeof(GuildDemoteStruct));
		return;
	}
	...
	if (!IsInAGuild())
		...
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Demoting %s (%d) from rank %s (%d) to %s (%d) in %s (%d)",
...
void Client::Handle_OP_GuildManageBanker(const EQApplicationPacket *app)
{
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Got OP_GuildManageBanker of len %d", app->size);
	if (app->size != sizeof(GuildManageBanker_Struct)) {
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Error: app size of %i != size of OP_GuildManageBanker of %i\n", app->size, sizeof(GuildManageBanker_Struct));
		return;
	}
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GuildPromote");
	...
	if (!IsInAGuild())
		...
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Promoting %s (%d) from rank %s (%d) to %s (%d) in %s (%d)",
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Received OP_GuildPublicNote");
	...
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Guilds, "Setting public note on %s (%d) in guild %s (%d) to: %s",
...
void Client::Handle_OP_GuildStatus(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_GuildStatus expected %i got %i",
...
void Client::Handle_OP_GuildUpdateURLAndChannel(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_GuildUpdateURLAndChannel expected %i got %i",
...
void Client::Handle_OP_HideCorpse(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_HideCorpse expected %i got %i",
...
void Client::Handle_OP_ItemVerifyRequest(const EQApplicationPacket *app)
{
	...
	if (slot_id < 0) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Unknown slot being used by %s, slot being used is: %i", GetName(), request->slot);
		return;
	}
	...
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "OP ItemVerifyRequest: spell=%i, target=%i, inv=%i", spell_id, target_id, slot_id);
	...
	if (m_inv.SupportsClickCasting(slot_id) || ((item->ItemType == ItemTypePotion || item->PotionBelt) && m_inv.SupportsPotionBeltCasting(slot_id))) // sanity check
	{
		...
		{
			Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Item with no effect right clicked by %s", GetName());
		}
		else if (inst->IsType(ItemClassCommon))
		{
			...
			else
			{
				if (GetClientVersion() >= EQClientSoD && !inst->IsEquipable(GetBaseRace(), GetClass()))
				{
					...
					{
						Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Error: unknown item->Click.Type (%i)", item->Click.Type);
					}
					else
					{
						...
						{
#if EQDEBUG >= 1
							Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Drinking Alcohol from slot:%i", slot_id);
#endif
...
{
	if (app->size != 1) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_LeadershipExpToggle expected %i got %i", 1, app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_MercenaryCommand(const EQApplicationPacket *app)
{
	if (app->size != sizeof(MercenaryCommand_Struct))
	{
		...
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_MercenaryCommand expected %i got %i", sizeof(MercenaryCommand_Struct), app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_MercenaryDataRequest(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_MercenaryDataRequest expected 4 got %i", app->size);
...
void Client::Handle_OP_MercenaryDataUpdateRequest(const EQApplicationPacket *app)
{
	...
	if (app->size != 0)
	{
		...
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_MercenaryDataUpdateRequest expected 0 got %i", app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_MercenaryDismiss(const EQApplicationPacket *app)
{
	...
	if (app->size > 1)
	{
		...
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_MercenaryDismiss expected 0 got %i", app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_MercenaryHire(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_MercenaryHire expected %i got %i", sizeof(MercenaryMerchantRequest_Struct), app->size);
...
void Client::Handle_OP_MercenarySuspendRequest(const EQApplicationPacket *app)
{
	if (app->size != sizeof(SuspendMercenary_Struct))
	{
		...
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_MercenarySuspendRequest expected %i got %i", sizeof(SuspendMercenary_Struct), app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_MercenaryTimerRequest(const EQApplicationPacket *app)
{
	...
	if (app->size > 1)
	{
		...
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_MercenaryTimerRequest expected 0 got %i", app->size);
		DumpPacket(app);
		return;
	}
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Tribute, "Received OP_OpenGuildTributeMaster of length %d", app->size);
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Tribute, "Received OP_OpenTributeMaster of length %d", app->size);
	_pkt(TRIBUTE__IN, app);
...
void Client::Handle_OP_PopupResponse(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(PopupResponse_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_PopupResponse expected %i got %i",
...
void Client::Handle_OP_PotionBelt(const EQApplicationPacket *app)
{
	if (app->size != sizeof(MovePotionToBelt_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_PotionBelt expected %i got %i",
	...
	if(!EQEmu::ValueWithin(mptbs->SlotNumber, 0U, 3U)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Client::Handle_OP_PotionBelt mptbs->SlotNumber out of range.");
		return;
	}
...
void Client::Handle_OP_PVPLeaderBoardDetailsRequest(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_PVPLeaderBoardDetailsRequest expected %i got %i",
...
void Client::Handle_OP_PVPLeaderBoardRequest(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_PVPLeaderBoardRequest expected %i got %i",
...
void Client::Handle_OP_RecipesFavorite(const EQApplicationPacket *app)
{
	...
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Requested Favorites for: %d - %d\n", tsf->object_type, tsf->some_id);
...
void Client::Handle_OP_RecipesSearch(const EQApplicationPacket *app)
{
	...
	Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Requested search recipes for: %d - %d\n", rss->object_type, rss->some_id);
...
void Client::Handle_OP_RemoveBlockedBuffs(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(BlockedBuffs_Struct))
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_RemoveBlockedBuffs expected %i got %i",
			sizeof(BlockedBuffs_Struct), app->size);
...
void Client::Handle_OP_RespawnWindow(const EQApplicationPacket *app)
{
	...
	if (app->size != 4)
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_RespawnWindow expected %i got %i",
			4, app->size);
		DumpPacket(app);
		return;
	}
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Tribute, "Received OP_SelectTribute of length %d", app->size);
	_pkt(TRIBUTE__IN, app);
...
void Client::Handle_OP_TaskHistoryRequest(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(TaskHistoryRequest_Struct)) {
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_TaskHistoryRequest expected %i got %i",
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Tribute, "Received OP_TributeItem of length %d", app->size);
	_pkt(TRIBUTE__IN, app);
	...
	if (app->size != sizeof(TributeItem_Struct))
		...
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Tribute, "Sending tribute item reply with %d points", t->tribute_points);
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Tribute, "Received OP_TributeMoney of length %d", app->size);
	_pkt(TRIBUTE__IN, app);
	...
	if (app->size != sizeof(TributeMoney_Struct))
		...
		Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Tribute, "Sending tribute money reply with %d points", t->tribute_points);
		_pkt(TRIBUTE__OUT, app);
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Tribute, "Received OP_TributeNPC of length %d", app->size);
	_pkt(TRIBUTE__IN, app);
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Tribute, "Received OP_TributeToggle of length %d", app->size);
	_pkt(TRIBUTE__IN, app);
...
{
	Log.DebugCategory(EQEmuLogSys::Detail, EQEmuLogSys::Tribute, "Received OP_TributeUpdate of length %d", app->size);
	_pkt(TRIBUTE__IN, app);
...
void Client::Handle_OP_VetClaimRequest(const EQApplicationPacket *app)
{
	...
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "OP_VetClaimRequest size lower than expected: got %u expected at least %u", app->size, sizeof(VeteranClaimRequest));
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_VoiceMacroIn(const EQApplicationPacket *app)
{
	...
	if (app->size != sizeof(VoiceMacroIn_Struct)) {
		...
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_VoiceMacroIn expected %i got %i",
...
void Client::Handle_OP_XTargetAutoAddHaters(const EQApplicationPacket *app)
{
	if (app->size != 1)
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_XTargetAutoAddHaters, expected 1, got %i", app->size);
		DumpPacket(app);
		return;
	}
...
void Client::Handle_OP_XTargetRequest(const EQApplicationPacket *app)
{
	if (app->size < 12)
	{
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Size mismatch in OP_XTargetRequest, expected at least 12, got %i", app->size);
		DumpPacket(app);
		return;
	}
	...
	default:
		Log.DebugCategory(EQEmuLogSys::General, EQEmuLogSys::None, "Unhandled XTarget Type %i", Type);
		break;
	}
