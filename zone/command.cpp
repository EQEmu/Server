...
void command_faction(Client *c, const Seperator *sep)
{
	...
	if (strcasecmp(sep->arg[1], "find") == 0) {
		...
		if (strcasecmp(sep->arg[2], "all") == 0) {
			...
			query = StringFormat("SELECT `id`,`name` FROM `faction_list`");
		}
		else {
			query = StringFormat("SELECT `id`,`name` FROM `faction_list` WHERE `name` LIKE '%%%s%%'", faction_filter.c_str());
		}
		auto results = database.QueryDatabase(query);
		if (!results.Success())
			return;
		...
		std::string response = _ctr > 0 ? StringFormat("Found %i matching factions", _ctr).c_str() : "No factions found.";
	...
	if (strcasecmp(sep->arg[1], "review") == 0) {
		...
		std::string response = _ctr2 > 0 ? StringFormat("Found %i matching factions", _ctr2).c_str() : "No faction hits found.";
		c->Message(Chat::Yellow, response.c_str());
	}
