...
int command_init(void)
{
	...
	if (
		command_add("acceptrules", "[acceptrules] - Accept the EQEmu Agreement", EQ::constants::AccountStatus::Player, command_acceptrules) ||
		command_add("advnpcspawn", "[maketype|makegroup|addgroupentry|addgroupspawn][removegroupspawn|movespawn|editgroupbox|cleargroupbox]", EQ::constants::AccountStatus::GMLeadAdmin, command_advnpcspawn) ||
		command_add("aggro", "(range) [-v] - Display aggro information for all mobs 'range' distance from your target. -v is verbose faction info.", EQ::constants::AccountStatus::QuestTroupe, command_aggro) ||
		command_add("aggrozone", "[aggro] - Aggro every mob in the zone with X aggro. Default is 0. Not recommend if you're not invulnerable.", EQ::constants::AccountStatus::GMAdmin, command_aggrozone) ||
		command_add("ai", "[factionid/spellslist/con/guard/roambox/stop/start] - Modify AI on NPC target", EQ::constants::AccountStatus::GMAdmin, command_ai) ||
		command_add("appearance", "[type] [value] - Send an appearance packet for you or your target", EQ::constants::AccountStatus::GMLeadAdmin, command_appearance) ||
		command_add("apply_shared_memory", "[shared_memory_name] - Tells every zone and world to apply a specific shared memory segment by name.", EQ::constants::AccountStatus::GMImpossible, command_apply_shared_memory) ||
		command_add("attack", "[targetname] - Make your NPC target attack targetname", EQ::constants::AccountStatus::GMLeadAdmin, command_attack) ||
		command_add("augmentitem",  "Force augments an item. Must have the augment item window open.", EQ::constants::AccountStatus::GMImpossible, command_augmentitem) ||
		command_add("ban", "[name] [reason]- Ban by character name", EQ::constants::AccountStatus::GMLeadAdmin, command_ban) ||
		command_add("beard", "- Change the beard of your target", EQ::constants::AccountStatus::QuestTroupe, command_beard) ||
		command_add("beardcolor", "- Change the beard color of your target", EQ::constants::AccountStatus::QuestTroupe, command_beardcolor) ||
		command_add("bestz", "- Ask map for a good Z coord for your x,y coords.", EQ::constants::AccountStatus::Player, command_bestz) ||
		command_add("bind", "- Sets your targets bind spot to their current location", EQ::constants::AccountStatus::GMMgmt, command_bind) ||

#ifdef BOTS
		command_add("bot", "- Type \"#bot help\" or \"^help\" to the see the list of available commands for bots.", EQ::constants::AccountStatus::Player, command_bot) ||
#endif

		command_add("camerashake",  "Shakes the camera on everyone's screen globally.", EQ::constants::AccountStatus::QuestTroupe, command_camerashake) ||
		command_add("castspell", "[Spell ID] [Instant (0 = False, 1 = True, Default is 1 if Unused)] - Cast a spell", EQ::constants::AccountStatus::Guide, command_castspell) ||
		command_add("chat", "[channel num] [message] - Send a channel message to all zones", EQ::constants::AccountStatus::GMMgmt, command_chat) ||
		command_add("checklos", "- Check for line of sight to your target", EQ::constants::AccountStatus::Guide, command_checklos) ||
		command_add("copycharacter", "[source_char_name] [dest_char_name] [dest_account_name] Copies character to destination account", EQ::constants::AccountStatus::GMImpossible, command_copycharacter) ||
		command_add("corpse", "- Manipulate corpses, use with no arguments for help", EQ::constants::AccountStatus::Guide, command_corpse) ||
		command_add("corpsefix", "Attempts to bring corpses from underneath the ground within close proximity of the player", EQ::constants::AccountStatus::Player, command_corpsefix) ||
		command_add("cvs", "- Summary of client versions currently online.", EQ::constants::AccountStatus::GMMgmt, command_cvs) ||
		command_add("damage", "[amount] - Damage your target", EQ::constants::AccountStatus::GMAdmin, command_damage) ||
		command_add("databuckets", "View|Delete [key] [limit]- View data buckets, limit 50 default or Delete databucket by key", EQ::constants::AccountStatus::QuestTroupe, command_databuckets) ||
		command_add("date", "[yyyy] [mm] [dd] [HH] [MM] - Set EQ time", EQ::constants::AccountStatus::EQSupport, command_date) ||
		command_add("dbspawn2", "[spawngroup] [respawn] [variance] - Spawn an NPC from a predefined row in the spawn2 table", EQ::constants::AccountStatus::GMAdmin, command_dbspawn2) ||
		command_add("delacct", "[accountname] - Delete an account", EQ::constants::AccountStatus::GMLeadAdmin, command_delacct) ||
		command_add("deletegraveyard", "[zone name] - Deletes the graveyard for the specified zone.", EQ::constants::AccountStatus::GMMgmt, command_deletegraveyard) ||
		command_add("delpetition", "[petition number] - Delete a petition", EQ::constants::AccountStatus::ApprenticeGuide, command_delpetition) ||
		command_add("depop", "- Depop your NPC target", EQ::constants::AccountStatus::Guide, command_depop) ||
		command_add("depopzone", "- Depop the zone", EQ::constants::AccountStatus::GMAdmin, command_depopzone) ||
		command_add("devtools", "- Manages devtools", EQ::constants::AccountStatus::GMMgmt, command_devtools) ||
		command_add("details", "- Change the details of your target (Drakkin Only)", EQ::constants::AccountStatus::QuestTroupe, command_details) ||
		command_add("disablerecipe",  "[recipe_id] - Disables a recipe using the recipe id.", EQ::constants::AccountStatus::QuestTroupe, command_disablerecipe) ||
		command_add("disarmtrap",  "Analog for ldon disarm trap for the newer clients since we still don't have it working.", EQ::constants::AccountStatus::QuestTroupe, command_disarmtrap) ||
		command_add("distance", "- Reports the distance between you and your target.", EQ::constants::AccountStatus::QuestTroupe, command_distance) ||
		command_add("door", "Door editing command", EQ::constants::AccountStatus::QuestTroupe, command_door) ||
		command_add("doanim", "[animnum] [type] - Send an EmoteAnim for you or your target", EQ::constants::AccountStatus::Guide, command_doanim) ||
		command_add("dye", "[slot|'help'] [red] [green] [blue] [use_tint] - Dyes the specified armor slot to Red, Green, and Blue provided, allows you to bypass darkness limits.", EQ::constants::AccountStatus::ApprenticeGuide, command_dye) ||
		command_add("dz", "Manage expeditions and dynamic zone instances", EQ::constants::AccountStatus::QuestTroupe, command_dz) ||
		command_add("dzkickplayers", "Removes all players from current expedition. (/kickplayers alternative for pre-RoF clients)", EQ::constants::AccountStatus::Player, command_dzkickplayers) ||
		command_add("editmassrespawn", "[name-search] [second-value] - Mass (Zone wide) NPC respawn timer editing command", EQ::constants::AccountStatus::GMAdmin, command_editmassrespawn) ||
		command_add("emote", "['name'/'world'/'zone'] [type] [message] - Send an emote message", EQ::constants::AccountStatus::QuestTroupe, command_emote) ||
		command_add("emotesearch", "Searches NPC Emotes", EQ::constants::AccountStatus::QuestTroupe, command_emotesearch) ||
		command_add("emoteview", "Lists all NPC Emotes", EQ::constants::AccountStatus::QuestTroupe, command_emoteview) ||
		command_add("emptyinventory", "- Clears you or your target's entire inventory (Equipment, General, Bank, and Shared Bank)", EQ::constants::AccountStatus::GMImpossible, command_emptyinventory) ||
		command_add("enablerecipe",  "[recipe_id] - Enables a recipe using the recipe id.", EQ::constants::AccountStatus::QuestTroupe, command_enablerecipe) ||
		command_add("endurance", "Restores you or your target's endurance.", EQ::constants::AccountStatus::Guide, command_endurance) ||
		command_add("equipitem", "[slotid(0-21)] - Equip the item on your cursor into the specified slot", EQ::constants::AccountStatus::Guide, command_equipitem) ||
		command_add("face", "- Change the face of your target", EQ::constants::AccountStatus::QuestTroupe, command_face) ||
		command_add("faction", "[Find (criteria | all ) | Review (criteria | all) | Reset (id)] - Resets Player's Faction", EQ::constants::AccountStatus::QuestTroupe, command_faction) ||
		command_add("findaliases", "[search criteria]- Searches for available command aliases, by alias or command", EQ::constants::AccountStatus::Player, command_findaliases) ||
		command_add("findclass", "[search criteria] - Search for a class", EQ::constants::AccountStatus::Guide, command_findclass) ||
		command_add("findfaction", "[search criteria] - Search for a faction", EQ::constants::AccountStatus::Guide, command_findfaction) ||
		command_add("findnpctype", "[search criteria] - Search database NPC types", EQ::constants::AccountStatus::GMAdmin, command_findnpctype) ||
		command_add("findrace", "[search criteria] - Search for a race", EQ::constants::AccountStatus::Guide, command_findrace) ||
		command_add("findskill", "[search criteria] - Search for a skill", EQ::constants::AccountStatus::Guide, command_findskill) ||
		command_add("findspell", "[search criteria] - Search for a spell", EQ::constants::AccountStatus::Guide, command_findspell) ||
		command_add("findtask", "[search criteria] - Search for a task", EQ::constants::AccountStatus::Guide, command_findtask) ||
		command_add("findzone", "[search criteria] - Search database zones", EQ::constants::AccountStatus::GMAdmin, command_findzone) ||
		command_add("fixmob", "[race|gender|texture|helm|face|hair|haircolor|beard|beardcolor|heritage|tattoo|detail] [next|prev] - Manipulate appearance of your target", EQ::constants::AccountStatus::QuestTroupe, command_fixmob) ||
		command_add("flag", "[status] [acctname] - Refresh your admin status, or set an account's admin status if arguments provided", EQ::constants::AccountStatus::Player, command_flag) ||
		command_add("flagedit", "- Edit zone flags on your target", EQ::constants::AccountStatus::GMAdmin, command_flagedit) ||
		command_add("flags", "- displays the flags of you or your target", EQ::constants::AccountStatus::Player, command_flags) ||
		command_add("flymode", "[0/1/2/3/4/5] - Set your or your player target's flymode to ground/flying/levitate/water/floating/levitate_running", EQ::constants::AccountStatus::Guide, command_flymode) ||
		command_add("fov", "- Check wether you're behind or in your target's field of view", EQ::constants::AccountStatus::QuestTroupe, command_fov) ||
		command_add("freeze", "- Freeze your target", EQ::constants::AccountStatus::QuestTroupe, command_freeze) ||
		command_add("gassign", "[id] - Assign targetted NPC to predefined wandering grid id", EQ::constants::AccountStatus::GMAdmin, command_gassign) ||
		command_add("gearup", "Developer tool to quickly equip a character", EQ::constants::AccountStatus::GMMgmt, command_gearup) ||
		command_add("gender", "[0/1/2] - Change your or your target's gender to male/female/neuter", EQ::constants::AccountStatus::Guide, command_gender) ||
		command_add("getplayerburiedcorpsecount", "- Get the target's total number of buried player corpses.", EQ::constants::AccountStatus::GMAdmin, command_getplayerburiedcorpsecount) ||
		command_add("getvariable", "[varname] - Get the value of a variable from the database", EQ::constants::AccountStatus::GMMgmt, command_getvariable) ||
		command_add("ginfo", "- get group info on target.", EQ::constants::AccountStatus::ApprenticeGuide, command_ginfo) ||
		command_add("giveitem", "[itemid] [charges] - Summon an item onto your target's cursor. Charges are optional.", EQ::constants::AccountStatus::GMMgmt, command_giveitem) ||
		command_add("givemoney", "[pp] [gp] [sp] [cp] - Gives specified amount of money to the target player.", EQ::constants::AccountStatus::GMMgmt, command_givemoney) ||
		command_add("globalview", "Lists all qglobals in cache if you were to do a quest with this target.", EQ::constants::AccountStatus::QuestTroupe, command_globalview) ||
		command_add("gm", "- Turn player target's or your GM flag on or off", EQ::constants::AccountStatus::QuestTroupe, command_gm) ||
		command_add("gmspeed", "[on/off] - Turn GM speed hack on/off for you or your player target", EQ::constants::AccountStatus::GMAdmin, command_gmspeed) ||
		command_add("gmzone", "[zone_short_name] [zone_version=0] [identifier=gmzone] - Zones to a private GM instance", EQ::constants::AccountStatus::GMAdmin, command_gmzone) ||
		command_add("goto", "[playername] or [x y z] [h] - Teleport to the provided coordinates or to your target", EQ::constants::AccountStatus::Steward, command_goto) ||
		command_add("grid", "[add/delete] [grid_num] [wandertype] [pausetype] - Create/delete a wandering grid", EQ::constants::AccountStatus::GMAreas, command_grid) ||
		command_add("guild", "- Guild manipulation commands. Use argument help for more info.", EQ::constants::AccountStatus::Steward, command_guild) ||
		command_add("guildapprove", "[guildapproveid] - Approve a guild with specified ID (guild creator receives the id)", EQ::constants::AccountStatus::Player, command_guildapprove) ||
		command_add("guildcreate", "[guildname] - Creates an approval setup for guild name specified", EQ::constants::AccountStatus::Player, command_guildcreate) ||
		command_add("guildlist", "[guildapproveid] - Lists character names who have approved the guild specified by the approve id", EQ::constants::AccountStatus::Player, command_guildlist) ||
		command_add("hair", "- Change the hair style of your target", EQ::constants::AccountStatus::QuestTroupe, command_hair) ||
		command_add("haircolor", "- Change the hair color of your target", EQ::constants::AccountStatus::QuestTroupe, command_haircolor) ||
		command_add("haste", "[percentage] - Set your haste percentage", EQ::constants::AccountStatus::GMAdmin, command_haste) ||
		command_add("hatelist", " - Display hate list for target.", EQ::constants::AccountStatus::QuestTroupe, command_hatelist) ||
		command_add("heal", "- Completely heal your target", EQ::constants::AccountStatus::Steward, command_heal) ||
		command_add("helm", "- Change the helm of your target", EQ::constants::AccountStatus::QuestTroupe, command_helm) ||
		command_add("help", "[search term] - List available commands and their description, specify partial command as argument to search", EQ::constants::AccountStatus::Player, command_help) ||
		command_add("heritage", "- Change the heritage of your target (Drakkin Only)", EQ::constants::AccountStatus::QuestTroupe, command_heritage) ||
		command_add("heromodel",  "[hero model] [slot] - Full set of Hero's Forge Armor appearance. If slot is set, sends exact model just to slot.", EQ::constants::AccountStatus::GMMgmt, command_heromodel) ||
		command_add("hideme", "[on/off] - Hide yourself from spawn lists.", EQ::constants::AccountStatus::QuestTroupe, command_hideme) ||
		command_add("hotfix", "[hotfix_name] - Reloads shared memory into a hotfix, equiv to load_shared_memory followed by apply_shared_memory", EQ::constants::AccountStatus::GMImpossible, command_hotfix) ||
		command_add("hp", "- Refresh your HP bar from the server.", EQ::constants::AccountStatus::Player, command_hp) ||
		command_add("incstat", "- Increases or Decreases a client's stats permanently.", EQ::constants::AccountStatus::GMMgmt, command_incstat) ||
		command_add("instance", "- Modify Instances", EQ::constants::AccountStatus::GMMgmt, command_instance) ||
		command_add("interrogateinv", "- use [help] argument for available options", EQ::constants::AccountStatus::Player, command_interrogateinv) ||
		command_add("interrupt", "[message id] [color] - Interrupt your casting. Arguments are optional.", EQ::constants::AccountStatus::Guide, command_interrupt) ||
		command_add("invsnapshot", "- Manipulates inventory snapshots for your current target", EQ::constants::AccountStatus::QuestTroupe, command_invsnapshot) ||
		command_add("invul", "[on/off] - Turn player target's or your invulnerable flag on or off", EQ::constants::AccountStatus::QuestTroupe, command_invul) ||
		command_add("ipban", "[IP address] - Ban IP by character name", EQ::constants::AccountStatus::GMMgmt, command_ipban) ||
		command_add("iplookup", "[charname] - Look up IP address of charname", EQ::constants::AccountStatus::GMMgmt, command_iplookup) ||
		command_add("iteminfo", "- Get information about the item on your cursor", EQ::constants::AccountStatus::Steward, command_iteminfo) ||
		command_add("itemsearch", "[search criteria] - Search for an item", EQ::constants::AccountStatus::Steward, command_itemsearch) ||
		command_add("kick", "[charname] - Disconnect charname", EQ::constants::AccountStatus::GMLeadAdmin, command_kick) ||
		command_add("kill", "- Kill your target", EQ::constants::AccountStatus::GMAdmin, command_kill) ||
		command_add("killallnpcs", " [npc_name] Kills all npcs by search name, leave blank for all attackable NPC's", EQ::constants::AccountStatus::GMMgmt, command_killallnpcs) ||
		command_add("lastname", "[new lastname] - Set your or your player target's lastname", EQ::constants::AccountStatus::Guide, command_lastname) ||
		command_add("level", "[level] - Set your or your target's level", EQ::constants::AccountStatus::Steward, command_level) ||
		command_add("list", "[npcs|players|corpses|doors|objects] [search] - Search entities", EQ::constants::AccountStatus::ApprenticeGuide, command_list) ||
		command_add("listpetition", "- List petitions", EQ::constants::AccountStatus::Guide, command_listpetition) ||
		command_add("load_shared_memory", "[shared_memory_name] - Reloads shared memory and uses the input as output", EQ::constants::AccountStatus::GMImpossible, command_load_shared_memory) ||
		command_add("loc", "- Print out your or your target's current location and heading", EQ::constants::AccountStatus::Player, command_loc) ||
		command_add("lock", "- Lock the worldserver", EQ::constants::AccountStatus::GMLeadAdmin, command_lock) ||
		command_add("logs",  "Manage anything to do with logs", EQ::constants::AccountStatus::GMImpossible, command_logs) ||
		command_add("makepet", "[level] [class] [race] [texture] - Make a pet", EQ::constants::AccountStatus::Guide, command_makepet) ||
		command_add("mana", "- Fill your or your target's mana", EQ::constants::AccountStatus::Guide, command_mana) ||
		command_add("maxskills", "Maxes skills for you.", EQ::constants::AccountStatus::GMMgmt, command_max_all_skills) ||
		command_add("memspell", "[slotid] [spellid] - Memorize spellid in the specified slot", EQ::constants::AccountStatus::Guide, command_memspell) ||
		command_add("merchant_close_shop",  "Closes a merchant shop", EQ::constants::AccountStatus::GMAdmin, command_merchantcloseshop) ||
		command_add("merchant_open_shop",  "Opens a merchants shop", EQ::constants::AccountStatus::GMAdmin, command_merchantopenshop) ||
		command_add("modifynpcstat", "- Modifys a NPC's stats", EQ::constants::AccountStatus::GMLeadAdmin, command_modifynpcstat) ||
		command_add("motd", "[new motd] - Set message of the day", EQ::constants::AccountStatus::GMLeadAdmin, command_motd) ||
		command_add("movechar", "[charname] [zonename] - Move charname to zonename", EQ::constants::AccountStatus::Guide, command_movechar) ||
		command_add("movement", "Various movement commands", EQ::constants::AccountStatus::GMMgmt, command_movement) ||
		command_add("myskills", "- Show details about your current skill levels", EQ::constants::AccountStatus::Player, command_myskills) ||
		command_add("mysql", "Mysql CLI, see 'help' for options.", EQ::constants::AccountStatus::GMImpossible, command_mysql) ||
		command_add("mystats", "- Show details about you or your pet", EQ::constants::AccountStatus::Guide, command_mystats) ||
		command_add("name", "[newname] - Rename your player target", EQ::constants::AccountStatus::GMLeadAdmin, command_name) ||
		command_add("netstats", "- Gets the network stats for a stream.", EQ::constants::AccountStatus::GMMgmt, command_netstats) ||
		command_add("network", "- Admin commands for the udp network interface.", EQ::constants::AccountStatus::GMImpossible, command_network) ||
		command_add("npccast", "[targetname/entityid] [spellid] - Causes NPC target to cast spellid on targetname/entityid", EQ::constants::AccountStatus::QuestTroupe, command_npccast) ||
		command_add("npcedit", "[column] [value] - Mega NPC editing command", EQ::constants::AccountStatus::GMAdmin, command_npcedit) ||
		command_add("npceditmass", "[name-search] [column] [value] - Mass (Zone wide) NPC data editing command", EQ::constants::AccountStatus::GMAdmin, command_npceditmass) ||
		command_add("npcemote", "[message] - Make your NPC target emote a message.", EQ::constants::AccountStatus::GMLeadAdmin, command_npcemote) ||
		command_add("npcloot", "[show/money/add/remove] [itemid/all/money: pp gp sp cp] - Manipulate the loot an NPC is carrying", EQ::constants::AccountStatus::QuestTroupe, command_npcloot) ||
		command_add("npcsay", "[message] - Make your NPC target say a message.", EQ::constants::AccountStatus::GMLeadAdmin, command_npcsay) ||
		command_add("npcshout", "[message] - Make your NPC target shout a message.", EQ::constants::AccountStatus::GMLeadAdmin, command_npcshout) ||
		command_add("npcspawn", "[create/add/update/remove/delete] - Manipulate spawn DB", EQ::constants::AccountStatus::GMAreas, command_npcspawn) ||
		command_add("npcspecialattk", "[flagchar] [perm] - Set NPC special attack flags. Flags are E(nrage) F(lurry) R(ampage) S(ummon).", EQ::constants::AccountStatus::QuestTroupe, command_npcspecialattk) ||
		command_add("npcstats", "- Show stats about target NPC", EQ::constants::AccountStatus::QuestTroupe, command_npcstats) ||
		command_add("npctype_cache",  "[id] or all - Clears the npc type cache for either the id or all npcs.", EQ::constants::AccountStatus::GMImpossible, command_npctype_cache) ||
		command_add("npctypespawn", "[npctypeid] [factionid] - Spawn an NPC from the db", EQ::constants::AccountStatus::Steward, command_npctypespawn) ||
		command_add("nudge", "- Nudge your target's current position by specific values", EQ::constants::AccountStatus::QuestTroupe, command_nudge) ||
		command_add("nukebuffs", "- Strip all buffs on you or your target", EQ::constants::AccountStatus::Guide, command_nukebuffs) ||
		command_add("nukeitem", "[itemid] - Remove itemid from your player target's inventory", EQ::constants::AccountStatus::GMLeadAdmin, command_nukeitem) ||
		command_add("object", "List|Add|Edit|Move|Rotate|Copy|Save|Undo|Delete - Manipulate static and tradeskill objects within the zone", EQ::constants::AccountStatus::GMAdmin, command_object) ||
		command_add("oocmute", "[1/0] - Mutes OOC chat", EQ::constants::AccountStatus::GMMgmt, command_oocmute) ||
		command_add("opcode", "- opcode management", EQ::constants::AccountStatus::GMImpossible, command_opcode) ||

#ifdef PACKET_PROFILER
		command_add("packetprofile", "- Dump packet profile for target or self.", EQ::constants::AccountStatus::GMImpossible, command_packetprofile) ||
#endif

		command_add("path", "- view and edit pathing", EQ::constants::AccountStatus::GMMgmt, command_path) ||
		command_add("peekinv", "[equip/gen/cursor/poss/limbo/curlim/trib/bank/shbank/allbank/trade/world/all] - Print out contents of your player target's inventory", EQ::constants::AccountStatus::GMAdmin, command_peekinv) ||
		command_add("peqzone", "[zonename] - Go to specified zone, if you have > 75% health", EQ::constants::AccountStatus::Player, command_peqzone) ||
		command_add("permaclass", "[classnum] - Change your or your player target's class (target is disconnected)", EQ::constants::AccountStatus::QuestTroupe, command_permaclass) ||
		command_add("permagender", "[gendernum] - Change your or your player target's gender (zone to take effect)", EQ::constants::AccountStatus::QuestTroupe, command_permagender) ||
		command_add("permarace", "[racenum] - Change your or your player target's race (zone to take effect)", EQ::constants::AccountStatus::QuestTroupe, command_permarace) ||
		command_add("petitioninfo", "[petition number] - Get info about a petition", EQ::constants::AccountStatus::ApprenticeGuide, command_petitioninfo) ||
		command_add("pf", "- Display additional mob coordinate and wandering data", EQ::constants::AccountStatus::Player, command_pf) ||
		command_add("picklock",  "Analog for ldon pick lock for the newer clients since we still don't have it working.", EQ::constants::AccountStatus::Player, command_picklock) ||
		command_add("profanity", "Manage censored language.", EQ::constants::AccountStatus::GMLeadAdmin, command_profanity) ||

#ifdef EQPROFILE
		command_add("profiledump", "- Dump profiling info to logs", EQ::constants::AccountStatus::GMImpossible, command_profiledump) ||
		command_add("profilereset", "- Reset profiling info", EQ::constants::AccountStatus::GMImpossible, command_profilereset) ||
#endif

		command_add("push", "Lets you do spell push", EQ::constants::AccountStatus::GMLeadAdmin, command_push) ||
		command_add("proximity", "Shows NPC proximity", EQ::constants::AccountStatus::GMLeadAdmin, command_proximity) ||
		command_add("pvp", "[on/off] - Set your or your player target's PVP status", EQ::constants::AccountStatus::GMAdmin, command_pvp) ||
		command_add("qglobal", "[on/off/view] - Toggles qglobal functionality on an NPC", EQ::constants::AccountStatus::GMAdmin, command_qglobal) ||
		command_add("questerrors", "Shows quest errors.", EQ::constants::AccountStatus::GMAdmin, command_questerrors) ||
		command_add("race", "[racenum] - Change your or your target's race. Use racenum 0 to return to normal", EQ::constants::AccountStatus::Guide, command_race) ||
		command_add("raidloot", "LEADER|GROUPLEADER|SELECTED|ALL - Sets your raid loot settings if you have permission to do so.", EQ::constants::AccountStatus::Player, command_raidloot) ||
		command_add("randomfeatures", "- Temporarily randomizes the Facial Features of your target", EQ::constants::AccountStatus::QuestTroupe, command_randomfeatures) ||
		command_add("refreshgroup", "- Refreshes Group.", EQ::constants::AccountStatus::Player, command_refreshgroup) ||
		command_add("reloadaa", "Reloads AA data", EQ::constants::AccountStatus::GMMgmt, command_reloadaa) ||
		command_add("reloadallrules", "Executes a reload of all rules.", EQ::constants::AccountStatus::QuestTroupe, command_reloadallrules) ||
		command_add("reloademote", "Reloads NPC Emotes", EQ::constants::AccountStatus::QuestTroupe, command_reloademote) ||
		command_add("reloadlevelmods", nullptr, EQ::constants::AccountStatus::Max, command_reloadlevelmods) ||
		command_add("reloadmerchants", nullptr, EQ::constants::AccountStatus::Max, command_reloadmerchants) ||
		command_add("reloadperlexportsettings", nullptr, EQ::constants::AccountStatus::Max, command_reloadperlexportsettings) ||
		command_add("reloadqst", " - Clear quest cache (any argument causes it to also stop all timers)", EQ::constants::AccountStatus::GMLeadAdmin, command_reloadqst) ||
		command_add("reloadrulesworld", "Executes a reload of all rules in world specifically.", EQ::constants::AccountStatus::QuestTroupe, command_reloadworldrules) ||
		command_add("reloadstatic", "- Reload Static Zone Data", EQ::constants::AccountStatus::GMLeadAdmin, command_reloadstatic) ||
		command_add("reloadtraps", "- Repops all traps in the current zone.", EQ::constants::AccountStatus::QuestTroupe, command_reloadtraps) ||
		command_add("reloadtitles", "- Reload player titles from the database", EQ::constants::AccountStatus::GMLeadAdmin, command_reloadtitles) ||
		command_add("reloadworld", "[0|1] - Clear quest cache (0 - no repop, 1 - repop)", EQ::constants::AccountStatus::Max, command_reloadworld) ||
		command_add("reloadzps", "- Reload zone points from database", EQ::constants::AccountStatus::GMLeadAdmin, command_reloadzps) ||
		command_add("repop", "[delay] - Repop the zone with optional delay", EQ::constants::AccountStatus::GMAdmin, command_repop) ||
		command_add("resetaa", "- Resets a Player's AA in their profile and refunds spent AA's to unspent, may disconnect player.", EQ::constants::AccountStatus::GMMgmt, command_resetaa) ||
		command_add("resetaa_timer", "Command to reset AA cooldown timers.", EQ::constants::AccountStatus::GMMgmt, command_resetaa_timer) ||
		command_add("resetdisc_timer", "Command to reset all discipline cooldown timers.", EQ::constants::AccountStatus::GMMgmt, command_resetdisc_timer) ||
		command_add("revoke", "[charname] [1/0] - Makes charname unable to talk on OOC", EQ::constants::AccountStatus::GMMgmt, command_revoke) ||
		command_add("roambox", "Manages roambox settings for an NPC", EQ::constants::AccountStatus::GMMgmt, command_roambox) ||
		command_add("rules", "(subcommand) - Manage server rules", EQ::constants::AccountStatus::GMImpossible, command_rules) ||
		command_add("save", "- Force your player or player corpse target to be saved to the database", EQ::constants::AccountStatus::Guide, command_save) ||
		command_add("scale", "- Handles npc scaling", EQ::constants::AccountStatus::GMLeadAdmin, command_scale) ||
		command_add("scribespell", "[spellid] - Scribe specified spell in your target's spell book.", EQ::constants::AccountStatus::GMCoder, command_scribespell) ||
		command_add("scribespells", "[max level] [min level] - Scribe all spells for you or your player target that are usable by them, up to level specified. (may freeze client for a few seconds)", EQ::constants::AccountStatus::GMLeadAdmin, command_scribespells) ||
		command_add("sendzonespawns", "- Refresh spawn list for all clients in zone", EQ::constants::AccountStatus::GMLeadAdmin, command_sendzonespawns) ||
		command_add("sensetrap",  "Analog for ldon sense trap for the newer clients since we still don't have it working.", EQ::constants::AccountStatus::Player, command_sensetrap) ||
		command_add("serverinfo", "- Get OS info about server host", EQ::constants::AccountStatus::GMMgmt, command_serverinfo) ||
		command_add("serverrules", "- Read this server's rules", EQ::constants::AccountStatus::Player, command_serverrules) ||
		command_add("setaapts", "[AA|Group|Raid] [AA Amount] - Set your or your player target's Available AA Points by Type", EQ::constants::AccountStatus::GMAdmin, command_setaapts) ||
		command_add("setaaxp", "[AA|Group|Raid] [AA Experience] - Set your or your player target's AA Experience by Type", EQ::constants::AccountStatus::GMAdmin, command_setaaxp) ||
		command_add("setadventurepoints", "- Set your or your player target's available adventure points", EQ::constants::AccountStatus::GMLeadAdmin, command_set_adventure_points) ||
		command_add("setanim", "[animnum] - Set target's appearance to animnum", EQ::constants::AccountStatus::GMMgmt, command_setanim) ||
		command_add("setcrystals", "[value] - Set your or your player target's available radiant or ebon crystals", EQ::constants::AccountStatus::GMAdmin, command_setcrystals) ||
		command_add("setfaction", "[faction number] - Sets targeted NPC's faction in the database", EQ::constants::AccountStatus::GMAreas, command_setfaction) ||
		command_add("setgraveyard", "[zone name] - Creates a graveyard for the specified zone based on your target's LOC.", EQ::constants::AccountStatus::GMMgmt, command_setgraveyard) ||
		command_add("setlanguage", "[language ID] [value] - Set your target's language skillnum to value", EQ::constants::AccountStatus::Guide, command_setlanguage) ||
		command_add("setlsinfo", "[email] [password] - Set login server email address and password (if supported by login server)", EQ::constants::AccountStatus::Steward, command_setlsinfo) ||
		command_add("setpass", "[accountname] [password] - Set local password for accountname", EQ::constants::AccountStatus::GMLeadAdmin, command_setpass) ||
		command_add("setpvppoints", "[Amount] - Set your or your player target's PVP points", EQ::constants::AccountStatus::GMAdmin, command_setpvppoints) ||
		command_add("setskill", "[skillnum] [value] - Set your target's skill skillnum to value", EQ::constants::AccountStatus::Guide, command_setskill) ||
		command_add("setskillall", "[value] - Set all of your target's skills to value", EQ::constants::AccountStatus::Guide, command_setskillall) ||
		command_add("setstartzone", "[zoneid] - Set target's starting zone. Set to zero to allow the player to use /setstartcity", EQ::constants::AccountStatus::QuestTroupe, command_setstartzone) ||
		command_add("setstat", "- Sets the stats to a specific value.", EQ::constants::AccountStatus::Max, command_setstat) ||
		command_add("setxp", "[value] - Set your or your player target's experience", EQ::constants::AccountStatus::GMAdmin, command_setxp) ||
		command_add("showbonusstats", "[item|spell|all] Shows bonus stats for target from items or spells. Shows both by default.", EQ::constants::AccountStatus::Guide, command_showbonusstats) ||
		command_add("showbuffs", "- List buffs active on your target or you if no target", EQ::constants::AccountStatus::Guide, command_showbuffs) ||
		command_add("shownumhits",  "Shows buffs numhits for yourself.", EQ::constants::AccountStatus::Player, command_shownumhits) ||
		command_add("shownpcgloballoot", "Show GlobalLoot entires on this npc", EQ::constants::AccountStatus::Guide, command_shownpcgloballoot) ||
		command_add("showskills", "- Show the values of your or your player target's skills", EQ::constants::AccountStatus::Guide, command_showskills) ||
		command_add("showspellslist", "Shows spell list of targeted NPC", EQ::constants::AccountStatus::GMAdmin, command_showspellslist) ||
		command_add("showstats", "- Show details about you or your target", EQ::constants::AccountStatus::Guide, command_showstats) ||
		command_add("showzonegloballoot", "Show GlobalLoot entires on this zone", EQ::constants::AccountStatus::Guide, command_showzonegloballoot) ||
		command_add("showzonepoints", "Show zone points for current zone", EQ::constants::AccountStatus::Guide, command_showzonepoints) ||
		command_add("shutdown", "- Shut this zone process down", EQ::constants::AccountStatus::GMLeadAdmin, command_shutdown) ||
		command_add("size", "[size] - Change size of you or your target", EQ::constants::AccountStatus::Guide, command_size) ||
		command_add("spawn", "[name] [race] [level] [material] [hp] [gender] [class] [priweapon] [secweapon] [merchantid] - Spawn an NPC", EQ::constants::AccountStatus::Steward, command_spawn) ||
		command_add("spawneditmass", "Mass editing spawn command", EQ::constants::AccountStatus::GMLeadAdmin, command_spawneditmass) ||
		command_add("spawnfix", "- Find targeted NPC in database based on its X/Y/heading and update the database to make it spawn at your current location/heading.", EQ::constants::AccountStatus::GMAreas, command_spawnfix) ||
		command_add("spawnstatus", "- Show respawn timer status", EQ::constants::AccountStatus::GMAdmin, command_spawnstatus) ||
		command_add("spellinfo", "[spellid] - Get detailed info about a spell", EQ::constants::AccountStatus::Steward, command_spellinfo) ||
		command_add("stun", "[duration] - Stuns you or your target for duration", EQ::constants::AccountStatus::GMAdmin, command_stun) ||
		command_add("summon", "[charname] - Summons your player/npc/corpse target, or charname if specified", EQ::constants::AccountStatus::QuestTroupe, command_summon) ||
		command_add("summonburiedplayercorpse", "- Summons the target's oldest buried corpse, if any exist.", EQ::constants::AccountStatus::GMAdmin, command_summonburiedplayercorpse) ||
		command_add("summonitem", "[itemid] [charges] - Summon an item onto your cursor. Charges are optional.", EQ::constants::AccountStatus::GMMgmt, command_summonitem) ||
		command_add("suspend", "[name] [days] [reason] - Suspend by character name and for specificed number of days", EQ::constants::AccountStatus::GMLeadAdmin, command_suspend) ||
		command_add("task", "(subcommand) - Task system commands", EQ::constants::AccountStatus::GMLeadAdmin, command_task) ||
		command_add("tattoo", "- Change the tattoo of your target (Drakkin Only)", EQ::constants::AccountStatus::QuestTroupe, command_tattoo) ||
		command_add("tempname", "[newname] - Temporarily renames your target. Leave name blank to restore the original name.", EQ::constants::AccountStatus::GMAdmin, command_tempname) ||
		command_add("petname", "[newname] - Temporarily renames your pet. Leave name blank to restore the original name.", EQ::constants::AccountStatus::GMAdmin, command_petname) ||
		command_add("texture", "[texture] [helmtexture] - Change your or your target's appearance, use 255 to show equipment", EQ::constants::AccountStatus::Steward, command_texture) ||
		command_add("time", "[HH] [MM] - Set EQ time", EQ::constants::AccountStatus::EQSupport, command_time) ||
		command_add("timers", "- Display persistent timers for target", EQ::constants::AccountStatus::GMMgmt, command_timers) ||
		command_add("timezone", "[HH] [MM] - Set timezone. Minutes are optional", EQ::constants::AccountStatus::EQSupport, command_timezone) ||
		command_add("title", "[text] [1 = create title table row] - Set your or your player target's title", EQ::constants::AccountStatus::Guide, command_title) ||
		command_add("titlesuffix", "[text] [1 = create title table row] - Set your or your player target's title suffix", EQ::constants::AccountStatus::Guide, command_titlesuffix) ||
		command_add("traindisc", "[level] - Trains all the disciplines usable by the target, up to level specified. (may freeze client for a few seconds)", EQ::constants::AccountStatus::GMLeadAdmin, command_traindisc) ||
		command_add("trapinfo", "- Gets infomation about the traps currently spawned in the zone.", EQ::constants::AccountStatus::QuestTroupe, command_trapinfo) ||
		command_add("tune",  "Calculate statistical values related to combat.", EQ::constants::AccountStatus::GMAdmin, command_tune) ||
		command_add("ucs", "- Attempts to reconnect to the UCS server", EQ::constants::AccountStatus::Player, command_ucs) ||
		command_add("undyeme", "- Remove dye from all of your armor slots", EQ::constants::AccountStatus::Player, command_undyeme) ||
		command_add("unfreeze", "- Unfreeze your target", EQ::constants::AccountStatus::QuestTroupe, command_unfreeze) ||
		command_add("unlock", "- Unlock the worldserver", EQ::constants::AccountStatus::GMLeadAdmin, command_unlock) ||
		command_add("unscribespell", "[spellid] - Unscribe specified spell from your target's spell book.", EQ::constants::AccountStatus::GMCoder, command_unscribespell) ||
		command_add("unscribespells", "- Clear out your or your player target's spell book.", EQ::constants::AccountStatus::GMCoder, command_unscribespells) ||
		command_add("untraindisc", "[spellid] - Untrain specified discipline from your target.", EQ::constants::AccountStatus::GMCoder, command_untraindisc) ||
		command_add("untraindiscs", "- Untrains all disciplines from your target.", EQ::constants::AccountStatus::GMCoder, command_untraindiscs) ||
		command_add("uptime", "[zone server id] - Get uptime of worldserver, or zone server if argument provided", EQ::constants::AccountStatus::Steward, command_uptime) ||
		command_add("version", "- Display current version of EQEmu server", EQ::constants::AccountStatus::Player, command_version) ||
		command_add("viewnpctype", "[NPC ID] - Show stats for an NPC by NPC ID", EQ::constants::AccountStatus::GMAdmin, command_viewnpctype) ||
		command_add("viewpetition", "[petition number] - View a petition", EQ::constants::AccountStatus::ApprenticeGuide, command_viewpetition) ||
		command_add("viewzoneloot", "[item id] - Allows you to search a zone's loot for a specific item ID. (0 shows all loot in the zone)", EQ::constants::AccountStatus::QuestTroupe, command_viewzoneloot) ||
		command_add("wc", "[wear slot] [material] - Sends an OP_WearChange for your target", EQ::constants::AccountStatus::GMMgmt, command_wc) ||
		command_add("weather", "[0/1/2/3] (Off/Rain/Snow/Manual) - Change the weather", EQ::constants::AccountStatus::QuestTroupe, command_weather) ||
		command_add("who", "[search]", EQ::constants::AccountStatus::ApprenticeGuide, command_who) ||
		command_add("worldshutdown", "- Shut down world and all zones", EQ::constants::AccountStatus::GMMgmt, command_worldshutdown) ||
		command_add("wp", "[add|delete] [grid_id] [pause] [waypoint_id] [-h] - Add or delete a waypoint by grid ID. (-h to use current heading)", EQ::constants::AccountStatus::GMAreas, command_wp) ||
		command_add("wpadd", "[pause] [-h] - Add your current location as a waypoint to your NPC target's AI path. (-h to use current heading)", EQ::constants::AccountStatus::GMAreas, command_wpadd) ||
		command_add("wpinfo", "- Show waypoint info about your NPC target", EQ::constants::AccountStatus::GMAreas, command_wpinfo) ||
		command_add("worldwide", "Performs world-wide GM functions such as cast (can be extended for other commands). Use caution", EQ::constants::AccountStatus::GMImpossible, command_worldwide) ||
		command_add("xtargets",  "Show your targets Extended Targets and optionally set how many xtargets they can have.", EQ::constants::AccountStatus::GMImpossible, command_xtargets) ||
		command_add("zclip", "[min] [max] - modifies and resends zhdr packet", EQ::constants::AccountStatus::QuestTroupe, command_zclip) ||
		command_add("zcolor", "[red] [green] [blue] - Change sky color", EQ::constants::AccountStatus::QuestTroupe, command_zcolor) ||
		command_add("zheader", "[zonename] - Load zheader for zonename from the database", EQ::constants::AccountStatus::QuestTroupe, command_zheader) ||
		command_add("zone", "[zonename] [x] [y] [z] - Go to specified zone (coords optional)", EQ::constants::AccountStatus::Guide, command_zone) ||
		command_add("zonebootup", "[ZoneServerID] [shortname] - Make a zone server boot a specific zone", EQ::constants::AccountStatus::GMLeadAdmin, command_zonebootup) ||
		command_add("zoneinstance", "[instanceid] [x] [y] [z] - Go to specified instance zone (coords optional)", EQ::constants::AccountStatus::Guide, command_zone_instance) ||
		command_add("zonelock", "[List|Lock|Unlock] [Zone ID|Zone Short Name] - Set or get lock status of a Zone by ID or Short Name", EQ::constants::AccountStatus::GMAdmin, command_zonelock) ||
		command_add("zoneshutdown", "[shortname] - Shut down a zone server", EQ::constants::AccountStatus::GMLeadAdmin, command_zoneshutdown) ||
		command_add("zonespawn", "- Not implemented", EQ::constants::AccountStatus::GMImpossible, command_zonespawn) ||
		command_add("zonestatus", "- Show connected zoneservers, synonymous with /servers", EQ::constants::AccountStatus::GMLeadAdmin, command_zonestatus) ||
		command_add("zopp",  "Troubleshooting command - Sends a fake item packet to you. No server reference is created.", EQ::constants::AccountStatus::GMImpossible, command_zopp) ||
		command_add("zsafecoords", "[x] [y] [z] - Set safe coords", EQ::constants::AccountStatus::QuestTroupe, command_zsafecoords) ||
		command_add("zsave", " - Saves zheader to the database", EQ::constants::AccountStatus::QuestTroupe, command_zsave) ||
		command_add("zsky", "[skytype] - Change zone sky type", EQ::constants::AccountStatus::QuestTroupe, command_zsky) ||
		command_add("zstats", "- Show info about zone header", EQ::constants::AccountStatus::QuestTroupe, command_zstats) ||
		command_add("zunderworld", "[zcoord] - Sets the underworld using zcoord", EQ::constants::AccountStatus::QuestTroupe, command_zunderworld) ||
		command_add("zuwcoords", "[z coord] - Set underworld coord", EQ::constants::AccountStatus::QuestTroupe, command_zuwcoords)
	) {
		command_deinit();
		return -1;
	}
...
void command_iteminfo(Client *c, const Seperator *sep)
{
	...
	if (c->Admin() >= EQ::constants::AccountStatus::GMMgmt)
