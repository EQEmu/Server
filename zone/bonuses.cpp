...
void Client::AddItemBonuses(const ItemInst *inst, StatBonuses* newbon, bool isAug, bool isTribute) {
	if(!inst || !inst->IsType(ItemClassCommon) || (inst->GetAugmentType() == 0 && isAug == true))
		return;
	...
	if (item->SkillModValue != 0 && item->SkillModType <= HIGHEST_SKILL) {
		if ((item->SkillModValue > 0 && newbon->skillmod[item->SkillModType] < item->SkillModValue) || (item->SkillModValue < 0 && newbon->skillmod[item->SkillModType] > item->SkillModValue))
		...
		for (i = 0; i < MAX_PP_AA_ARRAY; i++) {
			if (this->aa[i]) {
				...
				if (aa_AA > 0 || aa_value > 0) {
					...
					if (slots > 0)
						ApplyAABonuses(aa_AA, slots, newbon);
				}
			}
		}
	}
	...
	for (std::map<uint32, AA_Ability>::const_iterator iter = aa_effects[aaid].begin(); iter != aa_effects[aaid].end(); ++iter) {
		...
		switch (effect) {
			...
			case SE_SpellOnKill:
				for(int i = 0; i < (MAX_SPELL_TRIGGER * 3); i += 3) {
					if(!newbon->SpellOnKill[i] || ((newbon->SpellOnKill[i] == base2) && (newbon->SpellOnKill[i + 1] < base1))) {
						...
						if (GetLevel() > 15)
							newbon->SpellOnKill[i + 2] = (GetLevel() - 15);
						else
			...
			case SE_SpecialAttackKBProc:
				newbon->SpecialAttackKBProc[0] = base1;
				newbon->SpecialAttackKBProc[1] = base2;
				break;
			...
			case SE_SlayUndead:
				if(newbon->SlayUndead[1] < base1)
					newbon->SlayUndead[0] = base1;
					newbon->SlayUndead[1] = base2;
				break;
			...
			case SE_RaiseSkillCap:
				if(newbon->RaiseSkillCap[0] < base1){
					newbon->RaiseSkillCap[0] = base1;
					newbon->RaiseSkillCap[1] = base2;
				}
				break;
...
void Mob::CalcSpellBonuses(StatBonuses* newbon) {
	...
	newbon->AC = (newbon->AC * 10 / 34);
	if (GetClass() == BARD)
		newbon->ManaRegen = 0;
}

void Mob::ApplySpellsBonuses(uint16 spell_id, uint8 casterlevel, StatBonuses* newbon, uint16 casterId, bool item_bonus, uint32 ticsremaining, int buffslot, bool IsAISpellEffect, uint16 effect_id, int32 se_base, int32 se_limit, int32 se_max) {
		...
		switch (effectid) {
			...
			case SE_HitChance:
				if (RuleB(Spells, AdditiveBonusValues) && item_bonus){
					...
					else if (!newbon->HitChanceEffect[HIGHEST_SKILL + 1] || ((newbon->HitChanceEffect[HIGHEST_SKILL + 1] > 0) && (newbon->HitChanceEffect[HIGHEST_SKILL + 1] < effect_value)))
						newbon->HitChanceEffect[HIGHEST_SKILL + 1] = effect_value;
				}
					...
					if ((effect_value < 0) && (newbon->HitChanceEffect[base2] > effect_value))
						newbon->HitChanceEffect[base2] = effect_value;
					else if (!newbon->HitChanceEffect[base2] || ((newbon->HitChanceEffect[base2] > 0) && (newbon->HitChanceEffect[base2] < effect_value)))
						newbon->HitChanceEffect[base2] = effect_value;
				}
				break;
			case SE_DamageModifier:
			...
			case SE_DeathSave:
				if(newbon->DeathSave[0] < effect_value) {
					newbon->DeathSave[0] = effect_value;
					newbon->DeathSave[1] = buffslot;
					newbon->DeathSave[2] = base2;
					newbon->DeathSave[3] = max;
				}
				break;
			case SE_DivineSave:
				if (RuleB(Spells, AdditiveBonusValues) && item_bonus) {
			...
			case SE_Accuracy:
				...
				else if (!newbon->Accuracy[HIGHEST_SKILL + 1] || ((newbon->Accuracy[HIGHEST_SKILL + 1] > 0) && (newbon->Accuracy[HIGHEST_SKILL + 1] < effect_value)))
					newbon->Accuracy[HIGHEST_SKILL + 1] = effect_value;
				break;
			case SE_MaxHPChange:
			...
			case SE_SlayUndead:
				if(newbon->SlayUndead[1] < effect_value)
					newbon->SlayUndead[0] = effect_value;
					newbon->SlayUndead[1] = base2;
				break;
			...
			case SE_AlterNPCLevel:
				if (IsNPC()) {
					if (!newbon->AlterNPCLevel || ((effect_value < 0) && (newbon->AlterNPCLevel > effect_value)) || ((effect_value > 0) && (newbon->AlterNPCLevel < effect_value))) {	
	...
	if(newbon) {
		for(int i = 0; i < MAX_WORN_INVENTORY; i++) {
			...
			if(cur) {
				...
				if (cur->Worn.Effect>0 && (cur->Worn.Type == ET_WornEffect))
...
void Client::CalcItemScale() {
	...
	if(CalcItemScale(0, 21) || CalcItemScale(22, 30) || CalcItemScale(251, 341) || CalcItemScale(400, 405))
		changed = true;
...
void Client::DoItemEnterZone() {
	...
	if(DoItemEnterZone(0, 21) || DoItemEnterZone(22, 30) || DoItemEnterZone(251, 341) || DoItemEnterZone(400, 405))
		changed = true;
