			...
			if(LastGT != std::string::npos) {
				...
				if((*Iterator).find_first_of(" ") != std::string::npos) {
					...
					while(CurrentPos != std::string::npos) {
						...
						RecipientList.push_back(Secret + Recips.substr(CurrentPos, Space - CurrentPos));
						CurrentPos = Space + 1;
					}
					Iterator = RecipientList.begin();
					continue;
...
static void ProcessMailTo(Client *c, std::string MailMessage) {
	...
	for(unsigned int i=0; i<Recipients.size(); i++) {
		if(!database.SendMail(Recipients[i], c->MailBoxName(), Subject, Body, RecipientsString)) {
			_log(UCS__ERROR, "Failed in SendMail(%s, %s, %s, %s)", Recipients[i].c_str(), c->MailBoxName().c_str(), Subject.c_str(), RecipientsString.c_str());
	...
	if(Success) {
		...
		VARSTRUCT_ENCODE_STRING(PacketBuffer, "test");
		VARSTRUCT_ENCODE_STRING(PacketBuffer, "1");
		_pkt(UCS__PACKETS, outapp);
		c->QueuePacket(outapp);
		safe_delete(outapp);
...
void Clientlist::CheckForStaleConnections(Client *c) {
	...
	for(Iterator = ClientChatConnections.begin(); Iterator != ClientChatConnections.end(); ++Iterator) {
		if(((*Iterator) != c) && ((c->GetName() == (*Iterator)->GetName()) && (c->GetConnectionType() == (*Iterator)->GetConnectionType()))) {
			_log(UCS__CLIENT, "Removing old connection for %s", c->GetName().c_str());
			struct in_addr in;
			in.s_addr = (*Iterator)->ClientStream->GetRemoteIP();
			_log(UCS__CLIENT, "Client connection from %s:%d closed.", inet_ntoa(in), ntohs((*Iterator)->ClientStream->GetRemotePort()));
...
void Clientlist::Process() {
	...
	for(Iterator = ClientChatConnections.begin(); Iterator != ClientChatConnections.end(); ++Iterator) {
		...
		bool KeyValid = true;
		while(KeyValid && !(*Iterator)->GetForceDisconnect() && (app = (EQApplicationPacket *)(*Iterator)->ClientStream->PopPacket())) {
		...
		if(!KeyValid || (*Iterator)->GetForceDisconnect()) {
			struct in_addr in;
			in.s_addr = (*Iterator)->ClientStream->GetRemoteIP();
			_log(UCS__TRACE, "Force disconnecting client: %s:%d, KeyValid=%i, GetForceDisconnect()=%i", inet_ntoa(in), ntohs((*Iterator)->ClientStream->GetRemotePort()), KeyValid, (*Iterator)->GetForceDisconnect());
...
void Client::SendChannelMessage(std::string Message) {
	...
	if(RequiredChannel) {
		if(RuleB(Chat, EnableAntiSpam)) {
			if(!RequiredChannel->IsModerated() || RequiredChannel->HasVoice(GetName()) || RequiredChannel->IsOwner(GetName()) || RequiredChannel->IsModerator(GetName()) || IsChannelAdmin()) {
				if(GlobalChatLimiterTimer) {
...
void Client::SendChannelMessageByNumber(std::string Message) {
	...
	_log(UCS__TRACE, "%s tells %s, [%s]", GetName().c_str(), RequiredChannel->GetName().c_str(), Message.substr(MessageStart + 1).c_str());
	if(RuleB(Chat, EnableAntiSpam)) {
		if(!RequiredChannel->IsModerated() || RequiredChannel->HasVoice(GetName()) || RequiredChannel->IsOwner(GetName()) || RequiredChannel->IsModerator(GetName())) {
				if(GlobalChatLimiterTimer) {
					if(GlobalChatLimiterTimer->Check(false)) {
						GlobalChatLimiterTimer->Start(RuleI(Chat, IntervalDurationMS));
						AttemptedMessages = 0;
				...
				if(AttemptedMessages > AllowedMessages) {
					...
					if(GlobalChatLimiterTimer) {
						char TimeLeft[256];
						sprintf(TimeLeft, "You are currently rate limited, you cannot send more messages for %i seconds.", (GlobalChatLimiterTimer->GetRemainingTime() / 1000));
						GeneralChannelMessage(TimeLeft);
					}
					else
						GeneralChannelMessage("You are currently rate limited, you cannot send more messages for up to 60 seconds.");
...
std::string Client::MailBoxName() {
	if((Characters.size() == 0) || (CurrentMailBox > (Characters.size() - 1))) {
		_log(UCS__ERROR, "MailBoxName() called with CurrentMailBox set to %i and Characters.size() is %i", CurrentMailBox, Characters.size());

		return "";
	}
	_log(UCS__TRACE, "MailBoxName() called with CurrentMailBox set to %i and Characters.size() is %i", CurrentMailBox, Characters.size());
