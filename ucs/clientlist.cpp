...
static void ProcessMailTo(Client *c, std::string MailMessage) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "MAILTO: From %s, %s", c->MailBoxName().c_str(), MailMessage.c_str());
	...
	for (auto &Recipient : Recipients) {
		...
		if (!database.SendMail(Recipient, c->MailBoxName(), Subject, Body, RecipientsString)) {
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Failed in SendMail(%s, %s, %s, %s)", Recipient.c_str(),
...
static void ProcessCommandBuddy(Client *c, std::string Buddy) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Received buddy command with parameters %s", Buddy.c_str());
...
static void ProcessCommandIgnore(Client *c, std::string Ignoree) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Received ignore command with parameters %s", Ignoree.c_str());
...
Clientlist::Clientlist(int ChatPort) {
	...
	if (chatsf->Open())
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server,"Client (UDP) Chat listener started on port %i.", ChatPort);
	else {
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server,"Failed to start client (UDP) listener (port %-4i)", ChatPort);
...
void Clientlist::CheckForStaleConnections(Client *c) {
	...
	for(Iterator = ClientChatConnections.begin(); Iterator != ClientChatConnections.end(); ++Iterator) {
		...
		if(((*Iterator) != c) && ((c->GetName() == (*Iterator)->GetName())
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Removing old connection for %s", c->GetName().c_str());

			struct in_addr in;

			in.s_addr = (*Iterator)->ClientStream->GetRemoteIP();

			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Client connection from %s:%d closed.", inet_ntoa(in),
...
void Clientlist::Process() {
	...
	while((eqs = chatsf->Pop())) {
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "New Client UDP connection from %s:%d", inet_ntoa(in), ntohs(eqs->GetRemotePort()));
	...
	for(Iterator = ClientChatConnections.begin(); Iterator != ClientChatConnections.end(); ++Iterator) {
		...
		while( KeyValid && !(*Iterator)->GetForceDisconnect() &&
			...
			switch(opcode) {
				...
				case OP_MailLogin: {
					...
					{
						Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Mail key is the wrong size. Version of world incompatible with UCS.");
						KeyValid = false;
						break;
					}
					...
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Received login for user %s with key %s", MailBox, Key);

					if(!database.VerifyMailKey(CharacterName, (*Iterator)->ClientStream->GetRemoteIP(), Key)) {

						Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Chat Key for %s does not match, closing connection.", MailBox);
				...
				default: {
					...
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Unhandled chat opcode %8X", opcode);
					break;
				}
			}
			safe_delete(app);
		...
		if(!KeyValid || (*Iterator)->GetForceDisconnect()) {
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Force disconnecting client: %s:%d, KeyValid=%i, GetForceDisconnect()=%i",
...
void Clientlist::ProcessOPMailCommand(Client *c, std::string CommandString)
{
	...
	switch(CommandCode) {
		...
		case CommandSetMessageStatus:
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Set Message Status, Params: %s", Parameters.c_str());
		...
		default:
			c->SendHelp();
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Unhandled OP_Mail command: %s", CommandString.c_str());
	}
}
...
void Clientlist::CloseAllConnections() {
	...
	for(Iterator = ClientChatConnections.begin(); Iterator != ClientChatConnections.end(); ++Iterator) {
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Removing client %s", (*Iterator)->GetName().c_str());
...
void Client::AddCharacter(int CharID, const char *CharacterName, int Level) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Adding character %s with ID %i for %s", CharacterName, CharID, GetName().c_str());
...
void Client::AddToChannelList(ChatChannel *JoinedChannel) {
	...
	for(int i = 0; i < MAX_JOINED_CHANNELS; i++)
		if(JoinedChannels[i] == nullptr) {
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Added Channel %s to slot %i for %s", JoinedChannel->GetName().c_str(), i + 1, GetName().c_str());
			return;
		}
}
...
void Client::JoinChannels(std::string ChannelNameList) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Client: %s joining channels %s", GetName().c_str(), ChannelNameList.c_str());
...
void Client::LeaveChannels(std::string ChannelNameList) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Client: %s leaving channels %s", GetName().c_str(), ChannelNameList.c_str());
...
void Client::SendChannelMessage(std::string Message)
{
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "%s tells %s, [%s]", GetName().c_str(), ChannelName.c_str(), Message.substr(MessageStart + 1).c_str());
...
void Client::SendChannelMessageByNumber(std::string Message) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "%s tells %s, [%s]", GetName().c_str(), RequiredChannel->GetName().c_str(),
...
void Client::SetChannelPassword(std::string ChannelPassword) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Set password of channel [%s] to [%s] by %s", ChannelName.c_str(), Password.c_str(), GetName().c_str());
...
void Client::SetChannelOwner(std::string CommandString) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Set owner of channel [%s] to [%s]", ChannelName.c_str(), NewOwner.c_str());
...
void Client::ChannelInvite(std::string CommandString) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "[%s] invites [%s] to channel [%s]", GetName().c_str(), Invitee.c_str(), ChannelName.c_str());
...
void Client::ChannelGrantModerator(std::string CommandString) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "[%s] gives [%s] moderator rights to channel [%s]", GetName().c_str(), Moderator.c_str(), ChannelName.c_str());
...
void Client::ChannelGrantVoice(std::string CommandString) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "[%s] gives [%s] voice to channel [%s]", GetName().c_str(), Voicee.c_str(), ChannelName.c_str());
...
void Client::ChannelKick(std::string CommandString) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "[%s] kicks [%s] from channel [%s]", GetName().c_str(), Kickee.c_str(), ChannelName.c_str());
...
void Client::SetConnectionType(char c) {
	...
	switch(c)
	{
		case 'S':
		{
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Connection type is Combined (SoF/SoD)");
			break;
		}
		case 'U':
		{
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Connection type is Combined (Underfoot+)");
			break;
		}
		case 'M':
		{
			TypeOfConnection = ConnectionTypeMail;
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Connection type is Mail (6.2 or Titanium client)");
			break;
		}
		case 'C':
		{
			TypeOfConnection = ConnectionTypeChat;
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Connection type is Chat (6.2 or Titanium client)");
			break;
		}
		default:
		{
			TypeOfConnection = ConnectionTypeUnknown;
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Connection type is unknown.");
		}
	}
}
...
void Client::ChangeMailBox(int NewMailBox) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "%s Change to mailbox %i", MailBoxName().c_str(), NewMailBox);

	SetMailBox(NewMailBox);

	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "New mailbox is %s", MailBoxName().c_str());
...
std::string Client::MailBoxName() {
	...
	{
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "MailBoxName() called with CurrentMailBox set to %i and Characters.size() is %i",
