...
ChatChannel::ChatChannel(std::string inName, std::string inOwner, std::string inPassword, bool inPermanent, int inMinimumStatus) :
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "New ChatChannel created: Name: [%s], Owner: [%s], Password: [%s], MinStatus: %i",
...
void ChatChannelList::RemoveChannel(ChatChannel *Channel) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "RemoveChannel(%s)", Channel->GetName().c_str());
...
void ChatChannelList::RemoveAllChannels() {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "RemoveAllChannels");
...
void ChatChannel::AddClient(Client *c) {
	...
	if(IsClientInChannel(c)) {
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Client %s already in channel %s", c->GetName().c_str(), GetName().c_str());
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Adding %s to channel %s", c->GetName().c_str(), Name.c_str());
...
bool ChatChannel::RemoveClient(Client *c) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "RemoveClient %s from channel %s", c->GetName().c_str(), GetName().c_str());
	...
	if((PlayersInChannel == 0) && !Permanent) {
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Starting delete timer for empty password protected channel %s", Name.c_str());
...
void ChatChannel::SendMessageToChannel(std::string Message, Client* Sender) {
	...
	while(iterator.MoreElements()) {
		...
		if(ChannelClient)
		{
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Sending message to %s from %s",
...
ChatChannel *ChatChannelList::AddClientToChannel(std::string ChannelName, Client *c) {
	...
	Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "AddClient to channel [%s] with password [%s]", NormalisedName.c_str(), Password.c_str());
...
void ChatChannelList::Process() {
	...
	while(iterator.MoreElements()) {
		...
		if(CurrentChannel && CurrentChannel->ReadyToDelete()) {
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Empty temporary password protected channel %s being destroyed.",
...
void ChatChannel::AddInvitee(std::string Invitee) {
	...
	if(!IsInvitee(Invitee)) {
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Added %s as invitee to channel %s", Invitee.c_str(), Name.c_str());
	}
...
void ChatChannel::RemoveInvitee(std::string Invitee) {
	...
	for(Iterator = Invitees.begin(); Iterator != Invitees.end(); ++Iterator) {
		...
		if((*Iterator) == Invitee) {
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Removed %s as invitee to channel %s", Invitee.c_str(), Name.c_str());
...
void ChatChannel::AddModerator(std::string Moderator) {
	...
	if(!IsModerator(Moderator)) {
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Added %s as moderator to channel %s", Moderator.c_str(), Name.c_str());
	}
...
void ChatChannel::RemoveModerator(std::string Moderator) {
	...
	for(Iterator = Moderators.begin(); Iterator != Moderators.end(); ++Iterator) {
		...
		if((*Iterator) == Moderator) {
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Removed %s as moderator to channel %s", Moderator.c_str(), Name.c_str());
...
void ChatChannel::AddVoice(std::string inVoiced) {
	...
	if(!HasVoice(inVoiced)) {
		...
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Added %s as voiced to channel %s", inVoiced.c_str(), Name.c_str());
	}
...
void ChatChannel::RemoveVoice(std::string inVoiced) {
	...
	for(Iterator = Voiced.begin(); Iterator != Voiced.end(); ++Iterator) {
		...
		if((*Iterator) == inVoiced) {
			...
			Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::UCS_Server, "Removed %s as voiced to channel %s", inVoiced.c_str(), Name.c_str());
