...
void ClientList::Process() {
	...
	while(iterator.MoreElements()) {
		if (!iterator.GetData()->Process()) {
			...
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Removing client from %s:%d", inet_ntoa(in), iterator.GetData()->GetPort());
...
void ClientList::SendOnlineGuildMembers(uint32 FromID, uint32 GuildID)
{
	...
	if(!from)
	{
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Invalid client. FromID=%i GuildID=%i", FromID, GuildID);
		return;
	}
...
void ClientList::SendWhoAll(uint32 fromid,const char* to, int16 admin, Who_All_Struct* whom, WorldTCPConnection* connection) {
	...
	safe_delete(output);
		...
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Unknown error in world's SendWhoAll (probably mem error), ignoring...");
		return;
	}
}
...
void ClientList::SendFriendsWho(ServerFriendsWho_Struct *FriendsWho, WorldTCPConnection* connection) {
	...
	try{
		...
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Unknown error in world's SendFriendsWho (probably mem error), ignoring...");
		return;
	}
}
...
Client* ClientList::FindByAccountID(uint32 account_id) {
	...
	iterator.Reset();
	while(iterator.MoreElements()) {
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "ClientList[0x%08x]::FindByAccountID(%p) iterator.GetData()[%p]", this, account_id, iterator.GetData());
...
Client* ClientList::FindByName(char* charname) {
	...
	iterator.Reset();
	while(iterator.MoreElements()) {
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "ClientList[0x%08x]::FindByName(\"%s\") iterator.GetData()[%p]", this, charname, iterator.GetData());
