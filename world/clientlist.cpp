...
void ClientList::SendWhoAll(uint32 fromid,const char* to, int16 admin, Who_All_Struct* whom, WorldTCPConnection* connection) {
	...
	(whom == 0 || (
		((countcle->Admin() >= EQ::constants::AccountStatus::QuestTroupe && countcle->GetGM()) || whom->gmlookup == 0xFFFF) &&
		...
		(whomlen == 0 || (
			...
			(admin >= EQ::constants::AccountStatus::GMAdmin && strncasecmp(countcle->AccountName(), whom->whom, whomlen) == 0)
		))
	))
) {
			if((countcle->Anon()>0 && admin >= countcle->Admin() && admin > EQ::constants::AccountStatus::Player) || countcle->Anon()==0 ){
				totalusers++;
				if(totalusers<=20 || admin >= EQ::constants::AccountStatus::GMAdmin)
	...
	(whom == 0 || (
		((cle->Admin() >= EQ::constants::AccountStatus::QuestTroupe && cle->GetGM()) || whom->gmlookup == 0xFFFF) &&
		...
		(whomlen == 0 || (
			...
			(admin >= EQ::constants::AccountStatus::GMAdmin && strncasecmp(cle->AccountName(), whom->whom, whomlen) == 0)
		))
	))
) {
			line[0] = 0;
			uint32 rankstring = 0xFFFFFFFF;
				if((cle->Anon()==1 && cle->GetGM() && cle->Admin()>admin) || (idx>=20 && admin < EQ::constants::AccountStatus::GMAdmin)){ //hide gms that are anon from lesser gms and normal players, cut off at 20
					rankstring = 0;
					iterator.Advance();
					continue;
				} else if (cle->GetGM()) {
					if (cle->Admin() >= EQ::constants::AccountStatus::GMImpossible)
						rankstring = 5021;
					else if (cle->Admin() >= EQ::constants::AccountStatus::GMMgmt)
						rankstring = 5020;
					else if (cle->Admin() >= EQ::constants::AccountStatus::GMCoder)
						rankstring = 5019;
					else if (cle->Admin() >= EQ::constants::AccountStatus::GMAreas)
						rankstring = 5018;
					else if (cle->Admin() >= EQ::constants::AccountStatus::QuestMaster)
						rankstring = 5017;
					else if (cle->Admin() >= EQ::constants::AccountStatus::GMLeadAdmin)
						rankstring = 5016;
					else if (cle->Admin() >= EQ::constants::AccountStatus::GMAdmin)
						rankstring = 5015;
					else if (cle->Admin() >= EQ::constants::AccountStatus::GMStaff)
						rankstring = 5014;
					else if (cle->Admin() >= EQ::constants::AccountStatus::EQSupport)
						rankstring = 5013;
					else if (cle->Admin() >= EQ::constants::AccountStatus::GMTester)
						rankstring = 5012;
					else if (cle->Admin() >= EQ::constants::AccountStatus::SeniorGuide)
						rankstring = 5011;
					else if (cle->Admin() >= EQ::constants::AccountStatus::QuestTroupe)
						rankstring = 5010;
					else if (cle->Admin() >= EQ::constants::AccountStatus::Guide)
						rankstring = 5009;
					else if (cle->Admin() >= EQ::constants::AccountStatus::ApprenticeGuide)
						rankstring = 5008;
					else if (cle->Admin() >= EQ::constants::AccountStatus::Steward)
						rankstring = 5007;
				}
			idx++;
			...
			if(cle->Anon()==1 && (admin<cle->Admin() || admin == EQ::constants::AccountStatus::Player))
				formatstring=5024;
			else if(cle->Anon()==1 && admin>=cle->Admin() && admin > EQ::constants::AccountStatus::Player)
				formatstring=5022;
			else if(cle->Anon()==2 && (admin<cle->Admin() || admin == EQ::constants::AccountStatus::Player))
	...
	uint32 plzone=0;
	uint32 unknown80[2];
	if(cle->Anon()==0 || (admin>=cle->Admin() && admin> EQ::constants::AccountStatus::Player)){
		plclass_=cle->class_();
		pllevel=cle->level();
	...
	if(admin>=cle->Admin() && admin > EQ::constants::AccountStatus::Player)
...
void ClientList::ConsoleSendWhoAll(const char* to, int16 admin, Who_All_Struct* whom, WorldTCPConnection* connection) {
	...
	while (iterator.MoreElements()) {
		...
		if (
			...
			if (admin >= EQ::constants::AccountStatus::GMLeadAdmin && admin >= cle->Admin()) {
			...
			if (cle->Anon() == 2) { // Roleplay
				if (admin >= EQ::constants::AccountStatus::GMAdmin && admin >= cle->Admin())
				...
				else if (cle->Admin() >= EQ::constants::AccountStatus::QuestTroupe && admin < EQ::constants::AccountStatus::QuestTroupe && cle->GetGM()) {
					iterator.Advance();
					continue;
				}
				else
				...
				else if (cle->Admin() >= EQ::constants::AccountStatus::QuestTroupe && cle->GetGM()) {
					iterator.Advance();
					continue;
				}
				else
			...
			x++;
			if (x >= 20 && admin < EQ::constants::AccountStatus::QuestTroupe)
				break;
		}
		iterator.Advance();
	}
	...
	if (admin >= EQ::constants::AccountStatus::GMAdmin && (whom == 0 || whom->gmlookup != 0xFFFF)) {
		if (connection->IsConsole())
			fmt::format_to(out, "\r\n");
		else
			fmt::format_to(out, "\n");
