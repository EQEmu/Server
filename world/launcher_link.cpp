...
bool LauncherLink::Process() {
	...
	while((pack = tcpc->PopPacket())) {
			...
			if (WorldConfig::get()->SharedKey.length() > 0) {
				if (pack->opcode == ServerOP_ZAAuth && pack->size == 16) {
					...
					if (memcmp(pack->pBuffer, tmppass, 16) == 0)
						...
						struct in_addr in;
						in.s_addr = GetIP();
						Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Launcher authorization failed.");
		...
		case ServerOP_ZAAuth: {
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Got authentication from %s when they are already authenticated.", m_name.c_str());
			break;
		}
		case ServerOP_LauncherConnectInfo: {
			const LauncherConnectInfo *it = (const LauncherConnectInfo *) pack->pBuffer;
			if(HasName()) {
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Launcher '%s' received an additional connect packet with name '%s'. Ignoring.", m_name.c_str(), it->name);
				break;
			}
			m_name = it->name;
			...
			if(config == nullptr) {
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Unknown launcher '%s' connected. Disconnecting.", it->name);
				Disconnect();
				break;
			}

			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Launcher Identified itself as '%s'. Loading zone list.", it->name);
			...
			for(; cur != end; cur++) {
				...
				zs.up = false;
				zs.starts = 0;
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "%s: Loaded zone '%s' on port %d", m_name.c_str(), cur->name.c_str(), zs.port);
		...
		case ServerOP_LauncherZoneStatus: {
			...
			if(res == m_states.end()) {
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "%s: reported state for zone %s which it does not have.", m_name.c_str(), it->short_name);
				break;
			}
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "%s: %s reported state %s (%d starts)", m_name.c_str(), it->short_name, it->running?"STARTED":"STOPPED", it->start_count);
		...
		default:
		{
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Unknown ServerOPcode from launcher 0x%04x, size %d",pack->opcode,pack->size);
...
void LauncherLink::BootZone(const char *short_name, uint16 port) {
	...
	zs.up = false;
	zs.starts = 0;
	Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "%s: Loaded zone '%s' on port %d", m_name.c_str(), short_name, zs.port);
