...
bool ZoneServer::SetZone(uint32 iZoneID, uint32 iInstanceID, bool iStaticZone) {
	...
	if (iZoneID)
		Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Setting to '%s' (%d:%d)%s",(zn) ? zn : "",iZoneID, iInstanceID,
			iStaticZone ? " (Static)" : "");
...
bool ZoneServer::Process() {
	...
	while((pack = tcpc->PopPacket())) {
		...
		if (!authenticated) {
			if (WorldConfig::get()->SharedKey.length() > 0) {
				if (pack->opcode == ServerOP_ZAAuth && pack->size == 16) {
					...
					if (memcmp(pack->pBuffer, tmppass, 16) == 0)
						...
						struct in_addr in;
						in.s_addr = GetIP();
						Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Zone authorization failed.");
			...
			{
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"**WARNING** You have not configured a world shared key in your config file. You should add a <key>STRING</key> element to your <world> element to prevent unauthroized zone access.");
				authenticated = true;
			}
		}
		switch(pack->opcode) {
			...
			case ServerOP_RezzPlayer: {
				...
				if (zoneserver_list.SendPacket(pack)){
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Sent Rez packet for %s",sRezz->rez.your_name);
				}
				else {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Could not send Rez packet for %s",sRezz->rez.your_name);
				}
				break;
			}
			...
			case ServerOP_SetConnectInfo: {
				...
				if (!sci->port) {
					...
					sci->port = clientport;
					SendPacket(&p);
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Auto zone port configuration. Telling zone to use port %d",clientport);
				} else {
					clientport=sci->port;
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Zone specified port %d, must be a previously allocated zone reconnecting.",clientport);
				}
			...
			case ServerOP_SetLaunchName: {
				...
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Zone started with name %s by launcher %s", launched_name.c_str(), launcher_name.c_str());
				break;
			}
			...
			case ServerOP_ZoneToZoneRequest: {
				...
				if(GetZoneID() == ztz->current_zone_id && GetInstanceID() == ztz->current_instance_id) {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Processing ZTZ for egress from zone for client %s\n", ztz->name);
					...
					if(ingress_server) {
						Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Found a zone already booted for %s\n", ztz->name);
						ztz->response = 1;
					}
					/* Boot the Zone*/
					else {
						...
						if ((server_id = zoneserver_list.TriggerBootup(ztz->requested_zone_id, ztz->requested_instance_id))){
							Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Successfully booted a zone for %s\n", ztz->name);
							// bootup successful, ready to rock
							ztz->response = 1;
							ingress_server = zoneserver_list.FindByID(server_id);
						}
						else {
							Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"FAILED to boot a zone for %s\n", ztz->name);
				...
				/* Response from Ingress server, route back to egress */
					...
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Processing ZTZ for ingress to zone for client %s\n", ztz->name);
			...
			case ServerOP_ClientList: {
				if (pack->size != sizeof(ServerClientList_Struct)) {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Wrong size on ServerOP_ClientList. Got: %d, Expected: %d",pack->size,sizeof(ServerClientList_Struct));
					break;
				}
			...
			case ServerOP_ClientListKA: {
				...
				if (pack->size < 4 || pack->size != 4 + (4 * sclka->numupdates)) {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Wrong size on ServerOP_ClientListKA. Got: %d, Expected: %d",pack->size, (4 + (4 * sclka->numupdates)));
					break;
				}
			...
			case ServerOP_GMGoto: {
				if (pack->size != sizeof(ServerGMGoto_Struct)) {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Wrong size on ServerOP_GMGoto. Got: %d, Expected: %d",pack->size,sizeof(ServerGMGoto_Struct));
					break;
				}
			...
			case ServerOP_Lock: {
				if (pack->size != sizeof(ServerLock_Struct)) {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Wrong size on ServerOP_Lock. Got: %d, Expected: %d",pack->size,sizeof(ServerLock_Struct));
					break;
				}
			...
			case ServerOP_Motd: {
				if (pack->size != sizeof(ServerMotd_Struct)) {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Wrong size on ServerOP_Motd. Got: %d, Expected: %d",pack->size,sizeof(ServerMotd_Struct));
					break;
				}
			...
			case ServerOP_Uptime: {
				if (pack->size != sizeof(ServerUptime_Struct)) {
					Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Wrong size on ServerOP_Uptime. Got: %d, Expected: %d",pack->size,sizeof(ServerUptime_Struct));
					break;
				}
			...
			case ServerOP_GetWorldTime: {
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Broadcasting a world time update");
				auto pack = new ServerPacket;
			...
			case ServerOP_SpawnPlayerCorpse: {
				...
				if(zs) {
					if (zs->SendPacket(pack)) {
						Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Sent request to spawn player corpse id %i in zone %u.",s->player_corpse_id, s->zone_id);
					}
					else {
						Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Could not send request to spawn player corpse id %i in zone %u.",s->player_corpse_id, s->zone_id);
					}
				}
				break;
			}
			case ServerOP_Consent: {
				...
				if(cle) {
					if(cle->instance() != 0)
					{
						...
						if(zs) {
							if(zs->SendPacket(pack)) {
								Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Sent consent packet from player %s to player %s in zone %u.", s->ownername, s->grantname, cle->instance());
							}
							else {
								Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Unable to locate zone record for instance id %u in zoneserver list for ServerOP_Consent operation.", s->instance_id);
							}
						}
						else
						{
							...
							if(zs) {
								if(!zs->SendPacket(pack))
									Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Unable to send consent response back to player %s in instance %u.", s->ownername, zs->GetInstanceID());
							}
							else {
								Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Unable to locate zone record for instance id %u in zoneserver list for ServerOP_Consent_Response operation.", s->instance_id);
							}
						}
					}
					else
					{
						...
						if(zs) {
							if(zs->SendPacket(pack)) {
								Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Sent consent packet from player %s to player %s in zone %u.", s->ownername, s->grantname, cle->zone());
							}
							else {
								Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Unable to locate zone record for zone id %u in zoneserver list for ServerOP_Consent operation.", s->zone_id);
							}
						}
						else {
							...
							if(zs) {
								if(!zs->SendPacket(pack))
									Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Unable to send consent response back to player %s in zone %s.", s->ownername, zs->GetZoneName());
							}
							else {
								Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Unable to locate zone record for zone id %u in zoneserver list for ServerOP_Consent_Response operation.", s->zone_id);
							}
						}
					}
				}
			...
			{
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Received ServerOP_LSAccountUpdate packet from zone");
			...
			default:
			{
				Log.Out(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Unknown ServerOPcode from zone 0x%04x, size %d",pack->opcode,pack->size);
