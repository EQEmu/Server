...
bool Client::HandleSendLoginInfoPacket(const EQApplicationPacket *app) {
...
#endif
	{
		if (cle->AccountID() == 0 || (!minilogin && cle->LSID()==0)) {
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"ID is 0. Is this server connected to minilogin?");
			if(!minilogin)
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"If so you forget the minilogin variable...");
			else
				Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Could not find a minilogin account, verify ip address logging into minilogin is the same that is in your account table.");
			return false;
		}
		...
		if(minilogin){
			...
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"MiniLogin Account #%d",cle->AccountID());
		}
		else {
...
bool Client::HandleEnterWorldPacket(const EQApplicationPacket *app) { 
	...
	if(!pZoning && (RuleB(World, EnableTutorialButton) && (ew->tutorial || StartInTutorial))) {
		...
		{
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"'%s' is trying to go to tutorial but are not allowed...",char_name);
...
bool Client::OPCharCreate(char *name, CharCreate_Struct *cc)
{
	...
	Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server, "Race: %d  Class: %d  Gender: %d  Deity: %d  Start zone: %d  Tutorial: %s",
...
//	strcpy(pp.servername, WorldConfig::get()->ShortName.c_str());
	...
	if (ClientVersionBit & BIT_SoFAndLater && RuleI(World, SoFStartZoneID) > 0) {
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Found 'SoFStartZoneID' rule setting: %i", RuleI(World, SoFStartZoneID));
		pp.zone_id = RuleI(World, SoFStartZoneID);
		if (pp.zone_id)
			database.GetSafePoints(pp.zone_id, 0, &pp.x, &pp.y, &pp.z);
		else
			Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Error getting zone id for Zone ID %i", RuleI(World, SoFStartZoneID));
	} else {
	...
	Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Current location: %s (%d)  %0.2f, %0.2f, %0.2f, %0.2f",
		database.GetZoneName(pp.zone_id), pp.zone_id, pp.x, pp.y, pp.z, pp.heading);
	Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Bind location: %s (%d) %0.2f, %0.2f, %0.2f",
		database.GetZoneName(pp.binds[0].zoneId), pp.binds[0].zoneId,  pp.binds[0].x, pp.binds[0].y, pp.binds[0].z);
	Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Home location: %s (%d) %0.2f, %0.2f, %0.2f",
...
#define _TABLE_RACES 16
	...
	if (classtemp >= PLAYER_CLASS_COUNT) {
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"  class is out of range");
		return false;
	}
	if (racetemp >= _TABLE_RACES) {
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"  race is out of range");
		return false;
	}
	...
	if (bTOTAL + stat_points != cTOTAL) {
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"  stat points total doesn't match expected value: expecting %d got %d", bTOTAL + stat_points, cTOTAL);
		Charerrors++;
	}

	if (cc->STR > bSTR + stat_points || cc->STR < bSTR) {
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"  stat STR is out of range");
		Charerrors++;
	}
	if (cc->STA > bSTA + stat_points || cc->STA < bSTA) {
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"  stat STA is out of range");
		Charerrors++;
	}
	if (cc->AGI > bAGI + stat_points || cc->AGI < bAGI) {
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"  stat AGI is out of range");
		Charerrors++;
	}
	if (cc->DEX > bDEX + stat_points || cc->DEX < bDEX) {
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"  stat DEX is out of range");
		Charerrors++;
	}
	if (cc->WIS > bWIS + stat_points || cc->WIS < bWIS) {
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"  stat WIS is out of range");
		Charerrors++;
	}
	if (cc->INT > bINT + stat_points || cc->INT < bINT) {
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"  stat INT is out of range");
		Charerrors++;
	}
	if (cc->CHA > bCHA + stat_points || cc->CHA < bCHA) {
		Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"  stat CHA is out of range");
		Charerrors++;
	}
	...
	Log.DoLog(EQEmuLogSys::Detail, EQEmuLogSys::World_Server,"Found %d errors in character creation request", Charerrors);
