...
// SCORPIOUS2K - reversed parameter order for flag
			...
			else if (strcasecmp(sep.arg[0], "serverinfo") == 0 && admin >= EQ::constants::AccountStatus::GMMgmt) {
			...
			else if (strcasecmp(sep.arg[0], "IPLookup") == 0 && admin >= EQ::constants::AccountStatus::GMMgmt) {
				client_list.SendCLEList(admin, 0, this, sep.argplus[1]);
			}
			else if (strcasecmp(sep.arg[0], "LSReconnect") == 0 && admin >= EQ::constants::AccountStatus::GMAdmin) {
				#ifdef _WINDOWS
				...
				else if (strcasecmp(sep.arg[1], "lock") == 0 && admin >= EQ::constants::AccountStatus::GMAdmin) {
				...
				else if (strcasecmp(sep.arg[1], "unlock") == 0 && admin >= EQ::constants::AccountStatus::GMAdmin) {
					uint16 tmp = ZoneID(sep.arg[2]);
					if (tmp) {
			...
			else if (strcasecmp(sep.arg[0], "reloadworld") == 0 && admin > EQ::constants::AccountStatus::GMAdmin)
			{
