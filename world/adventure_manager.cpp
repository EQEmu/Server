...
bool AdventureManager::LoadAdventureTemplates()
{
    ...
    if (!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in AdventureManager:::LoadAdventures: %s (%s)", query.c_str(), results.ErrorMessage().c_str());
		return false;
    }
...
bool AdventureManager::LoadAdventureEntries()
{
    ...
    if (!results.Success())
	{
		Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in AdventureManager:::LoadAdventureEntries: %s (%s)", query.c_str(), results.ErrorMessage().c_str());
		return false;
	}
...
void AdventureManager::LoadLeaderboardInfo()
{
    ...
    auto results = database.QueryDatabase(query);
	if(!results.Success()) {
        Log.Out(EQEmuLogSys::General, EQEmuLogSys::Error, "Error in AdventureManager:::GetLeaderboardInfo: %s (%s)", query.c_str(), results.ErrorMessage().c_str());
		return;
	}
