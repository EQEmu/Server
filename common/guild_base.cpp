...
bool BaseGuildManager::RefreshGuild(uint32 guild_id) {
	...
	if (!results.Success())
	{
		Log.Out(Logs::Detail, Logs::Guilds, "Error reloading guilds '%s': %s", query.c_str(), results.ErrorMessage().c_str());
		return false;
	}
...
bool BaseGuildManager::DBSetGuildURL(uint32 GuildID, const char* URL)
{
	...
	if (!results.Success())
	{
		Log.Out(Logs::Detail, Logs::Guilds, "Error setting URL for guild %d '%s': %s", GuildID, query.c_str(), results.ErrorMessage().c_str());
		safe_delete_array(esc);
		return(false);
	}
	safe_delete_array(esc);
...
bool BaseGuildManager::DBSetGuild(uint32 charid, uint32 guild_id, uint8 rank) {
	...
	if(guild_id != GUILD_NONE) {
		...
		if (!results.Success()) {
			Log.Out(Logs::Detail, Logs::Guilds, "Error Changing char %d to guild %d '%s': %s", charid, guild_id, query.c_str(), results.ErrorMessage().c_str());
			return false;
		}
