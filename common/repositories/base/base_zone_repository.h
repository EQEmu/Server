...
public:
	struct Zone {
		...
		int32_t     max_movement_update_range;
	};
	...
	)
	{
		...
		if (results.RowCount() == 1) {
			...
			e.max_movement_update_range = row[100] ? static_cast<int32_t>(atoi(row[100])) : 600;
	...
	)
	{
		...
		v.push_back(columns[100] + " = " + std::to_string(e.max_movement_update_range));
