...
public:
	...
	)
	{
		...
		if (results.RowCount() == 1) {
			...
			e.quantity  = row[3] ? static_cast<uint32_t>(strtoul(row[3], nullptr, 10)) : 0;
			e.to_name   = row[4] ? row[4] : "";
	...
	)
	{
		...
		v.push_back(columns[3] + " = " + std::to_string(e.quantity));
		v.push_back(columns[4] + " = '" + Strings::Escape(e.to_name) + "'");
	...
	)
	{
		...
		v.push_back("'" + Strings::Escape(e.to_name) + "'");
