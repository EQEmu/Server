...
public:
	...
	)
	{
		...
		if (results.RowCount() == 1) {
			...
			entry.faction_amount         = atoi(row[23]);
			entry.adventure_template_id  = atoi(row[24]);
			entry.trap_template          = atoi(row[25]);
			entry.mindmg                 = atoi(row[26]);
			entry.maxdmg                 = atoi(row[27]);
			entry.attack_count           = atoi(row[28]);
			entry.npcspecialattks        = row[29] ? row[29] : "";
			entry.special_abilities      = row[30] ? row[30] : "";
			entry.aggroradius            = atoi(row[31]);
			entry.assistradius           = atoi(row[32]);
			entry.face                   = atoi(row[33]);
			entry.luclin_hairstyle       = atoi(row[34]);
			entry.luclin_haircolor       = atoi(row[35]);
			entry.luclin_eyecolor        = atoi(row[36]);
			entry.luclin_eyecolor2       = atoi(row[37]);
			entry.luclin_beardcolor      = atoi(row[38]);
			entry.luclin_beard           = atoi(row[39]);
			entry.drakkin_heritage       = atoi(row[40]);
			entry.drakkin_tattoo         = atoi(row[41]);
			entry.drakkin_details        = atoi(row[42]);
			entry.armortint_id           = atoi(row[43]);
			entry.armortint_red          = atoi(row[44]);
			entry.armortint_green        = atoi(row[45]);
			entry.armortint_blue         = atoi(row[46]);
			entry.d_melee_texture1       = atoi(row[47]);
			entry.d_melee_texture2       = atoi(row[48]);
			entry.ammo_idfile            = row[49] ? row[49] : "";
			entry.prim_melee_type        = atoi(row[50]);
			entry.sec_melee_type         = atoi(row[51]);
			entry.ranged_type            = atoi(row[52]);
			entry.runspeed               = static_cast<float>(atof(row[53]));
			entry.MR                     = atoi(row[54]);
			entry.CR                     = atoi(row[55]);
			entry.DR                     = atoi(row[56]);
			entry.FR                     = atoi(row[57]);
			entry.PR                     = atoi(row[58]);
			entry.Corrup                 = atoi(row[59]);
			entry.PhR                    = atoi(row[60]);
			entry.see_invis              = atoi(row[61]);
			entry.see_invis_undead       = atoi(row[62]);
			entry.qglobal                = atoi(row[63]);
			entry.AC                     = atoi(row[64]);
			entry.npc_aggro              = atoi(row[65]);
			entry.spawn_limit            = atoi(row[66]);
			entry.attack_speed           = static_cast<float>(atof(row[67]));
			entry.attack_delay           = atoi(row[68]);
			entry.findable               = atoi(row[69]);
			entry.STR                    = atoi(row[70]);
			entry.STA                    = atoi(row[71]);
			entry.DEX                    = atoi(row[72]);
			entry.AGI                    = atoi(row[73]);
			entry._INT                   = atoi(row[74]);
			entry.WIS                    = atoi(row[75]);
			entry.CHA                    = atoi(row[76]);
			entry.see_hide               = atoi(row[77]);
			entry.see_improved_hide      = atoi(row[78]);
			entry.trackable              = atoi(row[79]);
			entry.isbot                  = atoi(row[80]);
			entry.exclude                = atoi(row[81]);
			entry.ATK                    = atoi(row[82]);
			entry.Accuracy               = atoi(row[83]);
			entry.Avoidance              = atoi(row[84]);
			entry.slow_mitigation        = atoi(row[85]);
			entry.version                = atoi(row[86]);
			entry.maxlevel               = atoi(row[87]);
			entry.scalerate              = atoi(row[88]);
			entry.private_corpse         = atoi(row[89]);
			entry.unique_spawn_by_name   = atoi(row[90]);
			entry.underwater             = atoi(row[91]);
			entry.isquest                = atoi(row[92]);
			entry.emoteid                = atoi(row[93]);
			entry.spellscale             = static_cast<float>(atof(row[94]));
			entry.healscale              = static_cast<float>(atof(row[95]));
			entry.no_target_hotkey       = atoi(row[96]);
			entry.raid_target            = atoi(row[97]);
			entry.armtexture             = atoi(row[98]);
			entry.bracertexture          = atoi(row[99]);
			entry.handtexture            = atoi(row[101]);
			entry.legtexture             = atoi(row[101]);
			entry.feettexture            = atoi(row[102]);
			entry.light                  = atoi(row[103]);
			entry.walkspeed              = atoi(row[104]);
			entry.peqid                  = atoi(row[105]);
			entry.unique_                = atoi(row[106]);
			entry.fixed                  = atoi(row[107]);
			entry.ignore_despawn         = atoi(row[108]);
			entry.show_name              = atoi(row[109]);
			entry.untargetable           = atoi(row[110]);
			entry.charm_ac               = atoi(row[111]);
			entry.charm_min_dmg          = atoi(row[112]);
			entry.charm_max_dmg          = atoi(row[113]);
			entry.charm_attack_delay     = atoi(row[114]);
			entry.charm_accuracy_rating  = atoi(row[115]);
			entry.charm_avoidance_rating = atoi(row[116]);
			entry.charm_atk              = atoi(row[117]);
			entry.skip_global_loot       = atoi(row[118]);
			entry.rare_spawn             = atoi(row[119]);
			entry.stuck_behavior         = atoi(row[120]);
			entry.model                  = atoi(row[121]);
			entry.flymode                = atoi(row[122]);
			entry.always_aggro           = atoi(row[123]);
			entry.exp_mod                = atoi(row[124]);
