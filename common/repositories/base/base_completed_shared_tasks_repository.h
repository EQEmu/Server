...
public:
	...
	)
	{
		...
		update_values.push_back(columns[3] + " = FROM_UNIXTIME(" + (completed_shared_tasks_entry.completion_time > 0 ? std::to_string(completed_shared_tasks_entry.completion_time) : "null") + ")");
		update_values.push_back(columns[4] + " = " + std::to_string(completed_shared_tasks_entry.is_locked));
