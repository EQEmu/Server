...
class SendReqPool {
public:
	explicit SendReqPool(size_t pool_size) {
		for (size_t i = 0; i < pool_size; ++i) {
			pool.push(std::make_unique<uv_udp_send_t>());
		}
	}

	std::unique_ptr<uv_udp_send_t> acquire() {
		std::lock_guard<std::mutex> lock(pool_mutex);
		if (pool.empty()) {
			return std::make_unique<uv_udp_send_t>();  // Fallback: dynamically allocate
		}
		auto req = std::move(pool.top());
		pool.pop();
		return req;
	}

	void release(std::unique_ptr<uv_udp_send_t> req) {
		std::lock_guard<std::mutex> lock(pool_mutex);
		pool.push(std::move(req));
	}

private:
	std::stack<std::unique_ptr<uv_udp_send_t>> pool;
	...
	std::optional<std::pair<size_t, char*>> acquireBuffer() {
		...
		for (size_t i = 0; i < capacity; ++i) {
			...
			if (locks[index].compare_exchange_strong(expected, true)) {
				head = (index + 1) % capacity;
				return std::make_pair(index, buffer[index].data());
			}
		}

		// Buffer is full, grow and try one more time
		std::cout << "Growing buffer from " << capacity << " to " << (capacity * 2) << std::endl;
		grow();
...
private:
	...
	std::unique_ptr<std::atomic_bool[]> locks;  // C++20 friendly allocation
	size_t capacity;
	size_t head, tail;
	...
	void grow() {
		...
		std::cout << "Growing buffer from " << capacity << " to " << new_capacity << std::endl;

		buffer.resize(new_capacity);

		// Allocate new lock array
		auto new_locks = std::make_unique<std::atomic_bool[]>(new_capacity);

		// Copy old lock states and initialize new ones
		for (size_t i = 0; i < capacity; ++i) {
